...
unsigned int DecodeUtf8(const unsigned char *s, size_t len, size_t &i)
{
    ...
    if ((b0 & 0xF0) == 0xE0 && cont(1) && cont(2))
    {
        const unsigned int cp =
            ((b0 & 0x0Fu) << 12) | ((s[i + 1] & 0x3Fu) << 6) | (s[i + 2] & 0x3Fu);
        i += 3;
                        ...
                        int cbMultiByte, LPWSTR lpWideCharStr, int cchWideChar)
{
    ...
    const size_t srcLen =
        (cbMultiByte < 0) ? (std::strlen(lpMultiByteStr) + 1) : static_cast<size_t>(cbMultiByte);
                        ...
                        LPCSTR /*lpDefaultChar*/, LPBOOL /*lpUsedDefaultChar*/)
{
    ...
    // -1 means null-terminated; include the terminator in the processed length.
    const size_t srcLen =
        (cchWideChar < 0) ? (std::wcslen(lpWideCharStr) + 1) : static_cast<size_t>(cchWideChar);
...
int32_t CMultiLanguage::ConvertToUtf8(char *target, const wchar_t *source, int maxSourceLength)
{
    ...
    const int requiredBytesWithNull =
        WideCharToMultiByte(CP_UTF8, 0, source, -1, nullptr, 0, nullptr, nullptr);
    ...
    if (maxSourceLength > 0)
    {
        ...
        const int writtenWithNull = WideCharToMultiByte(CP_UTF8, 0, source, -1, tmp.data(),
                                                        requiredBytesWithNull, nullptr, nullptr);
    ...
    const int written =
        WideCharToMultiByte(CP_UTF8, 0, source, -1, target, requiredBytes, nullptr, nullptr);
    if (written <= 0)
    {
        target[0] = '\0';
        return 0;
