...
enum
{
    ...
    BITMAP_MYSHOPINTERFACE_NEW_PERSONALINVENTORY_END =
        BITMAP_MYSHOPINTERFACE_NEW_PERSONALINVENTORY_BEGIN + 18,
...
// True-colour TGA, raw or RLE, 24/32 bits, all four image origins.
...
// Boundary conversion from legacy GL_NEAREST/GL_LINEAR/GL_REPEAT/
// GL_CLAMP_TO_EDGE/GL_CLAMP constants to the portable CGlobalBitmap sampler
// enums (PLAN_P1R5.1.md section 5.2). The hundreds of call sites that pass
// these source values into the logical-asset catalog.
// OpenTexture stay unchanged; only the CGlobalBitmap doorway converts.
...
// Borrowed by exact-session effects. Its address survives unload/reload; only
// binding changes replace the facts, so rendering needs no namespace lookup.
struct SessionTexturePropertiesSlot final
{
    std::optional<SessionTextureProperties> properties;
    LogicalRenderAssetRef asset;
};

class SessionTextureNamespace final
{
  public:
    explicit SessionTextureNamespace(CGlobalBitmap &bitmaps) noexcept;
    ~SessionTextureNamespace();

    SessionTextureNamespace(const SessionTextureNamespace &) = delete;
    SessionTextureNamespace &operator=(const SessionTextureNamespace &) = delete;

    bool Load(std::uint32_t logicalIndex, const std::wstring &filename, CErrorReport &errorReport,
              LegacyTextureFilter filter, LegacyTextureWrap wrapMode);
    std::uint32_t LoadUnnamed(const std::wstring &filename, CErrorReport &errorReport,
                              LegacyTextureFilter filter, LegacyTextureWrap wrapMode);
    void Unload(std::uint32_t logicalIndex) noexcept;
    // Exchange ownership at world activation/retirement, retaining exact reference counts.
    void SwapBinding(std::uint32_t logicalIndex, SessionTextureNamespace &other);
    void RestorePreparedBinding(std::uint32_t logicalIndex,
                                const SessionTextureNamespace &prepared);
    void SelectPreparedTexture(std::uint32_t logicalIndex,
                               const LogicalRenderAssetLease &texture) noexcept;
    std::optional<LogicalRenderAssetRef> Resolve(std::uint32_t logicalIndex) const noexcept;
    ...
    std::optional<SessionTextureProperties> TryGetProperties(
        std::uint32_t logicalIndex) const noexcept;
    const SessionTexturePropertiesSlot &BindProperties(std::uint32_t logicalIndex);
    std::optional<LogicalRenderAssetMetadata> TryDescribeByName(
        const std::wstring &name) const noexcept;
    std::optional<std::uint32_t> RetainByName(const std::wstring &name) noexcept;
    ...
    bool CommitOwnerProducedRevision(LogicalRenderAssetRef asset, std::uint32_t width,
                                     ...
                                     std::shared_ptr<const std::vector<std::byte>> rgba8) noexcept;
    static bool IsTerrainLogicalIndex(std::uint32_t logicalIndex) noexcept;
    static bool IsApplicationSharedLogicalIndex(std::uint32_t logicalIndex) noexcept;

  private:
    struct Binding final
    {
        std::uint32_t legacyIndex = 0;
        LogicalRenderAssetRef asset;
        LogicalRenderAssetMetadata metadata;
        LogicalRenderAssetLease lease;
        SessionTextureProperties properties;
        std::size_t references = 1;
        mutable const SessionTexturePropertiesSlot *propertySlot = nullptr;
    };

    const Binding *FindCurrentBinding(std::uint32_t logicalIndex) const noexcept;
    void CacheDenseBinding(std::uint32_t logicalIndex, const Binding *binding) noexcept;
    void RefreshBoundProperties(std::uint32_t logicalIndex, const Binding *binding) noexcept;

    CGlobalBitmap &bitmaps_;
    std::unordered_map<std::uint32_t, Binding> bindings_;
    std::vector<const Binding *> denseBindings_;
    std::unordered_map<std::uint32_t, SessionTexturePropertiesSlot> propertySlots_;
    mutable std::uint32_t lastLogicalIndex_ = 0;
    mutable const Binding *lastBinding_ = nullptr;
};
...
namespace UI::Modern
{
// Owns the linker's textures on the application thread, before RmlUI starts.
class UiTextureLibrary final
{
  public:
    explicit UiTextureLibrary(CGlobalBitmap &bitmaps) noexcept;
    ~UiTextureLibrary();
    UiTextureLibrary(const UiTextureLibrary &) = delete;
    UiTextureLibrary &operator=(const UiTextureLibrary &) = delete;
    bool Load(const std::filesystem::path &linker, CErrorReport &errors);
...
// Explicit effect and scene textures loaded by map entry, relative to Data.
struct WorldTextureDependency final
{
    std::uint32_t slot;
    const wchar_t *path;
    LegacyTextureFilter filter = LegacyTextureFilter::Nearest;
    LegacyTextureWrap wrap = LegacyTextureWrap::ClampToEdge;
    static std::span<const WorldTextureDependency> Common() noexcept;
    static std::span<const WorldTextureDependency> For(int behaviorMap) noexcept;
};

// Shared native inventory frame asset (newui_item_back03.tga).
inline constexpr int BITMAP_INVENTORY_BACK_BOTTOM =
    BITMAP_INTERFACE_NEW_PERSONALINVENTORY_BEGIN + 4;
