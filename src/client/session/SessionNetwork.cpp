...
// Generated packet-size assertions must follow the declarations in SessionNetwork.h.
...
#include "app/ApplicationNetwork.h" // GetAddrInfoW + BSD-socket shims for the reachability probe
...
#include "ui/session/UiSessionLogic.h" // CUIMng, m_LoginWin
...
ReconnectManager::ReconnectManager(SessionKeeper &keeper) noexcept
    : SessionLegacyCalls(keeper), legacyUiManager_(keeper.LegacyUiManagerForConstruction()),
      muHelper_(keeper.MuHelperForConstruction()), WorldTime(keeper.FrameWorldTime())
{
}
...
void ReconnectManager::RequestBegin()
{
    ...
    m_muHelperWasActive = muHelper_.IsActive();
...
void ReconnectManager::UpdateConnecting()
{
    ...
    {
        CUIMng &uiMng = LegacyUiManager();
        uiMng.HideWin(uiMng.m_LoginWin.get());
...
{
    if (m_characterName[0] == L'\0' || CharactersClient.Size() == 0)
    {
        return false;
    }

    for (int i = 0; i < MAX_CHARACTERS_PER_ACCOUNT && i < CharactersClient.Size(); ++i)
    {
...
CSMServer::CSMServer(SessionKeeper &keeper) : SessionLegacyCalls(keeper)
{
    Init();
}
...
bool CSMServer::MatchesServer(const wchar_t *address, std::uint16_t port) const noexcept
{
    if (!m_hasServerInfo || address == nullptr || port != m_serverInfo.m_wMapSvrPort)
    {
        return false;
    }
    std::array<wchar_t, 16> mapAddress{};
    CMultiLanguage::ConvertFromUtf8(mapAddress.data(), m_serverInfo.m_szMapSvrIpAddress.data(),
                                    static_cast<int>(mapAddress.size()));
    return std::wcscmp(address, mapAddress.data()) == 0;
}

bool CSMServer::SendChangeMapServer()
{
    if (!m_hasServerInfo || LogIn == 0)
    {
        return false;
    ...
    CMultiLanguage::ConvertToUtf8(characterName.data(), m_heroId.c_str(),
    ...
    SocketClient->ToGameServer()->SendServerChangeAuthentication(
        reinterpret_cast<BYTE *>(loginId.data()), MAX_USERNAME_SIZE,
        reinterpret_cast<BYTE *>(characterName.data()), MAX_USERNAME_SIZE,
        m_serverInfo.m_iJoinAuthCode1, m_serverInfo.m_iJoinAuthCode2, m_serverInfo.m_iJoinAuthCode3,
        m_serverInfo.m_iJoinAuthCode4, GetCurrentTickMilliseconds(), Version, SIZE_PROTOCOLVERSION,
        Serial, SIZE_PROTOCOLSERIAL);
    return true;
}

CUIMng &SessionNetworkUnit::LegacyUiManager() noexcept
{
    return ui_.LegacyUiManager();
}

SessionNetworkUnit::SessionNetworkUnit(SessionKeeper &keeper) noexcept
    : SessionLegacyCalls(keeper), gameplay_(keeper.GameplayForConstruction()),
      ui_(keeper.UiForConstruction()), cursedTemple_(keeper.CursedTempleObject()),
      muHelper_(keeper.MuHelperForConstruction()), Items(keeper.ItemsStorage()),
      ...
      g_nTaxRate(keeper.TaxRate()), g_nChaosTaxRate(keeper.ChaosTaxRate()),
      g_iGoalEffect(keeper.GoalEffect()), g_ErrorReport(keeper.ErrorReport()),
      ...
      gSkillManager(keeper.SkillManagerObject()), g_SummonSystem(keeper.SummonSystemObject()),
      g_CMonkSystem(keeper.MonkSystemObject()), g_MessageBox(keeper.MessageBoxManagerObject()),
      g_PortalMgr(keeper.PortalManagerObject()), g_petProcess(keeper.PetProcessObject()),
      g_bUseWindowMode(keeper.PlatformWindowMode()), WindowWidth(keeper.PlatformWindowWidth()),
      WindowHeight(keeper.PlatformWindowHeight()), g_pFriendMenu(keeper.FriendMenuObject()),
      g_InGameShopSystem(keeper.InGameShopSystemObject()), g_pUIMapName(ui_.MapName()),
      g_MixRecipeMgr(keeper.MixRecipeManager()), g_csQuest(keeper.QuestObject()),
      g_QuestMng(keeper.QuestManagerObject()), notices_(ui_.Notices()),
      managedBindings_(keeper.ManagedBindingsForConstruction()),
      applicationNetwork_(keeper.ApplicationNetworkForConstruction()),
      sessionManager_(keeper.SessionManagerForConstruction()), csMapServer_(keeper)
{
    (void)sessionKeeper_.RegisterNetwork(*this);
}

bool SessionNetworkUnit::InitializeConnectedChildren()
{
    if (reconnect_)
    {
        return false;
    }
    reconnect_ = std::make_unique<ReconnectManager>(sessionKeeper_);
    return true;
}

SessionNetworkUnit::~SessionNetworkUnit()
{
    DeleteSocket();
}

std::unique_ptr<Connection> SessionNetworkUnit::CreateConnection(const wchar_t *address,
                                                                 unsigned short port,
                                                                 bool encrypted)
{
    return std::make_unique<Connection>(sessionKeeper_.ApplicationKeeperRef(), managedBindings_,
                                        applicationNetwork_, address, port, encrypted);
}

bool SessionNetworkUnit::PublishConnection(Connection &connection, ConnectionRole role)
{
    return applicationNetwork_
        .RegisterAndBeginReceive(connection, sessionManager_, sessionKeeper_.Id(), role)
        .has_value();
}

void SessionNetworkUnit::SendPing(DWORD tickCount, int attackSpeed)
{
    if (g_bGameServerConnected && SocketClient != nullptr)
    {
        SocketClient->ToGameServer()->SendPing(tickCount, attackSpeed);
    }
}

void SessionNetworkUnit::SendCheatDetectionLogout()
{
...
bool SessionNetworkUnit::BeginGameplaySendCapture(SessionOrderedEffectBatch &effects,
                                                  std::uint64_t &nextStableSequence) noexcept
{
    if (gameplaySendCaptureActive_)
    {
        return false;
    }

    gameplaySendCaptureActive_ = true;
    capturedSendConnection_ = nullptr;
    if (SocketClient == nullptr || !SocketClient->IsConnected())
    {
        return true;
    }

    capturedSendConnection_ = SocketClient;
    if (!capturedSendConnection_->BeginOrderedSendCapture(effects, nextStableSequence))
    {
        capturedSendConnection_ = nullptr;
        gameplaySendCaptureActive_ = false;
        return false;
    }
    return true;
}

bool SessionNetworkUnit::EndGameplaySendCapture() noexcept
{
    if (!gameplaySendCaptureActive_)
    {
        return false;
    ...
    gameplaySendCaptureActive_ = false;
    Connection *connection = capturedSendConnection_;
    capturedSendConnection_ = nullptr;
    return connection == nullptr || connection->EndOrderedSendCapture();
}

bool SessionNetworkUnit::CanCommitCapturedSend(SessionId sessionId,
                                               std::uint32_t channel) const noexcept
{
    return SocketClient != nullptr && SocketClient->CanCommitCapturedSend(sessionId, channel);
}

void SessionNetworkUnit::CommitCapturedSend(std::span<const std::byte> payload) noexcept
{
    if (SocketClient != nullptr)
    {
        SocketClient->CommitCapturedSend(payload);
    }
}
...
DWORD GetCheckSum(WORD wKey)
{
    ...
    HANDLE hFile = CreateFile(lpszFile, GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING,
...
bool CServerListManager::IsStrifeMap(int nMapIndex)
{
...
void SessionNetworkUnit::GateOpen(CHARACTER *c, OBJECT *o)
{
    SetAction(o, 1);

    g_CharacterRegisterBuff(o, eBuff_CastleGateIsOpen);

    SetCastleGate_Attribute((c->PositionX), (c->PositionY), 0);

    PlayBuffer(SOUND_BC_GATE_OPEN);
}

void SessionNetworkUnit::GateClose(CHARACTER *c, OBJECT *o)
{
    SetAction(o, 0);

    g_CharacterClearBuff(o);

    SetCastleGate_Attribute((c->PositionX), (c->PositionY), 1);

    PlayBuffer(SOUND_BC_GATE_OPEN);
}

void SessionNetworkUnit::DoInterfaceOpen(int Key)
{
    npcCharacterKey_ = Key;

    int Index = FindCharacterIndex(Key);
    CHARACTER *c = &CharactersClient[Index];
    OBJECT *o = &c->Object;

    isCurrentGateOpen_ = g_isCharacterBuff(o, eBuff_CastleGateIsOpen);
}

void SessionNetworkUnit::ProcessState(int Key, BYTE GateOnOff, BYTE State)
{
    switch (State)
    {
    case 0:
...
void SessionNetworkUnit::SendToggleGate()
{
    int State = 0;
    if (!IsGateOpened())
    {
    ...
    SocketClient->ToGameServer()->SendToggleCastleGateRequest(State, npcCharacterKey_);
    npcCharacterKey_ = 0;
}

bool SessionNetworkUnit::IsGateOpened()
{
    return isCurrentGateOpen_;
}
...
void SessionNetworkUnit::AddDebugText(const unsigned char *Buffer, int Size)
{
...
void SessionLegacyCalls::ReceiveChainMagic(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveChainMagic(receiveBuffer);
}
void SessionLegacyCalls::ReceiveSoccerScore(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveSoccerScore(receiveBuffer);
}
...
static int64_t GetMasterLowerBound(const short masterLevel)
{
    ...
    const auto saturatingMul = [](const int64_t left, const int64_t right) {
        ...
        const uint64_t resultLimit =
            isNegativeResult ? (uint64_t{1} << 63) : static_cast<uint64_t>(kInt64Max);
    ...
    const int64_t totalLevel =
        saturatingAdd(static_cast<int64_t>(masterLevel), static_cast<int64_t>(400));
    const int64_t overLevel = saturatingSub(totalLevel, static_cast<int64_t>(255));

    const int64_t leftTerm = saturatingMul(
        saturatingMul(saturatingMul(saturatingAdd(static_cast<int64_t>(9), totalLevel), totalLevel),
                      totalLevel),
        static_cast<int64_t>(10));
    const int64_t rightTerm = saturatingMul(
        saturatingMul(saturatingMul(saturatingAdd(static_cast<int64_t>(9), overLevel), overLevel),
                      overLevel),
...
BOOL SessionNetworkUnit::CreateSocket(const wchar_t *IpAddr, unsigned short Port)
{
    BOOL bResult = TRUE;
    g_ConsoleDebug.Write(MCD_NORMAL, L"[Connect to Server] ip address = %ls, port = %d", IpAddr,
                         Port);
    ...
    const ConnectionRole role =
        !isEncrypted ? ConnectionRole::ConnectServer
                     : (csMapServer_.MatchesServer(IpAddr, Port) ? ConnectionRole::MapServer
                                                                 : ConnectionRole::GameServer);
    std::unique_ptr<Connection> connection = CreateConnection(IpAddr, Port, isEncrypted);
    if (!connection->IsConnected() || !PublishConnection(*connection, role))
    {
        bResult = FALSE;
    ...
    {
        gameplay_.serverConnectionClosed_ = FALSE;
        if (isEncrypted)
        {
            ...
            Reconnect().CacheServer(IpAddr, Port);
        }
    }

    if (bResult)
    {
        SocketClient = connection.release();
    }

    return bResult;
}

BOOL SessionLegacyCalls::CreateSocket(const wchar_t *ipAddress, unsigned short port)
{
    return sessionKeeper_.Network()->CreateSocket(ipAddress, port);
}

void SessionNetworkUnit::DeleteSocket()
{
    if (SocketClient != nullptr)
    {
        Connection *connection = SocketClient;
        SocketClient = nullptr;
        delete connection;
    }
}

void SessionLegacyCalls::DeleteSocket()
{
    sessionKeeper_.Network()->DeleteSocket();
}

int SessionNetworkUnit::FindGuildName(wchar_t *Name)
{
...
void SessionNetworkUnit::GuildTeam(CHARACTER *c)
{
    OBJECT *o = &c->Object;
    if (o->Live)
    {
...
void SessionLegacyCalls::ReceiveCreateTransformViewport(std::span<const BYTE> receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCreateTransformViewport(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCreateSummonViewport(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCreateSummonViewport(receiveBuffer);
}
void SessionLegacyCalls::ReceiveGetItem(std::span<const BYTE> receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveGetItem(receiveBuffer);
}
void SessionLegacyCalls::ReceivePartyGetItem(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceivePartyGetItem(receiveBuffer);
}
void SessionLegacyCalls::ReceiveNPCDlgUIStart(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveNPCDlgUIStart(receiveBuffer);
}
void SessionLegacyCalls::ReceivePreviewPort(std::span<const BYTE> receiveBuffer)
{
    sessionKeeper_.Network()->ReceivePreviewPort(receiveBuffer);
}
bool SessionLegacyCalls::ReceiveIGS_UpdateScript(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveIGS_UpdateScript(receiveBuffer);
}
void SessionLegacyCalls::ReceiveQuestQSSelSentence(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveQuestQSSelSentence(receiveBuffer);
}
void SessionLegacyCalls::ReceiveQuestQSRequestReward(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveQuestQSRequestReward(receiveBuffer);
}
void SessionLegacyCalls::ReceiveDuelRequest(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDuelRequest(receiveBuffer);
}
void SessionLegacyCalls::ReceiveDuelStart(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDuelStart(receiveBuffer);
}
void SessionLegacyCalls::ReceiveDuelEnd(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDuelEnd(receiveBuffer);
}
void SessionLegacyCalls::ReceiveDuelWatchRequestReply(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDuelWatchRequestReply(receiveBuffer);
}
void SessionLegacyCalls::ReceivePK(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceivePK(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCreateShopTitleViewport(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCreateShopTitleViewport(receiveBuffer);
}
void SessionLegacyCalls::ReceiveShopTitleChange(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveShopTitleChange(receiveBuffer);
}
void SessionLegacyCalls::ReceiveServerList(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveServerList(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCharacterListExtended(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCharacterListExtended(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCharacterCard_New(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCharacterCard_New(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCreateCharacter(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCreateCharacter(receiveBuffer);
}
BOOL SessionLegacyCalls::ReceiveLogOut(const BYTE *receiveBuffer, BOOL encrypted)
{
    return sessionKeeper_.Network()->ReceiveLogOut(receiveBuffer, encrypted);
}
void SessionLegacyCalls::ReceiveRevival(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveRevival(receiveBuffer);
}
void SessionLegacyCalls::ReceiveMagicList(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveMagicList(receiveBuffer);
}
void SessionLegacyCalls::Receive_Master_SetSkillList(PMSG_MASTER_SKILL_LIST_SEND *message)
{
    sessionKeeper_.Network()->Receive_Master_SetSkillList(message);
}
void SessionLegacyCalls::ReceiveMuHelperConfigurationData(std::span<const BYTE> receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveMuHelperConfigurationData(receiveBuffer);
}
void SessionLegacyCalls::ReceiveConfirmPassword(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveConfirmPassword(receiveBuffer);
}
void SessionLegacyCalls::ReceiveConfirmPassword2(const std::span<const BYTE> receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveConfirmPassword2(receiveBuffer);
}
void SessionLegacyCalls::ReceiveChangePassword(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveChangePassword(receiveBuffer);
}
void SessionLegacyCalls::ReceiveDeleteInventory(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDeleteInventory(receiveBuffer);
}
int SessionLegacyCalls::CalcItemLength(std::span<const BYTE> receiveBuffer)
{
    return sessionKeeper_.Network()->CalcItemLength(receiveBuffer);
}
BOOL SessionLegacyCalls::ReceiveInventoryExtended(std::span<const BYTE> receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveInventoryExtended(receiveBuffer);
}
void SessionLegacyCalls::ReceiveTradeInventoryExtended(std::span<const BYTE> receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveTradeInventoryExtended(receiveBuffer);
}
void SessionLegacyCalls::ReceiveEquipment(std::span<const BYTE> receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveEquipment(receiveBuffer);
}
void SessionLegacyCalls::ReceiveChat(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveChat(receiveBuffer);
}
void SessionLegacyCalls::ReceiveChatKey(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveChatKey(receiveBuffer);
}
void SessionLegacyCalls::InitGame()
{
    sessionKeeper_.Network()->InitGame();
}
void SessionLegacyCalls::ReceiveChatWhisper(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveChatWhisper(receiveBuffer);
}
void SessionLegacyCalls::ReceiveMovePosition(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveMovePosition(receiveBuffer);
}
void SessionLegacyCalls::FallingStartCharacter(CHARACTER *character, OBJECT *object)
{
    sessionKeeper_.Network()->FallingStartCharacter(character, object);
}
void SessionLegacyCalls::ReceiveParty(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveParty(receiveBuffer);
}
void SessionLegacyCalls::ReceiveGuild(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveGuild(receiveBuffer);
}
void SessionLegacyCalls::ReceiveGuildEndWar(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveGuildEndWar(receiveBuffer);
}
void SessionLegacyCalls::ReceiveDeclareWar(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDeclareWar(receiveBuffer);
}
void SessionLegacyCalls::ReceiveGuildIDViewport(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveGuildIDViewport(receiveBuffer);
}
void SessionLegacyCalls::ReceiveServerCommand(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveServerCommand(receiveBuffer);
}
void SessionLegacyCalls::ReceiveGemMixResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveGemMixResult(receiveBuffer);
}
void SessionLegacyCalls::ReceiveGemUnMixResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveGemUnMixResult(receiveBuffer);
}
void SessionLegacyCalls::ReceiveMoveToDevilSquareResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveMoveToDevilSquareResult(receiveBuffer);
}
void SessionLegacyCalls::ReceiveDevilSquareOpenTime(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDevilSquareOpenTime(receiveBuffer);
}
void SessionLegacyCalls::ReceiveDevilSquareCountDown(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDevilSquareCountDown(receiveBuffer);
}
void SessionLegacyCalls::ReceiveMoveToEventMatchResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveMoveToEventMatchResult(receiveBuffer);
}
void SessionLegacyCalls::ReceiveEventZoneOpenTime(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveEventZoneOpenTime(receiveBuffer);
}
void SessionLegacyCalls::ReceiveMoveToEventMatchResult2(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveMoveToEventMatchResult2(receiveBuffer);
}
void SessionLegacyCalls::ReceiveDuelResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDuelResult(receiveBuffer);
}
void SessionLegacyCalls::ReceiveDisplayEffectViewport(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDisplayEffectViewport(receiveBuffer);
}
void SessionLegacyCalls::ReceiveServerImmigration(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveServerImmigration(receiveBuffer);
}
void SessionLegacyCalls::ReceiveScratchResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveScratchResult(receiveBuffer);
}
void SessionLegacyCalls::ReceiveOtherPlayerGensInfluenceViewport(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveOtherPlayerGensInfluenceViewport(receiveBuffer);
}
void SessionLegacyCalls::ReceiveUseStateItem(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveUseStateItem(receiveBuffer);
}
void SessionLegacyCalls::ReceiveQuestPrize(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveQuestPrize(receiveBuffer);
}
void SessionLegacyCalls::ReceiveKanturu3rdCurrentState(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveKanturu3rdCurrentState(receiveBuffer);
}
void SessionLegacyCalls::ReceiveKanturu3rdState(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveKanturu3rdState(receiveBuffer);
}
void SessionLegacyCalls::ReceiveKanturu3rdResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveKanturu3rdResult(receiveBuffer);
}
void SessionLegacyCalls::RecevieKanturu3rdMayaSKill(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->RecevieKanturu3rdMayaSKill(receiveBuffer);
}
void SessionLegacyCalls::RecevieKanturu3rdLeftUserandMonsterCount(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->RecevieKanturu3rdLeftUserandMonsterCount(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCrownSwitchState(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCrownSwitchState(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCrownRegist(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCrownRegist(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCrownState(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCrownState(receiveBuffer);
}
void SessionLegacyCalls::ReceiveBattleCastleRegiment(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveBattleCastleRegiment(receiveBuffer);
}
void SessionLegacyCalls::ReceiveBattleCastleStart(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveBattleCastleStart(receiveBuffer);
}
void SessionLegacyCalls::ReceiveBattleCastleProcess(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveBattleCastleProcess(receiveBuffer);
}
void SessionLegacyCalls::ReceiveGateState(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveGateState(receiveBuffer);
}
void SessionLegacyCalls::ReceiveGateOperator(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveGateOperator(receiveBuffer);
}
void SessionLegacyCalls::ReceiveGateCurrentState(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveGateCurrentState(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCastleHuntZoneInfo(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCastleHuntZoneInfo(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCastleHuntZoneResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCastleHuntZoneResult(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCrywolfAltarContract(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCrywolfAltarContract(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCrywolfInfo(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCrywolfInfo(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCrywolStateAltarfInfo(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCrywolStateAltarfInfo(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCrywolfLifeTime(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCrywolfLifeTime(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCrywolfTankerHit(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCrywolfTankerHit(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCrywolfBossMonsterInfo(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCrywolfBossMonsterInfo(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCrywolfPersonalRank(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCrywolfPersonalRank(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCrywolfHeroList(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCrywolfHeroList(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCursedTempleEnterResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCursedTempleEnterResult(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCursedTempleInfo(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCursedTempleInfo(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCursedTempleState(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCursedTempleState(receiveBuffer);
}
bool SessionLegacyCalls::ReceiveRegistLuckyCoin(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveRegistLuckyCoin(receiveBuffer);
}
bool SessionLegacyCalls::ReceiveRequestExChangeLuckyCoin(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveRequestExChangeLuckyCoin(receiveBuffer);
}
bool SessionLegacyCalls::ReceiveDoppelGangerState(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveDoppelGangerState(receiveBuffer);
}
bool SessionLegacyCalls::ReceiveDoppelGangerResult(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveDoppelGangerResult(receiveBuffer);
}
bool SessionLegacyCalls::ReceiveResultEmpireGuardian(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveResultEmpireGuardian(receiveBuffer);
}
bool SessionLegacyCalls::ReceiveIGS_BuyItem(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveIGS_BuyItem(receiveBuffer);
}
bool SessionLegacyCalls::ReceiveIGS_SendItemGift(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveIGS_SendItemGift(receiveBuffer);
}
bool SessionLegacyCalls::ReceiveIGS_UseStorageItem(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveIGS_UseStorageItem(receiveBuffer);
}
bool SessionLegacyCalls::ReceiveIGS_CashPoint(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveIGS_CashPoint(receiveBuffer);
}
bool SessionLegacyCalls::ReceiveIGS_ShopOpenResult(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveIGS_ShopOpenResult(receiveBuffer);
}
bool SessionLegacyCalls::ReceiveIGS_StorageItemListCount(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveIGS_StorageItemListCount(receiveBuffer);
}
bool SessionLegacyCalls::ReceiveIGS_StorageItemList(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveIGS_StorageItemList(receiveBuffer);
}
bool SessionLegacyCalls::ReceiveIGS_StorageGiftItemList(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveIGS_StorageGiftItemList(receiveBuffer);
}
bool SessionLegacyCalls::ReceiveIGS_EventItemlistCnt(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveIGS_EventItemlistCnt(receiveBuffer);
}
bool SessionLegacyCalls::ReceiveIGS_EventItemlist(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveIGS_EventItemlist(receiveBuffer);
}
bool SessionLegacyCalls::ReceiveIGS_UpdateBanner(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveIGS_UpdateBanner(receiveBuffer);
}
void SessionLegacyCalls::ReceiveNotice(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveNotice(receiveBuffer);
}
void SessionLegacyCalls::ReceiveFriendList(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveFriendList(receiveBuffer);
}
void SessionLegacyCalls::ReceiveAddFriendResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveAddFriendResult(receiveBuffer);
}
void SessionLegacyCalls::ReceiveRequestAcceptAddFriend(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveRequestAcceptAddFriend(receiveBuffer);
}
void SessionLegacyCalls::ReceiveDeleteFriendResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDeleteFriendResult(receiveBuffer);
}
void SessionLegacyCalls::ReceiveFriendStateChange(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveFriendStateChange(receiveBuffer);
}
void SessionLegacyCalls::ReceiveLetterSendResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveLetterSendResult(receiveBuffer);
}
void SessionLegacyCalls::ReceiveLetter(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveLetter(receiveBuffer);
}
void SessionLegacyCalls::ReceiveLetterDeleteResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveLetterDeleteResult(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCreateChatRoomResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCreateChatRoomResult(receiveBuffer);
}
void SessionLegacyCalls::ReceiveChatRoomInviteResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveChatRoomInviteResult(receiveBuffer);
}
void SessionLegacyCalls::ReceiveChangePlayer(std::span<const BYTE> receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveChangePlayer(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCreatePlayerViewportExtended(std::span<const BYTE> receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCreatePlayerViewportExtended(receiveBuffer);
}
void SessionLegacyCalls::ReceiveDeleteCharacterViewport(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDeleteCharacterViewport(receiveBuffer);
}
void SessionLegacyCalls::ReceiveDeleteCharacter(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDeleteCharacter(receiveBuffer);
}
void SessionLegacyCalls::ReceiveAttackDamage(CHARACTER *character, OBJECT *object,
                                             const bool success, const int key, const int damage,
                                             const int shieldDamage, const int damageType,
                                             const bool repeatedly, const bool endRepeatedly,
                                             const bool doubleEnable, const bool comboEnable)
{
    sessionKeeper_.Network()->ReceiveAttackDamage(character, object, success, key, damage,
                                                  shieldDamage, damageType, repeatedly,
                                                  endRepeatedly, doubleEnable, comboEnable);
}
void SessionLegacyCalls::ReceiveAttackDamageCastle(CHARACTER *character, OBJECT *object,
                                                   const bool success, const int key,
                                                   const int damage, const int shieldDamage,
                                                   const int damageType, const bool repeatedly,
                                                   const bool endRepeatedly,
                                                   const bool doubleEnable, const bool comboEnable)
{
    sessionKeeper_.Network()->ReceiveAttackDamageCastle(character, object, success, key, damage,
                                                        shieldDamage, damageType, repeatedly,
                                                        endRepeatedly, doubleEnable, comboEnable);
}
void SessionLegacyCalls::ReceiveMagicPosition(const BYTE *receiveBuffer, int size)
{
    sessionKeeper_.Network()->ReceiveMagicPosition(receiveBuffer, size);
}
BOOL SessionLegacyCalls::ReceiveDieExpLarge(const BYTE *receiveBuffer, BOOL encrypted)
{
    return sessionKeeper_.Network()->ReceiveDieExpLarge(receiveBuffer, encrypted);
}
void SessionLegacyCalls::ReceiveSkillStatus(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveSkillStatus(receiveBuffer);
}
void SessionLegacyCalls::ReceiveMagicFinish(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveMagicFinish(receiveBuffer);
}
void SessionLegacyCalls::ReceiveBuffState(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveBuffState(receiveBuffer);
}
void SessionLegacyCalls::RegisterBuff(eBuffState buff, OBJECT *object, const int buffTime)
{
    sessionKeeper_.Network()->RegisterBuff(buff, object, buffTime);
}
void SessionLegacyCalls::UnRegisterBuff(eBuffState buff, OBJECT *object)
{
    sessionKeeper_.Network()->UnRegisterBuff(buff, object);
}
void SessionLegacyCalls::AppearMonster(CHARACTER *character)
{
    sessionKeeper_.Network()->AppearMonster(character);
}
void SessionLegacyCalls::ReceiveSetPointsExtended(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveSetPointsExtended(receiveBuffer);
}
BOOL SessionLegacyCalls::ReceiveStraightAttack(const BYTE *receiveBuffer, int size, BOOL encrypted)
{
    return sessionKeeper_.Network()->ReceiveStraightAttack(receiveBuffer, size, encrypted);
}
void SessionLegacyCalls::InsertBuffPhysicalEffect(eBuffState buff, OBJECT *object)
{
    sessionKeeper_.Network()->InsertBuffPhysicalEffect(buff, object);
}
void SessionLegacyCalls::ClearBuffPhysicalEffect(eBuffState buff, OBJECT *object)
{
    sessionKeeper_.Network()->ClearBuffPhysicalEffect(buff, object);
}
BOOL SessionLegacyCalls::ReceiveDieExp(const BYTE *receiveBuffer, BOOL encrypted)
{
    return sessionKeeper_.Network()->ReceiveDieExp(receiveBuffer, encrypted);
}
BOOL SessionLegacyCalls::ReceiveJoinMapServer(std::span<const BYTE> receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveJoinMapServer(receiveBuffer);
}
BOOL SessionLegacyCalls::ReceiveEquipmentItemExtended(std::span<const BYTE> receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveEquipmentItemExtended(receiveBuffer);
}
void SessionLegacyCalls::ReceiveModifyItemExtended(std::span<const BYTE> receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveModifyItemExtended(receiveBuffer);
}
void SessionLegacyCalls::ReceiveBuyExtended(const std::span<const BYTE> receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveBuyExtended(receiveBuffer);
}
void SessionLegacyCalls::ReceiveTradeYourInventoryExtended(std::span<const BYTE> receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveTradeYourInventoryExtended(receiveBuffer);
}
void SessionLegacyCalls::ReceiveMixExtended(std::span<const BYTE> receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveMixExtended(receiveBuffer);
}
void SessionLegacyCalls::ReceiveRepair(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveRepair(receiveBuffer);
}
void SessionLegacyCalls::ReceiveLevelUp(const BYTE *receiveBuffer, int size)
{
    sessionKeeper_.Network()->ReceiveLevelUp(receiveBuffer, size);
}
void SessionLegacyCalls::ReceiveDurability(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDurability(receiveBuffer);
}
void SessionLegacyCalls::ReceiveStorageGold(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveStorageGold(receiveBuffer);
}
void SessionLegacyCalls::ReceiveStorageExit(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveStorageExit(receiveBuffer);
}
void SessionLegacyCalls::ReceivePartyList(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceivePartyList(receiveBuffer);
}
void SessionLegacyCalls::ReceiveGuildBeginWar(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveGuildBeginWar(receiveBuffer);
}
void SessionLegacyCalls::Receive_Master_LevelUp(const BYTE *receiveBuffer, int size)
{
    sessionKeeper_.Network()->Receive_Master_LevelUp(receiveBuffer, size);
}
void SessionLegacyCalls::Receive_Master_Level_Exp(const BYTE *receiveBuffer, int size)
{
    sessionKeeper_.Network()->Receive_Master_Level_Exp(receiveBuffer, size);
}
void SessionLegacyCalls::Receive_Master_LevelGetSkill(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->Receive_Master_LevelGetSkill(receiveBuffer);
}
void SessionLegacyCalls::ReceiveMixExit(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveMixExit(receiveBuffer);
}
BOOL SessionLegacyCalls::ReceiveTalk(const BYTE *receiveBuffer, BOOL encrypted)
{
    return sessionKeeper_.Network()->ReceiveTalk(receiveBuffer, encrypted);
}
void SessionLegacyCalls::ReceiveSetAttribute(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveSetAttribute(receiveBuffer);
}
void SessionLegacyCalls::ReceiveSetPriceResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveSetPriceResult(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCreatePersonalShop(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCreatePersonalShop(receiveBuffer);
}
void SessionLegacyCalls::ReceiveDestroyPersonalShop(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDestroyPersonalShop(receiveBuffer);
}
void SessionLegacyCalls::ReceivePersonalShopItemList(std::span<const BYTE> receiveBuffer)
{
    sessionKeeper_.Network()->ReceivePersonalShopItemList(receiveBuffer);
}
void SessionLegacyCalls::ReceiveRefreshItemList(std::span<const BYTE> receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveRefreshItemList(receiveBuffer);
}
void SessionLegacyCalls::ReceivePurchaseItem(std::span<const BYTE> receiveBuffer)
{
    sessionKeeper_.Network()->ReceivePurchaseItem(receiveBuffer);
}
void SessionLegacyCalls::ReceiveChangeMapServerResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveChangeMapServerResult(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCheckSumRequest(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCheckSumRequest(receiveBuffer);
}
void SessionLegacyCalls::LogSafeCastSizeMismatch(const char *packetType, std::size_t received,
                                                 std::size_t expected)
{
    sessionKeeper_.Network()->LogSafeCastSizeMismatch(packetType, received, expected);
}

template <typename T>
T *SessionLegacyCalls::safe_cast(const std::span<const BYTE> receiveBuffer, const char *packetType)
{
    return sessionKeeper_.Network()->safe_cast<T>(receiveBuffer, packetType);
}

template <typename T>
T *SessionNetworkUnit::safe_cast(const std::span<const BYTE> receiveBuffer, const char *packetType)
{
    if (receiveBuffer.size() < sizeof(T))
    {
        LogSafeCastSizeMismatch(packetType ? packetType : typeid(T).name(), receiveBuffer.size(),
                                sizeof(T));
        return nullptr;
    }

    return reinterpret_cast<T *>(const_cast<BYTE *>(receiveBuffer.data()));
}

void SessionNetworkUnit::InitGuildWar()
{
...
void SessionNetworkUnit::ReceiveServerList(const BYTE *ReceiveBuffer)
{
    ...
    for (int i = 0; i < g_ServerListManager.GetTotalServer(); i++)
    {
        ...
        g_ConsoleDebug.Write(MCD_RECEIVE, L"0xF4 [ReceiveServerList(%d %d %d)]", i, Data2->Index,
                             Data2->Percent);
    ...
    if (!rUIMng.m_CreditWin->IsShow())
    {
        ...
        rUIMng.ShowWin(rUIMng.m_LoginMainWin.get());
        (void)rUIMng.m_ServerSelWin->TryAutoConnect();
    }
...
void SessionNetworkUnit::ReceiveServerConnect(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPRECEIVE_SERVER_ADDRESS)ReceiveBuffer;
    if (!LegacyUiManager().m_ServerSelWin->AcceptConnectionPort(Data->Port))
    {
        return;
    }
    wchar_t IP[16];
...
void SessionNetworkUnit::ReceiveServerConnectBusy(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPRECEIVE_SERVER_BUSY)ReceiveBuffer;
    SocketClient->ToConnectServer()->SendServerListRequest();
}

void SessionLegacyCalls::ReceiveServerConnectBusy(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveServerConnectBusy(receiveBuffer);
}

bool SessionNetworkUnit::ReceiveJoinServer(const BYTE *ReceiveBuffer)
{
    ...
    if (LogIn != 0)
    {
        ...
        return csMapServer_.SendChangeMapServer();
    }
    else
    {
        CUIMng &rUIMng = LegacyUiManager();
        ...
        case 0x01:
            ...
            if (!Reconnect().IsActive() && !rUIMng.m_LoginWin->TryAutoLogin())
            {
        ...
        if (actual < received)
        {
            rUIMng.HideWin(rUIMng.m_LoginWin.get());
...
void SessionNetworkUnit::ReceiveConfirmPassword(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveConfirmPassword2(const std::span<const BYTE> ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveChangePassword(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveCharacterListExtended(const BYTE *ReceiveBuffer)
{
    InitGuildWar();
...
#ifdef _DEBUG
    g_ConsoleDebug.Write(MCD_RECEIVE, L"[ReceiveList Count %d Max class %d]", Data->CharacterCount,
                         Data->MaxClass);
#else
    g_ErrorReport.Write(L"[ReceiveList Count %d Max class %d]", Data->CharacterCount,
                        Data->MaxClass);
#endif

    CharacterAttribute->IsVaultExtended = Data->IsVaultExtended;
    if (Data->CharacterCount > MAX_CHARACTERS_PER_ACCOUNT)
    {
        return;
    }

    if (sessionKeeper_.ModelPoolObject().StorageBytes() == 0)
    {
        const std::size_t packetSize = sizeof(PHEADER_DEFAULT_CHARACTER_LIST) +
                                       static_cast<std::size_t>(Data->CharacterCount) *
                                           sizeof(PRECEIVE_CHARACTER_LIST_EXTENDED);
        pendingCharacterList_.assign(ReceiveBuffer, ReceiveBuffer + packetSize);
        CurrentProtocolState = RECEIVE_CHARACTERS_LIST;
        return;
    }

    MaterializeCharacterList(ReceiveBuffer);
}

void SessionNetworkUnit::MaterializeCharacterList(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPHEADER_DEFAULT_CHARACTER_LIST)ReceiveBuffer;
...
bool SessionNetworkUnit::MaterializePendingCharacterListOnOwner()
{
    if (pendingCharacterList_.empty())
    {
        return true;
    }
    if (sessionKeeper_.ModelPoolObject().StorageBytes() == 0)
    ...
    MaterializeCharacterList(pendingCharacterList_.data());
    pendingCharacterList_.clear();
    return true;
}

void SessionNetworkUnit::ReceiveCharacterCard_New(const BYTE *ReceiveBuffer)
{
    ...
    g_ConsoleDebug.Write(MCD_NORMAL, L"[BOTH MESSAGE] CharacterCard Recv %d = %d %d %d",
                         Data->CharacterCard, g_CharCardEnable.bCharacterEnable[0],
                         g_CharCardEnable.bCharacterEnable[1],
                         g_CharCardEnable.bCharacterEnable[2]);
}

void SessionNetworkUnit::ReceiveCreateCharacter(const BYTE *ReceiveBuffer)
{
    ...
    if (Data->Result == 1)
    {
        ...
        LegacyUiManager().PopUpMsgWin(RECEIVE_CREATE_CHARACTER_FAIL);
    else if (Data->Result == 2)
        LegacyUiManager().PopUpMsgWin(RECEIVE_CREATE_CHARACTER_FAIL2);

    g_ConsoleDebug.Write(MCD_RECEIVE, L"0x01 [ReceiveCreateCharacter]");
}

void SessionNetworkUnit::ReceiveDeleteCharacter(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPHEADER_DEFAULT_SUBCODE)ReceiveBuffer;
    switch (Data->Value)
    {
    case 1:
        ...
        LegacyUiManager().PopUpMsgWin(MESSAGE_DELETE_CHARACTER_SUCCESS);
        break;
    case 0:
        LegacyUiManager().PopUpMsgWin(MESSAGE_DELETE_CHARACTER_GUILDWARNING);
        break;
    case 3:
        LegacyUiManager().PopUpMsgWin(MESSAGE_DELETE_CHARACTER_ITEM_BLOCK);
        break;
    case 2:
    default:
        LegacyUiManager().PopUpMsgWin(MESSAGE_STORAGE_RESIDENTWRONG);
        break;
    }
}
...
void SessionNetworkUnit::ResetCombatTargetState()
{
    AttackPlayer = -1;
    CurrentSkill = 0;
    ...
    muHelper_.Stop();
    muHelper_.DeleteAllTargets();
}

void SessionNetworkUnit::ResetCharacterSessionState()
{
...
void SessionNetworkUnit::InitGame()
{
    ResetCharacterSessionState();
...
BOOL SessionNetworkUnit::ReceiveLogOut(const BYTE *ReceiveBuffer, BOOL bEncrypted)
{
    LogOut = false;
    auto Data = (LPPHEADER_DEFAULT_SUBCODE)ReceiveBuffer;
    switch (Data->Value)
    ...
    case 1:
        ...
        sessionKeeper_.WorldUnit()->Leave(World::LeaveReason::Logout);
        CryWolfMVPInit();
...
void SessionNetworkUnit::ResetForServerSelection()
{
    sessionKeeper_.WorldUnit()->Leave(World::LeaveReason::Logout);
    SceneFlag = LOG_IN_SCENE;
    InitLogIn = false;
    ...
    CurrentProtocolState = REQUEST_JOIN_SERVER;
    LogIn = 0;
    gameplay_.loginProtocolPrepared_ = false;
    sessionKeeper_.Config().autoLoginPort = 0;
    const ApplicationKeeper &app = sessionKeeper_.ApplicationKeeperRef();
    szServerIpAddress = app.BootstrapServerIp().c_str();
    g_ServerPort = app.BootstrapServerPort();
    csMapServer_.Init();
    InitGame();
}

void SessionNetworkUnit::ResetClientToLoginScene()
{
...
void SessionLegacyCalls::ResetClientToLoginScene()
{
    sessionKeeper_.Network()->ResetClientToLoginScene();
}

void SessionNetworkUnit::LogSafeCastSizeMismatch(const char *packet_type, std::size_t received,
                                                 std::size_t expected)
{
    ...
    g_ConsoleDebug.Write(
        MCD_ERROR, L"safe_cast<%.64hs>: received %u bytes, expected at least %u -- packet dropped",
        packet_type ? packet_type : "?", static_cast<unsigned>(received),
        static_cast<unsigned>(expected));
}

std::optional<std::uint32_t> SessionNetworkUnit::CurrentMapRoute() noexcept
{
    if (SocketClient == nullptr)
        return std::nullopt;
    const int selectedServer = (std::max)(0, g_ServerListManager.GetSelectServerIndex());
    const std::uint32_t server = csMapServer_.GetServerCode() != 0
                                     ? csMapServer_.GetServerCode()
                                     : static_cast<std::uint32_t>(selectedServer);
    return sessionKeeper_.ApplicationKeeperRef().SharedCharacters().ResolveWorldRoute(
        SocketClient->Host(), SocketClient->Port(), server);
}

BOOL SessionNetworkUnit::RejectWorldTransfer()
{
    sessionKeeper_.WorldUnit()->FinishLoad(false);
    if (SocketClient != nullptr)
        SocketClient->Close();
    g_bGameServerConnected = false;
    const CHARACTER *controlled = CharactersClient.Hero;
    ClearCharacters(controlled != nullptr ? controlled->Key : -1);
    return FALSE;
}

void SessionNetworkUnit::StopInactiveWorldEventAudio()
{
...
BOOL SessionNetworkUnit::ReceiveJoinMapServer(std::span<const BYTE> ReceiveBuffer)
{
    ...
    const auto route = CurrentMapRoute();
    if (!route)
        return RejectWorldTransfer();

    auto transition = sessionKeeper_.WorldUnit()->PrepareTransition(WorldTransition::Reason::Join,
                                                                    Data->Map, route);
    if (!transition)
        return RejectWorldTransfer();
    ...
    MouseLButton = false;
    ClearCharacters(-1);
    HeroIndex = CharactersClient.BindControlled(HeroKey);
    if (!CharactersClient.IsValidIndex(HeroIndex))
        return RejectWorldTransfer();
    ...
    CreateEventMatch(gMapManager.ContextMap());

    CreateCharacterPointer(c, MODEL_PLAYER, Data->PositionX, Data->PositionY,
                           ((float)Data->Angle - 1.f) * 45.f);
    c->Key = HeroKey;

    g_ConsoleDebug.Write(MCD_RECEIVE, L"0x03 [ReceiveJoinMapServer] Key: %d Map: %d X: %d Y:%d",
                         c->Key, gMapManager.ContextMap(), Data->PositionX, Data->PositionY);
    OBJECT *o = &c->Object;
    ...
    {
        wchar_t Text[256];
        mu_swprintf(Text, I18N::Game::WelcomeTo, gMapManager.GetMapName(gMapManager.ContextMap()));
    ...
    if (gMapManager.ContextMap() < WD_69EMPIREGUARDIAN1 ||
        WD_72EMPIREGUARDIAN4 < gMapManager.ContextMap())
    {
    ...
    return transition->Complete() ? TRUE : RejectWorldTransfer();
}

void SessionNetworkUnit::ReceiveRevival(const BYTE *ReceiveBuffer)
{
    ...
    auto transition = sessionKeeper_.WorldUnit()->PrepareTransition(
        WorldTransition::Reason::Revival, Data->Map, route);
    if (!transition)
    {
        RejectWorldTransfer();
        return;
    ...
    if (gCharacterManager.IsMasterExperienceActive(CharacterAttribute->Class,
                                                   CharacterAttribute->Level) == true)
    {
        ...
        const bool swappedConvertible =
            swappedRevivalExperience <= static_cast<uint64_t>(kInt64Max);
        const auto rawCandidate =
            rawConvertible ? static_cast<int64_t>(rawRevivalExperience) : currentExperience;
        const auto swappedCandidate =
            swappedConvertible ? static_cast<int64_t>(swappedRevivalExperience) : currentExperience;

        const bool isRawPlausible =
            rawConvertible && (rawCandidate >= lowerBound && rawCandidate <= upperBound);
        const bool isSwappedPlausible = swappedConvertible && (swappedCandidate >= lowerBound &&
                                                               swappedCandidate <= upperBound);
        ...
        Master_Level_Data.lMasterLevel_Experince =
            ClampToInterval(selectedExperience, lowerBound, upperBound);
    }
    else
    {
        ...
        const bool isRawPlausible =
            rawRevivalExperience >= lowerBound && rawRevivalExperience <= upperBound;
    ...
    if (gMapManager.ContextMap() >= WD_45CURSEDTEMPLE_LV1 &&
        gMapManager.ContextMap() <= WD_45CURSEDTEMPLE_LV6)
    {
        if (!(Data->Map >= WD_45CURSEDTEMPLE_LV1 && Data->Map <= WD_45CURSEDTEMPLE_LV6))
        {
            cursedTemple_.ResetCursedTemple();
            g_pNewUISystem->Hide(SEASON3B::INTERFACE_CURSEDTEMPLE_GAMESYSTEM);
        }
    }

    const int OldWorld = gMapManager.ContextMap();
    if (!transition->Activate())
    {
        RejectWorldTransfer();
        return;
    }
    if (transition->ReloadsResources())
    {

        if ((gMapManager.InChaosCastle(OldWorld) == true && OldWorld != gMapManager.ContextMap()) ||
        ...
        if (gMapManager.ContextMap() == -1 || c->Helper.Type != MODEL_HORN_OF_DINORANT ||
            c->SafeZone)
        {
...
void SessionNetworkUnit::ReceiveMagicList(const BYTE *ReceiveBuffer)
{
    ...
    else
    {
        ...
        if (gCharacterManager.GetBaseClass(Hero->Class) == CLASS_DARK_LORD)
        {
            ...
            {
                CharacterAttribute->Skill[AT_PET_COMMAND_DEFAULT + i] =
                    (ActionSkillType)(AT_PET_COMMAND_DEFAULT + i);
            }
        }
    }
    ...
    if (CharacterAttribute->SkillNumber == 1)
        Hero->CurrentSkill = 0;
    if (Hero->CurrentSkill >= 0 &&
        CharacterAttribute->Skill[Hero->CurrentSkill] == AT_SKILL_UNDEFINED)
        Hero->CurrentSkill = 0;
...
void SessionNetworkUnit::Receive_Master_SetSkillList(PMSG_MASTER_SKILL_LIST_SEND *lpMsg)
{
    auto interface = g_pNewUISystem->GetUI_NewMasterLevelInterface();
    ...
    {
        auto lpInfo =
            (PMSG_MASTER_SKILL_LIST *)(((BYTE *)lpMsg) + sizeof(PMSG_MASTER_SKILL_LIST_SEND) +
                                       (sizeof(PMSG_MASTER_SKILL_LIST) * n));
        interface->SetMasterSkillTreeInfo(lpInfo->SkillIndex, lpInfo->SkillLevel, lpInfo->MainValue,
                                          lpInfo->NextValue);
    }
...
void SessionNetworkUnit::ReceiveMuHelperConfigurationData(std::span<const BYTE> ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveMuHelperStatusUpdate(std::span<const BYTE> ReceiveBuffer)
{
    ...
    else
    {
        ...
        {
            muHelper_.AddCost(pMuHelperStatus->Money);
            int iTotalCost = muHelper_.GetTotalCost();

            wchar_t Text[100];
            mu_swprintf(Text, I18N::Game::DZenSHaveBeenSpentInImplementingOfficialMUHelper,
                        iTotalCost);
            g_pSystemLogBox->AddText(Text, SEASON3B::TYPE_SYSTEM_MESSAGE);
        }
    }
...
void SessionNetworkUnit::ReceiveDeleteInventory(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPHEADER_DEFAULT_SUBCODE)ReceiveBuffer;
    if (Data->SubCode != 0xff)
    {
        ...
        {
            sessionKeeper_.Gameplay()->UnequipItem(itemindex);
        }
        else if (IsMainInventorySlot(itemindex))
        {
            sessionKeeper_.GameData()->DeleteInventoryItem(itemindex);
        }
        ...
        {
            sessionKeeper_.GameData()->Inventory(InventoryRole::MyShop).RemoveItemAt(itemindex);
        }
    }
    ...
    g_ConsoleDebug.Write(MCD_RECEIVE, L"0x28 [ReceiveDeleteInventory(%d %d)]", Data->SubCode,
                         Data->Value);
}

int SessionNetworkUnit::CalcItemLength(std::span<const BYTE> ReceiveBuffer)
{
    return static_cast<int>(GameLogic::Items::ItemPacketLength(ReceiveBuffer));
}

BOOL SessionNetworkUnit::ReceiveInventoryExtended(std::span<const BYTE> ReceiveBuffer)
{
    for (auto &i : CharacterMachine->Equipment)
    {
        i.Type = -1;
    ...
    sessionKeeper_.Gameplay()->UnequipAllItems();
    sessionKeeper_.GameData()->ClearPlayerInventory();
    ...
    for (int i = 0; i < Data->Value; i++)
    {
        ...
        if (itemindex >= 0 && itemindex < MAX_EQUIPMENT_INDEX)
        {
            sessionKeeper_.Gameplay()->EquipItem(itemindex, itemData);
        }
        else if (IsMainInventorySlot(itemindex))
        {
            sessionKeeper_.GameData()->InsertInventoryItem(itemindex, itemData);
        }
        else if (IsInventoryExtensionSlot(itemindex))
        {
            sessionKeeper_.GameData()->InsertInventoryItem(itemindex, itemData);
        ...
        {
            sessionKeeper_.GameData()
                ->Inventory(InventoryRole::MyShop)
                .AddItem(itemindex, itemData);
        }
...
void SessionNetworkUnit::ReceiveTradeInventoryExtended(std::span<const BYTE> ReceiveBuffer)
{
    auto Data = safe_cast<PHEADER_DEFAULT_SUBCODE_WORD>(ReceiveBuffer);
    if (Data == nullptr)
    {
    ...
    if (Data->SubCode == 3)
    {
        ...
        sessionKeeper_.GameData()->Inventory(InventoryRole::Crafting).RemoveAllItems();
    }
    ...
    else
    {
        ...
        if (sessionKeeper_.InventoryStorage().npcInventory == InventoryRole::NpcShop)
        {
            sessionKeeper_.GameData()->Inventory(InventoryRole::NpcShop).RemoveAllItems();
        }
    }
    ...
    for (int i = 0; i < Data->Value; i++)
    {
        ...
        {
            sessionKeeper_.GameData()
                ->Inventory(InventoryRole::Crafting)
                .AddItem(itemindex, itemData);
        }
        else
        {
            ...
            {
                sessionKeeper_.GameData()
                    ->Inventory(InventoryRole::NpcShop)
                    .AddItem(itemindex, itemData);
            }
            else if (sessionKeeper_.InventoryStorage().npcInventory == InventoryRole::Vault)
            {
                if (itemindex < MAX_SHOP_INVENTORY)
                {
                    sessionKeeper_.GameData()
                        ->Inventory(InventoryRole::Vault)
                        .AddItem(itemindex, itemData);
                }
                else
                {
                    sessionKeeper_.GameData()
                        ->Inventory(InventoryRole::VaultExtension)
                        .AddItem(itemindex, itemData);
                }
            }
        }
...
void SessionNetworkUnit::ReceiveChat(const BYTE *ReceiveBuffer)
{
    if (SceneFlag == LOG_IN_SCENE)
    {
        // Connect servers may repeat hello after each endpoint response.
        // Keep the outstanding port lookup instead of restarting server selection.
        if (LegacyUiManager().m_ServerSelWin->IsAutoConnectPending())
        {
            return;
        }
    ...
    else
    {
        ...
        if (g_pChatInputBox->IsBlockedUser(ID))
            return;
...
void SessionNetworkUnit::ReceiveChatWhisper(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPCHATING)ReceiveBuffer;
...
void SessionNetworkUnit::ReceiveChatWhisperResult(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveChatKey(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveNotice(const BYTE *ReceiveBuffer)
{
    ...
    else if (Data->Result >= 10 && Data->Result <= 15)
    {
        ...
        {
            g_pSlideHelpMgr->AddSlide(Data->Count, Data->Delay, Text, Data->Result - 10,
                                      Data->Speed / 10.0f, Data->Color);
        }
    }
...
void SessionNetworkUnit::ReceiveMoveCharacter(std::span<const BYTE> ReceiveBuffer)
{
    ...
    g_ConsoleDebug.Write(MCD_RECEIVE, L"ID : %ls | sX : %d | sY : %d | tX : %d | tY : %d", c->ID,
    ...
    if (o->Type == MODEL_CRUST)
    {
        c->PositionX = Data->TargetX;
        c->PositionY = Data->TargetY;
        o->SetAngleZ(CreateAngle(o->Position[0], o->Position[1], c->PositionX * 100.f,
                                 c->PositionY * 100.f));
        SetPlayerWalk(c);
        c->JumpTime = 1;
    ...
    if (c->Appear == 0)
    {
        ...
        if (gMapManager.ContextMap() >= WD_65DOPPLEGANGER1 &&
            gMapManager.ContextMap() <= WD_68DOPPLEGANGER4 && Key != HeroKey)
        {
            iDefaultWall = TW_NOMOVE;
        }

        if (PathFinding2(c->PositionX, c->PositionY, c->TargetX, c->TargetY, &c->Path, 0.0f,
                         iDefaultWall))
        {
            c->Movement = true;
        }
...
void SessionNetworkUnit::ReceiveMovePosition(const BYTE *ReceiveBuffer)
{
...
BOOL SessionNetworkUnit::ReceiveTeleport(const BYTE *ReceiveBuffer, BOOL bEncrypted)
{
    auto Data = (LPPRECEIVE_TELEPORT_POSITION)ReceiveBuffer;
    const auto route = Data->Flag == 0 ? std::nullopt : CurrentMapRoute();
    if (Data->Flag != 0 && !route)
        return RejectWorldTransfer();
    auto transition = Data->Flag == 0 ? std::optional<WorldTransition>{}
                                      : sessionKeeper_.WorldUnit()->PrepareTransition(
                                            WorldTransition::Reason::Teleport, Data->Map, route);
    if (Data->Flag != 0 && !transition)
        return RejectWorldTransfer();
    sessionKeeper_.Gameplay()->RestorePickedItem();
    ...
    if (gMapManager.ContextMap() == -1 || Hero->Helper.Type != MODEL_HORN_OF_DINORANT ||
        Hero->SafeZone)
    {
        o->Position[2] = RequestTerrainHeight(o->Position[0], o->Position[1]);
    }
    ...
    o->SetAngleZ(((float)(Data->Angle) - 1.f) * 45.f);
    if (Data->Flag == 0)
    {
        CreateTeleportEnd(o);
    }
    else
    {
        ...
        if (transition->ReloadsResources())
        {
            ...
            if ((gMapManager.InChaosCastle(OldWorld) == true &&
                 OldWorld != gMapManager.ContextMap()) ||
                gMapManager.InChaosCastle() == true)
            {
            ...
            {
                if (gMapManager.ContextMap() == WD_8TARKAN ||
                    gMapManager.ContextMap() == WD_10HEAVEN)
                    o->Position[2] = RequestTerrainHeight(o->Position[0], o->Position[1]) + 90.f;
                else
                    o->Position[2] = RequestTerrainHeight(o->Position[0], o->Position[1]) + 30.f;
            }
            ...
            {
                wchar_t Text[256];
                mu_swprintf(Text, I18N::Game::WelcomeTo,
                            gMapManager.GetMapName(gMapManager.ContextMap()));
...
void SessionNetworkUnit::ReceiveEquipment(std::span<const BYTE> ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveChangePlayer(std::span<const BYTE> ReceiveBuffer)
{
    ...
    g_ConsoleDebug.Write(
        MCD_RECEIVE,
        L"0x25 ReceiveChangePlayer Key(0x%04X) Item Slot(0x%02X) Group(0x%02X) Number(0x%04X) Level(0x%02X)",
    ...
    case 8:
        if (Data->ItemGroup == 0xFF)
        {
            c->Helper.Type = -1;
            DeleteMount(o);
            g_petProcess.DeletePet(c, c->Helper.Type, true);
        }
        else
        {
            ...
            case ITEM_SPIRIT_OF_GUARDIAN: {
                {
                    g_petProcess.CreatePet(Type, c->Helper.Type, o->Position, c);
                }
            }
            break;
...
void SessionNetworkUnit::ReceiveCreatePlayerViewportExtended(std::span<const BYTE> ReceiveBuffer)
{
    ...
    c->Object.SetAngleZ(((float)(Data->RotationAndHeroState >> 4) - 1.f) * 45.f);
    o->Kind = KIND_PLAYER;
    o->Type = MODEL_PLAYER;
    c->PositionX = Data->PositionX;
    c->PositionY = Data->PositionY;
    ...
    g_ConsoleDebug.Write(MCD_RECEIVE, L"(RCPV)ID : %ls | sX : %d | sY : %d | tX : %d | tY : %d",
                         c->ID, c->PositionX, c->PositionY, c->TargetX, c->TargetY);
...
void SessionNetworkUnit::ReceiveCreateTransformViewport(std::span<const BYTE> ReceiveBuffer)
{
    ...
    for (int i = 0; i < Data->Value; i++)
    {
        ...
        if (FindText(characterName, L"webzen") == false)
        {
            ...
            for (int j = 0; j < Data2->s_BuffCount; ++j)
            {
                ...
                SettingBattleFormation(c, static_cast<eBuffState>(Data2->s_BuffEffectState[j]));
            ...
            c->Object.SetAngleZ(((float)(Data2->Path >> 4) - 1.f) * 45.f);
            ...
            else if (PathFinding2((c->PositionX), (c->PositionY), Data2->TargetX, Data2->TargetY,
                                  &c->Path))
            {
                c->Movement = true;
            }
        ...
        Offset += (sizeof(PCREATE_TRANSFORM_EXTENDED) -
                   (sizeof(BYTE) * (MAX_BUFF_SLOT_INDEX - Data2->s_BuffCount)));
    }

    g_ConsoleDebug.Write(MCD_RECEIVE, L"0x45 [ReceiveCreateTransformViewport(%d)]", Data->Value);
}

void SessionNetworkUnit::AppearMonster(CHARACTER *c)
{
    OBJECT *o = &c->Object;
    o->EnableBoneMatrix = false;
...
void SessionNetworkUnit::ReceiveCreateMonsterViewport(const BYTE *ReceiveBuffer)
{
    ...
    for (int i = 0; i < Data->Value; i++)
    {
        ...
        g_ConsoleDebug.Write(
            MCD_RECEIVE, L"0x13 [ReceiveCreateMonsterViewport(Type : %d | Key : %d)]", Type, Key);
        ...
        c->Object.SetAngleZ(((float)(Data2->Path >> 4) - 1.f) * fAngle);

        if (o->Type >= MODEL_LITTLESANTA + 1 && o->Type <= MODEL_LITTLESANTA + 4)
        {
            c->Object.SetAngleZ(90.f);
        }
        ...
        if (gMapManager.InBattleCastle() && c->MonsterIndex == MONSTER_CASTLE_GATE1)
        {
            if (g_isCharacterBuff((&c->Object), eBuff_CastleGateIsOpen))
            {
                ...
                SetCastleGate_Attribute(Data2->PositionX, Data2->PositionY, 0);
            }
            else
            {
                SetAction(&c->Object, 0);
                SetCastleGate_Attribute(Data2->PositionX, Data2->PositionY, 1);
            }
        }
        if (CreateFlag)
        {
        ...
        else if (gMapManager.ContextMap() == WD_39KANTURU_3RD &&
                 o->Type == MODEL_DARK_SKULL_SOLDIER_5 && TeleportFlag)
        {
            vec3_t Light;
        ...
        Offset +=
            (sizeof(PCREATE_MONSTER) - (sizeof(BYTE) * (MAX_BUFF_SLOT_INDEX - Data2->s_BuffCount)));
    }
}

void SessionNetworkUnit::ReceiveCreateSummonViewport(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPWHEADER_DEFAULT_WORD)ReceiveBuffer;
    int Offset = sizeof(PWHEADER_DEFAULT_WORD);
    for (int i = 0; i < Data->Value; i++)
    {
        ...
        {
            c = CreateHellGate(Data2->ID, Key, Type, Data2->PositionX, Data2->PositionY,
                               CreateFlag);
        }
        else
        {
        ...
        Offset +=
            (sizeof(PCREATE_SUMMON) - (sizeof(BYTE) * (MAX_BUFF_SLOT_INDEX - Data2->s_BuffCount)));
    }

    g_ConsoleDebug.Write(MCD_RECEIVE, L"0x1F [ReceiveCreateSummonViewport(%d)]", Data->Value);
}

void SessionNetworkUnit::ReceiveDeleteCharacterViewport(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
    ...
    for (int i = 0; i < Data->Value; i++)
    {
        ...
        iIndex = CharactersClient.FindIndexByKey(Key);
        if (!CharactersClient.IsValidIndex(iIndex))
        {
        ...
        CHARACTER *pCha = &CharactersClient[iIndex];
        CHARACTER *shopPlayer = FindCharacterTagShopTitle(Key);
        if (shopPlayer != nullptr)
            RemoveShopTitle(shopPlayer);

        const bool retiring =
            !CharactersClient.IsShared(iIndex) || CharactersClient.ObserverCount(iIndex) == 1;
        if (retiring)
        {
            const int buffSize = g_CharacterBuffSize((&pCha->Object));
...
void SessionNetworkUnit::ReceiveDamage(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveAttackDamageCastle(
    CHARACTER *c, OBJECT *o, const bool success, const int key, const int damage,
    const int shieldDamage, const int damageType, const bool bRepeatedly, const bool bEndRepeatedly,
    const bool bDoubleEnable, const bool bComboEnable)
{
    vec3_t Light;
    ...
    else
    {
        ...
        {
            if (c->MonsterIndex != MONSTER_ILLUSION_OF_KUNDUN_7 && WorldRandom() % 2 == 0)
            {
                SetPlayerShock(c, damage);
            }
        }
...
void SessionNetworkUnit::ReceiveAttackDamage(CHARACTER *c, OBJECT *o, const bool success,
                                             const int key, const int damage,
                                             const int shieldDamage, const int damageType,
                                             const bool bRepeatedly, const bool bEndRepeatedly,
                                             const bool bDoubleEnable, const bool bComboEnable)
{
    vec3_t Light;
    if (success)
...
void SessionNetworkUnit::ReceiveAttackDamageExtended(const BYTE *ReceiveBuffer)
{
    ...
    g_ConsoleDebug.Write(MCD_RECEIVE, L"0x15 [ReceiveAttackDamageExtended(%d %d)]", AttackPlayer,
                         Damage);
    if (IsMonster(c))
    {
        muHelper_.AddTarget(Key, true);
    }
    if (Key == HeroKey && (Damage > 0 || ShieldDamage > 0) &&
        CharactersClient.IsValidIndex(AttackPlayer))
    {
        CHARACTER *attacker = &CharactersClient[AttackPlayer];
        if (IsPlayer(attacker) && attacker != Hero)
        {
            muHelper_.AddTarget(attacker->Key, true);
        }
    }
    ...
    {
        ReceiveAttackDamageCastle(c, o, Success, Key, Damage, ShieldDamage, DamageType, bRepeatedly,
                                  bEndRepeatedly, bDoubleEnable, bComboEnable);
    }
    else
    {
        ReceiveAttackDamage(c, o, Success, Key, Damage, ShieldDamage, DamageType, bRepeatedly,
                            bEndRepeatedly, bDoubleEnable, bComboEnable);
    }
}

void SessionNetworkUnit::ReceiveAction(const BYTE *ReceiveBuffer, int Size)
{
    ...
    c->Object.SetAngleZ(((float)(Data->Angle) - 1.f) * 45.f);
    c->Movement = false;
    ...
    case AT_ATTACK2:
        SetPlayerAttack(c);
        c->AttackTime = 1;
        c->Object.SetAnimationFrame(0);

        SetCharacterTarget(*c, HeroIndex);
        if (IsMonster(c) || (IsPlayer(c) && c != Hero && iTargetKey == HeroKey))
        {
            muHelper_.AddTarget(Key, true);
        }
    ...
    case AT_SANTA1_3: {
        ...
        gameplay_.XmasEvent().CreateXmasEventEffect(c, o, i);
        o->m_iAnimation = 0;
...
void SessionNetworkUnit::ReceiveSkillStatus(const BYTE *ReceiveBuffer)
{
    ...
    if (Data->State == 1) // add
    {
        ...
        if (g_isCharacterBuff(o, bufftype))
        {
            if ((o->Type >= MODEL_CRYWOLF_ALTAR1 && o->Type <= MODEL_CRYWOLF_ALTAR5) ||
                MODEL_SMITH || MODEL_NPC_SERBIS || MODEL_MERCHANT_MAN || MODEL_STORAGE ||
                MODEL_NPC_BREEDER)
            {
        ...
        else
        {
            ...
            if (bufftype == eBuff_CastleRegimentDefense ||
                bufftype == eBuff_CastleRegimentAttack1 ||
                bufftype == eBuff_CastleRegimentAttack2 || bufftype == eBuff_CastleRegimentAttack3)
            {
                SettingBattleFormation(c, bufftype);
            }
            ...
            g_ConsoleDebug.Write(MCD_RECEIVE, L"RegisterBuff ID : %ls, Buff : %d", c->ID,
    ...
    else // clear
    {
        ...
        if (bufftype == eBuff_CastleRegimentDefense || bufftype == eBuff_CastleRegimentAttack1 ||
            bufftype == eBuff_CastleRegimentAttack2 || bufftype == eBuff_CastleRegimentAttack3)
        {
            DeleteBattleFormation(c, bufftype);
...
void SessionNetworkUnit::ReceiveMagicFinish(const BYTE *ReceiveBuffer)
{
    ...
    switch (
        (ActionSkillType)Data
            ->Value) // todo: is this correct? Data->Value is a byte, but ActionSkillType is an int
    {
...
BOOL SessionNetworkUnit::ReceiveMonsterSkill(const BYTE *ReceiveBuffer, int Size, BOOL bEncrypted)
{
    ...
    SetCharacterTarget(*sc, TargetIndex);

    if (SkillNumber > ATMON_SKILL_BIGIN && SkillNumber < ATMON_SKILL_END)
    {
        sc->Object.SetAngleZ(CreateAngle2D(so->Position, to->Position));
        SetPlayerAttack(sc);
        so->SetAnimationFrame(0);
        sc->AttackTime = 1;
    }

    g_ConsoleDebug.Write(MCD_RECEIVE,
                         L"0x69 [ReceiveMonsterSkill(Skill : %d | SKey : %d |TKey : %d)]",
                         SkillNumber, SourceKey, TargetKey);

    return (TRUE);
}

BOOL SessionNetworkUnit::ReceiveMagic(const BYTE *ReceiveBuffer, int Size, BOOL bEncrypted)
{
    ...
    if (MagicNumber == AT_SKILL_ATTACK || MagicNumber == AT_SKILL_DEFENSE ||
        MagicNumber == AT_SKILL_ATTACK_STR || MagicNumber == AT_SKILL_DEFENSE_STR ||
        MagicNumber == AT_SKILL_ATTACK_MASTERY || MagicNumber == AT_SKILL_DEFENSE_MASTERY)
    {
        if (Success == false)
        {
            if (SourceKey == HeroKey)
            {
                g_pSystemLogBox->AddText(I18N::Game::StongerEffectHasTakenPlace,
    ...
    if (TargetKey == HeroKey && IsPlayer(sc) && sc != Hero &&
        !IsCorrectSkillType_FrendlySkill(MagicNumber) &&
        !IsCorrectSkillType_Buff(MagicNumber))
    {
        muHelper_.AddTarget(SourceKey, true);
    }

    if (MagicNumber != AT_SKILL_COMBO)
    {
        if (sc != Hero && MagicNumber != AT_SKILL_TELEPORT &&
            MagicNumber != AT_SKILL_TELEPORT_ALLY && to->Visible)
            so->SetAngleZ(CreateAngle2D(so->Position, to->Position));
        SetCharacterTarget(*sc, TargetIndex);
    ...
    case AT_SKILL_TELEPORT:
        if (gMapManager.ContextMap() == WD_39KANTURU_3RD && so->Type == MODEL_DARK_SKULL_SOLDIER_5)
        {
            vec3_t Light;
            Vector(1.0f, 1.0f, 1.0f, Light);
            so->AlphaTarget = 0.f;
    ...
    case AT_SKILL_FORCE_WAVE_STR:
        if ((sc->Helper.Type >= MODEL_HORN_OF_UNIRIA && sc->Helper.Type <= MODEL_DARK_HORSE_ITEM) &&
            !sc->SafeZone)
        {
    ...
    case AT_SKILL_FALLING_SLASH_STR:
        SetAction(so, static_cast<int>(PLAYER_ATTACK_SKILL_SWORD1) + AT_SKILL_FALLING_SLASH -
    ...
    case AT_SKILL_LUNGE_STR:
        SetAction(so, static_cast<int>(PLAYER_ATTACK_SKILL_SWORD1) + AT_SKILL_LUNGE -
                          AT_SKILL_FALLING_SLASH);
        sc->AttackTime = 1;
    ...
    case AT_SKILL_UPPERCUT:
        SetAction(so, static_cast<int>(PLAYER_ATTACK_SKILL_SWORD1) + AT_SKILL_UPPERCUT -
                          AT_SKILL_FALLING_SLASH);
        sc->AttackTime = 1;
    ...
    case AT_SKILL_CYCLONE_STR_MG:
        SetAction(so, static_cast<int>(PLAYER_ATTACK_SKILL_SWORD1) + AT_SKILL_CYCLONE -
                          AT_SKILL_FALLING_SLASH);
        sc->AttackTime = 1;
    ...
    case AT_SKILL_SLASH_STR:
        ...
        {
            SetAction(so, static_cast<int>(PLAYER_ATTACK_SKILL_SWORD1) + AT_SKILL_SLASH -
                              AT_SKILL_FALLING_SLASH);
        }
        else
        {
    ...
    case AT_SKILL_FIRE_SCREAM_STR: {
        ...
        else if ((sc->Helper.Type >= MODEL_HORN_OF_UNIRIA) &&
                 (sc->Helper.Type <= MODEL_DARK_HORSE_ITEM))
        {
            SetAction(so, PLAYER_ATTACK_RIDE_STRIKE);
        }
        else
    ...
    case AT_SKILL_CHAOTIC_DISEIER: {
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            CreateJoint(BITMAP_2LINE_GHOST, Position, o->Position, o->Angle, 0, o, 20.f, o->PKKey,
                        0, o->m_bySkillSerialNum);
        }
    ...
    case AT_SKILL_KILLING_BLOW_MASTERY: {
        g_CMonkSystem.SetRageSkillAni(MagicNumber, *sc);
    ...
    case AT_SKILL_DEF_UP_OURFORCES_MASTERY: {
        ...
        if (MagicNumber == AT_SKILL_HP_UP_OURFORCES || MagicNumber == AT_SKILL_HP_UP_OURFORCES_STR)
        {
            ...
            CreateEffect(BITMAP_LIGHT_RED, _pObj->Position, _pObj->Angle, _pObj->Light, 0, _pObj,
                         -1, 0, 0, 0, 1.5f);
        }
        else if (MagicNumber == AT_SKILL_DEF_UP_OURFORCES ||
                 MagicNumber == AT_SKILL_DEF_UP_OURFORCES_STR ||
                 MagicNumber == AT_SKILL_DEF_UP_OURFORCES_MASTERY)
        {
            DeleteEffect(BITMAP_LIGHT_RED, _pObj, 2);
            CreateEffect(BITMAP_LIGHT_RED, _pObj->Position, _pObj->Angle, _pObj->Light, 2, _pObj,
                         -1, 0, 0, 0, 1.5f);

            vec3_t vLight;
            Vector(0.7f, 0.7f, 1.0f, vLight);
            DeleteEffect(MODEL_WINDFOCE, _pObj, 1);
            CreateEffect(MODEL_WINDFOCE, _pObj->Position, _pObj->Angle, vLight, 1, _pObj, -1, 0, 0,
                         0, 1.0f);
        }
        else if (MagicNumber == AT_SKILL_ATT_UP_OURFORCES)
        {
            DeleteEffect(BITMAP_LIGHT_RED, _pObj, 1);
            CreateEffect(BITMAP_LIGHT_RED, _pObj->Position, _pObj->Angle, _pObj->Light, 1, _pObj,
                         -1, 0, 0, 0, 1.5f);
        }
    ...
    case AT_SKILL_DRAGON_ROAR_STR: {
        ...
        {
            CreateEffect(MODEL_VOLCANO_OF_MONK, to->Position, to->Angle, vLight,
                         g_CMonkSystem.GetLowerEffCnt(), to, -1, 0, 0, 0, 0.8f);
        }

        CreateEffect(MODEL_TARGETMON_EFFECT, to->Position, to->Angle, vLight, 0, to, -1, 0, 0, 0,
                     1.0f);
    ...
    case AT_SKILL_DARKSIDE_STR: {
        if (sc != Hero)
            g_CMonkSystem.SetDarksideCnt(*sc, *tc);
    }
    break;
    }
...
BOOL SessionNetworkUnit::ReceiveMagicContinue(const BYTE *ReceiveBuffer, int Size, BOOL bEncrypted)
{
    ...
    // Ranged and area skills use this packet instead of ReceiveMagic. Keep
    // their caster available for the following damage packet so Self Defense
    // can identify the player who hit Hero (for example, Triple Shot).
    if (IsPlayer(sc) && sc != Hero && !IsCorrectSkillType_FrendlySkill(MagicNumber) &&
        !IsCorrectSkillType_Buff(MagicNumber))
    {
        AttackPlayer = Index;
    }
    ...
    so->SetAngleZ((Data->Angle / 255.f) * 360.f);
    ...
    if (so->Type == MODEL_PLAYER)
    {
        if (sc != Hero)
        {
            ...
            case AT_SKILL_SUMMON_POLLUTION: {
                g_SummonSystem.CastSummonSkill(MagicNumber, sc, so, Data->PositionX,
                                               Data->PositionY);
            }
            break;
...
// ChainLightning
void SessionNetworkUnit::ReceiveChainMagic(const BYTE *ReceiveBuffer)
{
    ...
    const WORD magicNumber = (static_cast<WORD>(pPacketData->MagicH) << 8) +
                             pPacketData->MagicL;
    const bool playerAttack = IsPlayer(pSourceChar) && pSourceChar != Hero &&
                              !IsCorrectSkillType_FrendlySkill(magicNumber) &&
                              !IsCorrectSkillType_Buff(magicNumber);
    bool registeredSelfDefense = false;
    ...
    for (int i = 0; i < (int)(pPacketData->byCount); i++)
    {
        ...
        if (playerAttack && !registeredSelfDefense && pPacketData2->wTargetIndex == HeroKey)
        {
            muHelper_.AddTarget(pSourceChar->Key, true);
            registeredSelfDefense = true;
        }
        ...
        if (pTempObject != pTargetObject && pTargetObject != nullptr && pTargetObject->Live == true)
        {
            ...
            CreateEffect(MODEL_CHAIN_LIGHTNING, pSourceObject->Position, vAngle,
                         pSourceObject->Light, i, pTempObject, -1, 0, 0, 0, 0.f,
                         pPacketData2->wTargetIndex);
        }
...
void SessionNetworkUnit::ReceiveMagicPosition(const BYTE *ReceiveBuffer, int Size)
{
    ...
    CHARACTER *c = &CharactersClient[Index];
    OBJECT *o = &c->Object;
    const bool playerAttack = IsPlayer(c) && c != Hero &&
                              !IsCorrectSkillType_FrendlySkill(MagicNumber) &&
                              !IsCorrectSkillType_Buff(MagicNumber);
    bool registeredSelfDefense = false;
    CreateMagicShiny(c);
    ...
    for (int i = 0; i < Data->Count; i++)
    {
        ...
        OBJECT *to = &tc->Object;
        if (playerAttack && !registeredSelfDefense && TargetKey == HeroKey)
        {
            muHelper_.AddTarget(SourceKey, true);
            registeredSelfDefense = true;
        }
...
void SessionNetworkUnit::ReceiveSkillCount(const BYTE *ReceiveBuffer)
{
...
BOOL SessionNetworkUnit::ReceiveDieExp(const BYTE *ReceiveBuffer, BOOL bEncrypted)
{
                                                   ...
                                                   CharacterAttribute->Level) == true)
    {
        ...
        Master_Level_Data.lMasterLevel_Experince =
            ClampToInterval(SaturatingAddToUpper(currentExperience, addedExperience, upperBound),
    ...
    else
    {
        ...
        const auto upperBound = CharacterAttribute->NextExperience;
        const auto currentExperience =
            ClampToInterval(CharacterAttribute->Experience, lowerBound, upperBound);
...
BOOL SessionNetworkUnit::ReceiveDieExpLarge(const BYTE *ReceiveBuffer, BOOL bEncrypted)
{
    ...
    case eExperienceType_MaxLevelReached:
        ...
        g_pSystemLogBox->AddText(L"You already reached maximum Level.",
                                 SEASON3B::TYPE_SYSTEM_MESSAGE);
        return TRUE;
    case eExperienceType_MaxMasterLevelReached:
        // TODO: show message "You already reached maximum master Level."
        g_pSystemLogBox->AddText(L"You already reached maximum master Level.",
                                 SEASON3B::TYPE_SYSTEM_MESSAGE);
        return TRUE;
    ...
    if (experienceType == eExperienceType_Master)
    {
        ...
        const auto currentExperience =
            ClampToInterval(Master_Level_Data.lMasterLevel_Experince, lowerBound, upperBound);
        Master_Level_Data.lMasterLevel_Experince =
            ClampToInterval(SaturatingAddToUpper(currentExperience,
                                                 static_cast<int64_t>(addedExperience), upperBound),
                            lowerBound, upperBound);
    }
    else
    {
        ...
        CharacterAttribute->Experience = ClampToInterval(
            SaturatingAddToUpper(currentExperience, static_cast<uint64_t>(addedExperience),
                                 upperBound),
            lowerBound, upperBound);
    }
...
void SessionNetworkUnit::FallingStartCharacter(CHARACTER *c, OBJECT *o)
{
    ...
    if (gMapManager.InChaosCastle() == true &&
        (TerrainWall[WallIndex] & TW_NOGROUND) == TW_NOGROUND)
    {
        c->StormTime = 0;
...
void SessionNetworkUnit::ReceiveDie(const BYTE *ReceiveBuffer, int Size)
{
    ...
    else
    {
        ...
        if (SkillType == AT_SKILL_NOVA || SkillType == AT_SKILL_COMBO)
        {
            ...
            o->SetAngleZ(CreateAngle2D(o->Position, to->Position));
...
void SessionNetworkUnit::ReceiveCreateMoney(std::span<const BYTE> ReceiveBuffer)
{
    ...
    muHelper_.AddItem(Data->Id, {Data->PositionX, Data->PositionY});

    g_ConsoleDebug.Write(MCD_RECEIVE, L"0x20 [ReceiveCreateMoney]");
}

void SessionNetworkUnit::ReceiveCreateItemViewportExtended(std::span<const BYTE> ReceiveBuffer)
{
    auto Data = safe_cast<PWHEADER_DEFAULT_WORD>(ReceiveBuffer);
    if (Data == nullptr)
    {
    ...
    for (int i = 0; i < Data->Value; i++)
    {
        ...
        auto params = GameLogic::Items::ParseItemData(itemData);
        if (!params)
            return;
        vec3_t Position;
        ...
        muHelper_.AddItem(id, {itemStartData->PositionX, itemStartData->PositionY});
...
void SessionNetworkUnit::ReceiveDeleteItemViewport(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPWHEADER_DEFAULT_WORD)ReceiveBuffer;
    int Offset = sizeof(PWHEADER_DEFAULT_WORD);
    std::vector<OBJECT *> retired;
    retired.reserve(Data->Value);
    for (int i = 0; i < Data->Value; i++)
    {
        auto Data2 = (LPPDELETE_CHARACTER)(ReceiveBuffer + Offset);
        int Key = ((int)(Data2->KeyH) << 8) + Data2->KeyL;
        ...
        retired.push_back(&Items[Key].Object);
        Offset += sizeof(PDELETE_CHARACTER);

        muHelper_.DeleteItem(Key);
    }
    gameplay_.RetireCharacterEffectTargets(retired);
}
...
void SessionNetworkUnit::RequestInventorySync()
{
...
void SessionNetworkUnit::ReceiveGetItem(std::span<const BYTE> ReceiveBuffer)
{
    ...
    else
    {
        if (Data->Value == GET_ITEM_ZEN)
        {
            ...
            CharacterMachine->Gold = (Data2->Money[0] << 24) + (Data2->Money[1] << 16) +
                                     (Data2->Money[2] << 8) + (Data2->Money[3]);

            int getGold = CharacterMachine->Gold - backupGold;

            if (getGold > 0)
            {
                mu_swprintf(szMessage, L"%d %ls %ls", getGold, I18N::Game::Zen,
                            I18N::Game::Obtained);
        ...
        else
        {
            ...
            if (itemIndex != GET_ITEM_MULTI)
            {
                ...
                {
                    if (sessionKeeper_.GameData()->InsertInventoryItem(itemIndex, itemData))
                    {
                        pickedItem = sessionKeeper_.GameData()->FindInventoryItemBySlot(itemIndex);
                    }
                    else
                    {
            ...
            if (Type == ITEM_JEWEL_OF_BLESS || Type == ITEM_JEWEL_OF_SOUL ||
                ...
                Type == ITEM_JEWEL_OF_CREATION || Type == INDEX_COMPILED_CELE ||
...
void SessionNetworkUnit::ReceiveDropItem(const BYTE *ReceiveBuffer)
{
    ...
    if (Data->KeyH)
    {
        if (Data->KeyL < 12)
        {
            sessionKeeper_.Gameplay()->UnequipItem(Data->KeyL);
        }
        else
        {
            sessionKeeper_.GameData()->DeleteInventoryItem(Data->KeyL);
        }
...
BOOL SessionNetworkUnit::ReceiveEquipmentItemExtended(std::span<const BYTE> ReceiveBuffer)
{
    EquipmentItem = false;
    ...
    if (Data->SubCode != 255)
    {
        ...
        int iSourceIndex = sessionKeeper_.InventoryStorage().pendingMoveSlot;
        if (pPickedItem)
        {
        ...
        sessionKeeper_.GameData()->CompleteItemMove(true);
        ...
        if (storageType == STORAGE_TYPE::INVENTORY)
        {
            ...
            {
                shouldResyncInventory = !sessionKeeper_.GameData()
                                             ->Inventory(InventoryRole::MyShop)
                                             .AddItem(itemindex, itemData);
            }
        ...
        {
            sessionKeeper_.GameData()
                ->Inventory(InventoryRole::TradeOwn)
                .AddItem(Data->Index, itemData);
        }
        ...
        if (storageType == STORAGE_TYPE::CHAOS_MIX ||
            (storageType >= STORAGE_TYPE::TRAINER_MIX &&
             storageType <= STORAGE_TYPE::DETACH_SOCKET_MIX))
        {
            sessionKeeper_.GameData()->ClearPickedItem();
        ...
        else if (storageType == STORAGE_TYPE::LUCKYITEM_TRADE ||
                 storageType == STORAGE_TYPE::LUCKYITEM_REFINERY)
        {
            sessionKeeper_.GameData()->Inventory(InventoryRole::LuckyCrafting).RemoveAllItems();
            sessionKeeper_.GameData()
                ->Inventory(InventoryRole::LuckyCrafting)
                .AddItem(Data->Index, itemData);
            if (g_pNewUISystem && g_pLuckyItemWnd)
                g_pLuckyItemWnd->GetResult(1);
        }
    ...
    {
        sessionKeeper_.Gameplay()->RestorePickedItem();
        sessionKeeper_.GameData()->CompleteItemMove(false);
    }

    if (g_pNewUISystem && g_pStorageInventory)
        g_pStorageInventory->SetItemAutoMove(false);
    if (g_pNewUISystem && g_pStorageInventoryExt)
        g_pStorageInventoryExt->SetItemAutoMove(false);
    ...
    g_ConsoleDebug.Write(MCD_RECEIVE, L"0x24 [ReceiveEquipmentItem(%d %d)]", Data->SubCode,
                         Data->Index);

    return (TRUE);
}

void SessionNetworkUnit::ReceiveModifyItemExtended(std::span<const BYTE> ReceiveBuffer)
{
    auto Data = safe_cast<PHEADER_DEFAULT_SUBCODE_ITEM_EXTENDED>(ReceiveBuffer);
    if (Data == nullptr)
    {
    ...
    int itemindex = Data->Index;
    if (IsMainInventorySlot(itemindex) &&
        sessionKeeper_.GameData()->FindInventoryItemBySlot(itemindex))
    {
        sessionKeeper_.GameData()->DeleteInventoryItem(itemindex);
    }
    else if (IsInventoryExtensionSlot(itemindex) &&
    ...
    else if (g_GambleSystem.IsGambleShop())
    {
    }
    else
    {
...
BOOL SessionNetworkUnit::ReceiveTalk(const BYTE *ReceiveBuffer, BOOL bEncrypted)
{
    auto Data = (LPPHEADER_DEFAULT)ReceiveBuffer;

    sessionKeeper_.InventoryStorage().npcInventory =
        Data->Value == 2 ? InventoryRole::Vault : InventoryRole::NpcShop;
    ...
    case 7:
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CTrainerMenuMsgBoxLayout, sessionKeeper_));
        break;
    ...
    case INDEX_NPC_LAHAP: {
        if (isAble())
        {
            g_pNewUISystem->HideAll();
            SEASON3B::CreateMessageBox(
                MSGBOX_LAYOUT_CLASS(SEASON3B::CGemIntegrationMsgBoxLayout, sessionKeeper_));
        }
    }
    break;
    ...
    case 0x11: {
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CElpisMsgBoxLayout, sessionKeeper_));
    }
    break;
    case 0x12: {
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::COsbourneMsgBoxLayout, sessionKeeper_));
        // 			BYTE *pbyChaosRate = ( &Data->Value) + 1;
    ...
    case 0x17: {
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CSeedMasterMenuMsgBoxLayout, sessionKeeper_));
    }
    break;
    case 0x18: {
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CSeedInvestigatorMenuMsgBoxLayout, sessionKeeper_));
    }
    break;
    case 0x19: {
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CResetCharacterPointMsgBoxLayout, sessionKeeper_));
    }
    break;
    case 0x20: {
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CDelgardoMainMenuMsgBoxLayout, sessionKeeper_));
    }
    break;
    case 0x21: {
    ...
    case 0x26: {
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CLuckyTradeMenuMsgBoxLayout, sessionKeeper_));
    }
    break;
    default: {
        // Data->Value
...
void ReceiveBuy(const BYTE* ReceiveBuffer)
{
    ...
    if (Data->Index != 255)
    {
        ...
        {
            sessionKeeper_.GameData()->InsertInventoryItem(Data->Index, Data->Item, Old);
        }
...
void SessionNetworkUnit::ReceiveBuyExtended(const std::span<const BYTE> ReceiveBuffer)
{
    ...
    else
    {
        ...
        {
            sessionKeeper_.GameData()->InsertInventoryItem(Data->Index, itemData);
        }
...
void SessionNetworkUnit::ReceiveTradeYourInventoryExtended(std::span<const BYTE> ReceiveBuffer)
{
    ...
    sessionKeeper_.GameData()->Inventory(InventoryRole::TradeOther).AddItem(Data->Index, itemData);
    if (g_pNewUISystem && g_pTrade)
        g_pTrade->AlertYourTradeInven();
    PlayBuffer(SOUND_GET_ITEM01);
}

void SessionNetworkUnit::ReceiveMixExtended(std::span<const BYTE> ReceiveBuffer)
{
    auto Data = safe_cast<PHEADER_DEFAULT_ITEM_EXTENDED>(ReceiveBuffer);
    if (Data == nullptr)
    {
    ...
    if (sessionKeeper_.GameData()->CompleteLuckyMix(Data->Index, itemData))
    {
        if (g_pNewUISystem && g_pLuckyItemWnd)
            g_pLuckyItemWnd->GetResult(Data->Index);
        return;
    }
    ...
    case 0: {
        ...
        case SEASON3A::MIXTYPE_CHERRYBLOSSOM:
            mu_swprintf(szText, I18N::Game::SHasFailed2112,
                        I18N::Game::CherryBlossomsBranchesAssembly);
            g_pSystemLogBox->AddText(szText, SEASON3B::TYPE_ERROR_MESSAGE);
            break;
        }
    }
    ...
    case 1: {
        ...
        case SEASON3A::MIXTYPE_CHAOS_CARD:
            ...
            break;
        case SEASON3A::MIXTYPE_CHERRYBLOSSOM:
            mu_swprintf(szText, I18N::Game::SWasSuccessful,
                        I18N::Game::CherryBlossomsBranchesAssembly);
            g_pSystemLogBox->AddText(szText, SEASON3B::TYPE_SYSTEM_MESSAGE);
            break;
        }
        ...
        sessionKeeper_.GameData()->Inventory(InventoryRole::Crafting).RemoveAllItems();
        sessionKeeper_.GameData()->Inventory(InventoryRole::Crafting).AddItem(0, itemData);
    ...
    break;
    case 4:
        CreateOkMessageBox(I18N::Game::MustBeOverLevel10ToCombineTheInvitationToDevilSquare);
        g_pMixInventory->SetMixState(SEASON3B::CNewUIMixInventory::MIX_FINISHED);
        break;

    case 9:
        CreateOkMessageBox(I18N::Game::MustBeOverLevel15ToCombineACloakOfInvisibility);
        g_pMixInventory->SetMixState(SEASON3B::CNewUIMixInventory::MIX_FINISHED);
        break;
...
void SessionNetworkUnit::ReceiveSell(const BYTE *ReceiveBuffer)
{
    ...
    if (Data->Flag != 0)
    {
        if (Data->Flag == 0xff)
        {
            ...
            g_pChatListBox->AddText(Hero->ID, I18N::Game::CannotBeSold,
                                    SEASON3B::TYPE_ERROR_MESSAGE);
        }
...
void SessionLegacyCalls::ReceiveSell(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveSell(receiveBuffer);
}

void SessionNetworkUnit::ReceiveRepair(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveLevelUp(const BYTE *ReceiveBuffer, int Size)
{
...
void SessionNetworkUnit::ReceiveAddPoint(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveAddPointExtended(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveSetPointsExtended(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveStatsExtended(const BYTE *ReceiveBuffer)
{
    ...
    default:
        ...
        if (ITEM *pItem = sessionKeeper_.GameData()->FindInventoryItemBySlot(Data->Index))
        {
            if (pItem->Durability > 0)
                pItem->Durability--;
            if (pItem->Durability <= 0)
                sessionKeeper_.GameData()->DeleteInventoryItem(Data->Index);
        }
...
void SessionNetworkUnit::ReceivePK(const BYTE *ReceiveBuffer)
{
    ...
    case 5: {
        wchar_t szTemp[100];
        mu_swprintf(szTemp, L"%ls %d%ls", I18N::Game::_1stStageOutlaw, 1,
                    I18N::Game::_2ndStageOutlaw);
    ...
    case 6: {
        wchar_t szTemp[100];
        mu_swprintf(szTemp, L"%ls %d%ls", I18N::Game::_1stStageOutlaw, 2,
                    I18N::Game::_2ndStageOutlaw);
        wcscat(message, szTemp);
        g_pSystemLogBox->AddText(message, SEASON3B::TYPE_ERROR_MESSAGE);
    }
...
void SessionNetworkUnit::ReceiveDurability(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPHEADER_DEFAULT_VALUE_KEY)ReceiveBuffer;
    ...
    {
        ITEM *pItem = sessionKeeper_.GameData()->FindInventoryItemBySlot(Data->Value);
        if (pItem == nullptr && IsInventoryExtensionSlot(Data->Value))
        {
            pItem = sessionKeeper_.GameData()->FindInventoryItemBySlot(Data->Value);
        }
...
BOOL SessionNetworkUnit::ReceiveHelperItem(const BYTE *ReceiveBuffer, BOOL bEncrypted)
{
...
void SessionNetworkUnit::ReceiveWeather(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
...
void SessionNetworkUnit::ReceiveEvent(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveSummonLife(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPHEADER_DEFAULT_SUBCODE)ReceiveBuffer;
    SummonLife = Data->Value;
}
BOOL SessionNetworkUnit::ReceiveTrade(const BYTE *ReceiveBuffer, BOOL bEncrypted)
{
    auto Data = (LPPCHATING)ReceiveBuffer;
...
void SessionNetworkUnit::ReceiveTradeResult(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPTRADE)ReceiveBuffer;
    g_pTrade->ProcessToReceiveTradeResult(Data);
}

void SessionNetworkUnit::ReceiveTradeYourInventoryDelete(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
    if (g_pNewUISystem && g_pTrade)
        g_pTrade->BackUpYourInven(Data->Value);
    sessionKeeper_.GameData()->Inventory(InventoryRole::TradeOther).RemoveItemAt(Data->Value);
    if (g_pNewUISystem && g_pTrade)
        g_pTrade->AlertYourTradeInven();
    PlayBuffer(SOUND_GET_ITEM01);
}
...
void SessionNetworkUnit::ReceiveTradeMyGold(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
    g_pTrade->ProcessToReceiveMyTradeGold(Data->Value);
}

void SessionNetworkUnit::ReceiveTradeYourGold(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPHEADER_DEFAULT_DWORD)ReceiveBuffer;
    g_pTrade->SetYourTradeGold(int(Data->Value));
}

void SessionNetworkUnit::ReceiveTradeYourResult(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
    g_pTrade->ProcessToReceiveYourConfirm(Data->Value);
}

void SessionNetworkUnit::ReceiveTradeExit(const BYTE *ReceiveBuffer)
{
    if (EquipmentItem)
    {
        if (!g_bPacketAfter_EquipmentItem)
...
void SessionNetworkUnit::ReceivePing(const BYTE *ReceiveBuffer)
{
    SocketClient->ToGameServer()->SendPingResponse();
}

void SessionLegacyCalls::ReceivePing(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceivePing(receiveBuffer);
}

void SessionNetworkUnit::ReceiveStorageGold(const BYTE *ReceiveBuffer)
{
    ...
    g_ConsoleDebug.Write(MCD_RECEIVE, L"0x81 [ReceiveStorageGold(%d %d %d)]", Data->Result,
                         Data->StorageGold, Data->Gold);
}

void SessionNetworkUnit::ReceiveStorageExit(const BYTE *ReceiveBuffer)
{
    g_ConsoleDebug.Write(MCD_RECEIVE, L"0x82 [ReceiveStorageExit]");
}

void SessionNetworkUnit::ReceiveStorageStatus(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
...
void SessionNetworkUnit::ReceiveParty(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPHEADER_DEFAULT_KEY)ReceiveBuffer;
    PartyKey = ((int)(Data->KeyH) << 8) + Data->KeyL;

    SEASON3B::CreateMessageBox(MSGBOX_LAYOUT_CLASS(SEASON3B::CPartyMsgBoxLayout, sessionKeeper_));
}

void SessionNetworkUnit::ReceivePartyResult(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
    switch (Data->Value)
    {
    case 0:
        ...
        break;
    case 1:
        g_pSystemLogBox->AddText(I18N::Game::YourRequestHasBeenDenied,
                                 SEASON3B::TYPE_ERROR_MESSAGE);
        break;
    case 2:
    ...
    case 3:
        ...
        break;
    case 4:
        g_pSystemLogBox->AddText(I18N::Game::TheUserIsAlreadyInAnotherParty,
                                 SEASON3B::TYPE_ERROR_MESSAGE);
        break;
    case 5:
    ...
    case 6:
        ...
        break;
    case 7:
        g_pSystemLogBox->AddText(I18N::Game::YouCannotFormAPartyWithinABattleZone,
                                 SEASON3B::TYPE_ERROR_MESSAGE);
        break;
    case 8:
        g_pSystemLogBox->AddText(I18N::Game::PartiesAreNotActivatedWithinABattleZone,
                                 SEASON3B::TYPE_ERROR_MESSAGE);
        break;
    }
}

void SessionNetworkUnit::ReceivePartyList(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceivePartyInfo(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceivePartyLeave(const BYTE *ReceiveBuffer)
{
    partyLeaveClearedFollow_ = false;
    PartyNumber = 0;
    ...
    if (g_iFollowCharacter >= 0)
    {
        ...
        if (IsParty == false)
        {
            partyLeaveClearedFollow_ = true;
        }
    }
}

void SessionNetworkUnit::ReceivePartyGetItem(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveGuild(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPHEADER_DEFAULT_KEY)ReceiveBuffer;
    ...
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CGuildRequestMsgBoxLayout, sessionKeeper_), &pMsgBox);
...
void SessionNetworkUnit::ReceiveGuildResult(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
    switch (Data->Value)
    {
    case 0:
        ...
        break;
    case 1:
        g_pSystemLogBox->AddText(I18N::Game::YouHaveJustJoinedTheGuild,
                                 SEASON3B::TYPE_ERROR_MESSAGE);
        break;
    case 2:
    ...
    case 4:
        ...
        break;
    case 5:
        g_pSystemLogBox->AddText(I18N::Game::YouCannotJoinMoreThanOneGuild,
                                 SEASON3B::TYPE_ERROR_MESSAGE);
        break;
    case 6:
        g_pSystemLogBox->AddText(
            I18N::Game::TheGuildMasterIsTooBusyToApproveYourRequestToJoinTheGuild,
            SEASON3B::TYPE_ERROR_MESSAGE);
        break;
    case 7:
        g_pSystemLogBox->AddText(I18N::Game::ChractersOverLevel6CanJoinAGuild,
                                 SEASON3B::TYPE_ERROR_MESSAGE);
        break;
    case 0xA1:
        g_pSystemLogBox->AddText(I18N::Game::TheGuildMasterHasNotJoinedTheGens,
                                 SEASON3B::TYPE_ERROR_MESSAGE);
        break;
    case 0xA2:
        g_pSystemLogBox->AddText(I18N::Game::TheGuildMasterIsWithADifferentGens,
                                 SEASON3B::TYPE_ERROR_MESSAGE);
        break;
    case 0xA3:
...
void SessionNetworkUnit::ReceiveGuildList(const BYTE *ReceiveBuffer)
{
    ...
    CMultiLanguage::ConvertFromUtf8(rivalGuildName, Data->szRivalGuildName,
                                    sizeof Data->szRivalGuildName);
...
void SessionNetworkUnit::ReceiveGuildLeave(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
    switch (Data->Value)
    {
    ...
    case 1:
        ...
        break;
    case 2:
        g_pSystemLogBox->AddText(I18N::Game::OnlyAGuildMasterCanDisbandAGuild,
                                 SEASON3B::TYPE_ERROR_MESSAGE);
        break;
    case 3:
        g_pSystemLogBox->AddText(I18N::Game::YouHaveFailedFromTheGuild,
                                 SEASON3B::TYPE_ERROR_MESSAGE);
        break;
    case 4:
        g_pSystemLogBox->AddText(I18N::Game::TheGuildHasBeenDissolved,
                                 SEASON3B::TYPE_ERROR_MESSAGE);
        break;
    case 5:
        g_pSystemLogBox->AddText(I18N::Game::GuildMemberHasBeenWithdrawn,
                                 SEASON3B::TYPE_ERROR_MESSAGE);
        break;
    }
...
void SessionLegacyCalls::ReceiveGuildLeave(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveGuildLeave(receiveBuffer);
}

void SessionNetworkUnit::ReceiveCreateGuildInterface(const BYTE *ReceiveBuffer)
{
    g_pNewUISystem->Show(SEASON3B::INTERFACE_NPCGUILDMASTER);
}

void SessionNetworkUnit::ReceiveCreateGuildMasterInterface(const BYTE *ReceiveBuffer)
{
}

void SessionNetworkUnit::ReceiveDeleteGuildViewport(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPHEADER_DEFAULT_KEY)ReceiveBuffer;
...
void SessionNetworkUnit::ReceiveCreateGuildResult(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPMSG_GUILD_CREATE_RESULT)ReceiveBuffer;
    guildCreateResetCadence_ = false;
    switch (Data->Value)
    {
    case 0:
        g_pSystemLogBox->AddText(I18N::Game::TheGuildNameAlreadyExists,
                                 SEASON3B::TYPE_ERROR_MESSAGE);
        break;
    case 2:
        g_pSystemLogBox->AddText(I18N::Game::GuildNameMustBeAtLeast4Characters,
                                 SEASON3B::TYPE_ERROR_MESSAGE);
        break;
    case 3:
        g_pSystemLogBox->AddText(I18N::Game::YouAreAlreadyInAGuild518,
                                 SEASON3B::TYPE_ERROR_MESSAGE);
        break;
    case 4:
        g_pSystemLogBox->AddText(I18N::Game::NoSpaceAllowedInGuildNames,
                                 SEASON3B::TYPE_ERROR_MESSAGE);
        break;
    case 5:
        g_pSystemLogBox->AddText(I18N::Game::NoSymbolsAllowedInGuildNames,
                                 SEASON3B::TYPE_ERROR_MESSAGE);
        break;
    case 6:
    ...
    case 1:
        ...
        guildCreateResetCadence_ = true;
        break;
    }
}

void SessionNetworkUnit::ReceiveDeclareWar(const BYTE *ReceiveBuffer)
{
    ...
    {
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CBattleSoccerMsgBoxLayout, sessionKeeper_));
    }
    else
    {
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CGuildWarMsgBoxLayout, sessionKeeper_));
    }
}

void SessionNetworkUnit::ReceiveDeclareWarResult(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
    switch (Data->Value)
    {
    case 0:
        ...
        break;
    case 1:
        g_pSystemLogBox->AddText(I18N::Game::YouHaveDeclaredAGuildWar,
                                 SEASON3B::TYPE_ERROR_MESSAGE);
        break;
    case 2:
        g_pSystemLogBox->AddText(I18N::Game::TheOpposingGuildMasterIsNotInTheGame,
                                 SEASON3B::TYPE_ERROR_MESSAGE);
        break;
    case 3:
        g_pSystemLogBox->AddText(I18N::Game::ThatGuildDoesNotExist, SEASON3B::TYPE_ERROR_MESSAGE);
        break;
    case 4:
        g_pSystemLogBox->AddText(I18N::Game::YouCanNotDeclareAGuildWarNow,
                                 SEASON3B::TYPE_ERROR_MESSAGE);
        break;
    case 5:
        g_pSystemLogBox->AddText(I18N::Game::OnlyGuildMastersCanDeclareAGuildWar,
                                 SEASON3B::TYPE_ERROR_MESSAGE);
        break;
    case 6:
        g_pSystemLogBox->AddText(I18N::Game::YourRequestForAGuildWarIsRefused,
                                 SEASON3B::TYPE_ERROR_MESSAGE);
        break;
    }
...
void SessionNetworkUnit::ReceiveGuildBeginWar(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPRECEIVE_WAR)ReceiveBuffer;
    EnableGuildWar = true;
...
void SessionNetworkUnit::ReceiveGuildEndWar(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
    wchar_t Text[100];
    int Win = 0;
...
void SessionNetworkUnit::ReceiveGuildWarScore(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveGuildIDViewport(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPWHEADER_DEFAULT_WORD)ReceiveBuffer;
    int Offset = sizeof(PWHEADER_DEFAULT_WORD);
    for (int i = 0; i < Data->Value; ++i)
...
void SessionNetworkUnit::ReceiveGuildInfo(const BYTE *ReceiveBuffer)
{
...
// 길드직책을 임명/변경/해제 결과
void SessionNetworkUnit::ReceiveGuildAssign(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveGuildRelationShip(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPMSG_GUILD_RELATIONSHIP)ReceiveBuffer;

    g_pGuildInfoWindow->ReceiveGuildRelationShip(
        static_cast<GuildRelationshipType>(pData->byRelationShipType),
        static_cast<GuildRequestType>(pData->byRequestType), pData->byTargetUserIndexH,
        pData->byTargetUserIndexL);
}

void SessionNetworkUnit::ReceiveGuildRelationShipResult(const BYTE *ReceiveBuffer)
{
    ...
    if (nCharKey == HeroKey && pData->byResult == 0x01 && pData->byRelationShipType == 0x01 &&
        ...
        GuildMark[Hero->GuildMarkIndex].UnionName[0] = 0;
    if (pData->byResult == 1 && Hero->GuildMarkIndex >= 0)
    {
        SocketClient->ToGameServer()->SendGuildInfoRequest(GuildMark[Hero->GuildMarkIndex].Key);
        SocketClient->ToGameServer()->SendGuildListRequest();
        SocketClient->ToGameServer()->SendRequestAllianceList();
    }
}

void SessionNetworkUnit::ReceiveBanUnionGuildResult(const BYTE *ReceiveBuffer)
{
...
void SessionLegacyCalls::ReceiveBanUnionGuildResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveBanUnionGuildResult(receiveBuffer);
}

void SessionNetworkUnit::ReceiveUnionViewportNotify(const BYTE *ReceiveBuffer)
{
    ...
    for (int i = 0; i < pData->byCount; ++i)
    {
        ...
        CMultiLanguage::ConvertFromUtf8(GuildMark[nGuildMarkIndex].UnionName, pData2->szUnionName,
                                        MAX_GUILDNAME);
...
void SessionNetworkUnit::ReceiveUnionList(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveSoccerTime(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPRECEIVE_SOCCER_TIME)ReceiveBuffer;
    SoccerTime = Data->Time;
}

void SessionNetworkUnit::ReceiveSoccerScore(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveSoccerGoal(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
    wchar_t Text[100];
...
void SessionNetworkUnit::Receive_Master_LevelUp(const BYTE *ReceiveBuffer, int Size)
{
    ...
    wchar_t szText[256] = {};
    DWORD iExp =
        Master_Level_Data.lNext_MasterLevel_Experince - Master_Level_Data.lMasterLevel_Experince;
    if (iExp > 0)
    {
...
void SessionNetworkUnit::Receive_Master_Level_Exp(const BYTE *ReceiveBuffer, int Size)
{
...
void SessionNetworkUnit::Receive_Master_LevelGetSkill(const BYTE *ReceiveBuffer)
{
    ...
    if (Data->Result == 1 && Data->SkillNumber > -1)
    {
        ...
        interface->SkillUpgrade(Data->SkillIndex, Data->SkillLevel, Data->DisplayValue,
...
void SessionNetworkUnit::ReceiveServerCommand(const BYTE *ReceiveBuffer)
{
    ...
    case 1:
        if (Data->Cmd2 >= 20)
        {
            CreateOkMessageBox(I18N::Game::Lookup(830 + Data->Cmd2 - 20));
        }
        else
        {
            CreateOkMessageBox(I18N::Game::Lookup(650 + Data->Cmd2));
        }
        break;

    case 3:
        CreateOkMessageBox(I18N::Game::Lookup(710 + Data->Cmd2));
        break;
    case 4:
        CreateOkMessageBox(I18N::Game::Lookup(725 + Data->Cmd2));
        break;
    case 5: {
        SEASON3B::CDialogMsgBox *pMsgBox = nullptr;
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CDialogMsgBoxLayout, sessionKeeper_), &pMsgBox);
        if (pMsgBox)
        {
    ...
    case 6:
        CreateOkMessageBox(I18N::Game::DissolveOrLeaveYourGuild);
        break;
    case 13:
        CreateOkMessageBox(I18N::Game::YouCanNowStandAloneWithoutMySupport);
        break;
    case 14: {
        switch (Data->Cmd2)
        {
        case 0:
            SEASON3B::CreateMessageBox(
                MSGBOX_LAYOUT_CLASS(SEASON3B::CWhiteAngelEventLayout, sessionKeeper_));
            break;

        case 1:
            CreateOkMessageBox(I18N::Game::ThisIsNotAEventPrize);
            break;

        case 2:
            CreateOkMessageBox(I18N::Game::ItemHasAlreadyGiven);
            break;

        case 3:
            CreateOkMessageBox(I18N::Game::FailedToGetAnItemPleaseTryAgain);
            break;
    ...
    case 15: {
        ...
        case 1:
            SEASON3B::CreateMessageBox(
                MSGBOX_LAYOUT_CLASS(SEASON3B::CHarvestEventLayout, sessionKeeper_));
            break;
        case 2:
            CreateOkMessageBox(I18N::Game::FailedToGetAnItemPleaseTryAgain);
            break;
    ...
    case 16: {
        ...
        case 0:
            SEASON3B::CreateMessageBox(
                MSGBOX_LAYOUT_CLASS(SEASON3B::CSantaTownSantaMsgBoxLayout, sessionKeeper_),
                &pMsgBox);
        ...
        case 1:
            ...
            break;
        case 2:
            CreateOkMessageBox(I18N::Game::YouCanClickOnlyOnce);
            break;
        case 3:
            CreateOkMessageBox(I18N::Game::FailedToGetAnItemPleaseTryAgain);
            break;
    ...
    case 17:
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CSantaTownLeaveMsgBoxLayout, sessionKeeper_));
        break;
    case 47:
    case 48:
    case 49:
        CreateOkMessageBox(I18N::Game::Lookup(1823 + Data->Cmd1 - 47));
        break;
    case 55: {
        wchar_t strText[128];
        mu_swprintf(strText, I18N::Game::KillersAreRestrictedToEnterS, I18N::Game::DevilSquare);
        CreateOkMessageBox(strText);
    }
    break;
    case 56: {
        wchar_t strText[128];
...
void SessionNetworkUnit::ReceiveMixExit(const BYTE *ReceiveBuffer)
{
    g_ConsoleDebug.Write(MCD_RECEIVE, L"0x87 [ReceiveMixExit]");
}

void SessionNetworkUnit::ReceiveGemMixResult(const BYTE *ReceiveBuffer)
{
    ...
    case 1: {
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CGemIntegrationUnityResultMsgBoxLayout, sessionKeeper_));
    }
    break;
    case 4: {
...
void SessionNetworkUnit::ReceiveGemUnMixResult(const BYTE *ReceiveBuffer)
{
    ...
    case 1: {
        SEASON3B::CreateMessageBox(MSGBOX_LAYOUT_CLASS(
            SEASON3B::CGemIntegrationDisjointResultMsgBoxLayout, sessionKeeper_));
    }
    break;
    case 2:
    case 3:
    case 4:
    case 6: {
        g_pSystemLogBox->AddText(I18N::Game::CorrespondingItemIsInappropriate,
                                 SEASON3B::TYPE_SYSTEM_MESSAGE);
        gameplay_.GetBack();
    }
    break;
    case 7: {
        g_pSystemLogBox->AddText(I18N::Game::InventorySpaceIsInsufficient,
                                 SEASON3B::TYPE_SYSTEM_MESSAGE);
        gameplay_.GetBack();
    }
    break;
...
void SessionNetworkUnit::ReceiveMoveToDevilSquareResult(const BYTE *ReceiveBuffer)
{
    ...
    case 0:
        break;
    case 1:
        CreateOkMessageBox(I18N::Game::BringTheDevilSInvitationToEnter);
        break;

    case 2:
        CreateOkMessageBox(I18N::Game::YouVeComeTooLateToEnterTheDevilSquare);
        break;

    case 3:
        CreateOkMessageBox(I18N::Game::YouReUnderestimatingYourselfChooseAnotherSquare);
        break;

    case 4:
        CreateOkMessageBox(I18N::Game::IfYouWishToStayAliveChooseAnotherSquare);
        break;

    case 5:
        CreateOkMessageBox(I18N::Game::DevilSquareIsFull);
        break;
...
void SessionNetworkUnit::ReceiveDevilSquareOpenTime(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
    if (0 == Data->Value)
    {
        CreateOkMessageBox(I18N::Game::YouCanEnterDevilSquareNow);
    }
    else
    {
        wchar_t strText[128];
...
void SessionNetworkUnit::ReceiveDevilSquareCountDown(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
    ...
    if (gMapManager.IsCursedTemple())
    {
        ...
        {
            cursedTemple_.SetInterfaceState(false, 0);
            gameplay_.StartMatchCountDown(Data->Value + 1);
        }
    }
             ...
             gMapManager.ContextMap() <= WD_68DOPPLEGANGER4)
    {
        ...
        {
            gameplay_.StartMatchCountDown((BYTE)(Data->Value + 1));
        }
    }
    else
    {
...
void SessionNetworkUnit::ReceiveDevilSquareRank(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPDEVILRANK)ReceiveBuffer;
    gameplay_.SetMatchResult(Data->m_Count, Data->m_MyRank, (MatchResult *)&Data->m_byRank,
                             Data->m_MyRank);
}

void SessionNetworkUnit::ReceiveMoveToEventMatchResult(const BYTE *ReceiveBuffer)
{
    g_pNewUISystem->HideAll();
    ...
    case 0:
        break;
    case 1:
        CreateOkMessageBox(I18N::Game::TheLevelOfTheCloakOfInvisibilityIsIncorrect);
        break;
...
void SessionNetworkUnit::ReceiveEventZoneOpenTime(const BYTE *ReceiveBuffer)
{
    ...
    else if (Data->Value == 2)
    {
        ...
        {
            mu_swprintf(strText, I18N::Game::AfterDMinutesYouMayEnterS, (int)Data->KeyH,
                        I18N::Game::BloodCastle);
        }
        CreateOkMessageBox(strText);
    }
    else if (Data->Value == 4)
    {
        ...
        if (0 == time)
        {
            ...
            mu_swprintf(szOpenTime2, I18N::Game::InSCurrentlyDDEntered, I18N::Game::ChaosCastle,
                        Data->KeyM, 100);

            SEASON3B::CNewUICommonMessageBox *pMsgBox = nullptr;
            SEASON3B::CreateMessageBox(
                MSGBOX_LAYOUT_CLASS(SEASON3B::CChaosCastleTimeCheckMsgBoxLayout, sessionKeeper_),
                &pMsgBox);
            if (pMsgBox)
            {
...
void SessionNetworkUnit::ReceiveMoveToEventMatchResult2(const BYTE *ReceiveBuffer)
{
    g_pNewUISystem->HideAll();
...
void SessionNetworkUnit::ReceiveSetAttribute(const BYTE *ReceiveBuffer)
{
    ...
    case 0: {
        ...
        {
            SetActionObject(gMapManager.ContextMap(), 36, 0, 1.f);
        }
        ...
        for (int k = 0; k < Data->m_byCount; k++)
        {
            ...
            AddTerrainAttributeRange(Data->m_vAttribute[(k * 2)].m_byX,
                                     Data->m_vAttribute[(k * 2)].m_byY, dx, dy, Data->m_byMapAttr,
                                     1 - Data->m_byMapSetType);
        }
    }
    break;
        ...
        for (int i = 0; i < Data->m_byCount; ++i)
        {
            if (Data->m_byMapSetType)
            {
                g_ErrorReport.Write(L"SubTerrainAttribute - count:%d, x:%d, y:%d \r\n",
                                    Data->m_byCount, Data->m_vAttribute[i].m_byX,
                                    Data->m_vAttribute[i].m_byY);
                SubTerrainAttribute(Data->m_vAttribute[i].m_byX, Data->m_vAttribute[i].m_byY,
                                    Data->m_byMapAttr);
            }
            else
            {
                g_ErrorReport.Write(L"AddTerrainAttribute - count:%d, x:%d, y:%d \r\n",
                                    Data->m_byCount, Data->m_vAttribute[i].m_byX,
                                    Data->m_vAttribute[i].m_byY);
                AddTerrainAttribute(Data->m_vAttribute[i].m_byX, Data->m_vAttribute[i].m_byY,
                                    Data->m_byMapAttr);
            }
        }
        break;
...
void SessionNetworkUnit::ReceiveMatchGameCommand(const BYTE *ReceiveBuffer)
{
    gameplay_.SetMatchGameCommand(
        reinterpret_cast<LPPRECEIVE_MATCH_GAME_STATE>(const_cast<BYTE *>(ReceiveBuffer)));
}

void SessionNetworkUnit::ReceiveDuelRequest(const BYTE *ReceiveBuffer)
{
    ...
    {
        SocketClient->ToGameServer()->SendDuelStartResponse(FALSE, enemyKey, playerName);
        return;
    }

    SEASON3B::CreateMessageBox(MSGBOX_LAYOUT_CLASS(SEASON3B::CDuelMsgBoxLayout, sessionKeeper_));
    PlayBuffer(SOUND_OPEN_DUELWINDOW);
}

void SessionNetworkUnit::ReceiveDuelStart(const BYTE *ReceiveBuffer)
{
    ...
    if (Data->nResult == 0)
    {
        ...
        mu_swprintf(szMessage, I18N::Game::SHasAcceptedYourChallenge,
                    g_DuelMgr.GetDuelPlayerID(DUEL_ENEMY));
    ...
    {
        g_DuelMgr.SetDuelPlayer(DUEL_ENEMY, MAKEWORD(Data->bIndexL, Data->bIndexH), playerName);
        mu_swprintf(szMessage, I18N::Game::SHasDeclinedYourChallenge,
                    g_DuelMgr.GetDuelPlayerID(DUEL_ENEMY));
        g_pSystemLogBox->AddText(szMessage, SEASON3B::TYPE_ERROR_MESSAGE);
    }
    ...
    {
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CDuelCreateErrorMsgBoxLayout, sessionKeeper_));
    }
...
void SessionNetworkUnit::ReceiveDuelEnd(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveDuelScore(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveDuelHP(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveDuelChannelList(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveDuelWatchRequestReply(const BYTE *ReceiveBuffer)
{
    ...
    {
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CDuelWatchErrorMsgBoxLayout, sessionKeeper_));
    }
    else
    {
    }
...
void SessionNetworkUnit::ReceiveDuelWatcherJoin(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveDuelWatchEnd(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveDuelWatcherQuit(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveDuelWatcherList(const BYTE *ReceiveBuffer)
{
    g_DuelMgr.RemoveAllDuelWatchUser();
...
void SessionNetworkUnit::ReceiveDuelResult(const BYTE *ReceiveBuffer)
{
    ...
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CDuelResultMsgBoxLayout, sessionKeeper_), &lpMsgBox);
    if (lpMsgBox)
    {
...
void SessionNetworkUnit::ReceiveDuelRound(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveCreateShopTitleViewport(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveShopTitleChange(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveSetPriceResult(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveCreatePersonalShop(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveDestroyPersonalShop(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceivePersonalShopItemList(std::span<const BYTE> ReceiveBuffer)
{
    ...
    if (Header->byResult == Success)
    {
        ...
        sessionKeeper_.GameData()->Inventory(InventoryRole::OtherShop).RemoveAllItems();
        ...
        for (int i = 0; i < Header->ItemCount; i++)
        {
            ...
            {
                sessionKeeper_.GameData()
                    ->Inventory(InventoryRole::OtherShop)
                    .AddItem(pShopItem->ItemSlot, itemData);
                AddPersonalItemPrice(pShopItem->ItemSlot, pShopItem->MoneyPrice,
            ...
            else
            {
                ...
                g_ErrorReport.Write(
                    L"@ ReceivePersonalShopItemList - item price less than zero(%d)\n",
    ...
    else
    {
        ...
        {
        case Fail1: {
            g_pSystemLogBox->AddText(I18N::Game::StoreIsNotOpenAtTheMoment,
                                     SEASON3B::TYPE_ERROR_MESSAGE);
        }
        break;
        case Fail2:
...
void SessionNetworkUnit::ReceiveRefreshItemList(std::span<const BYTE> ReceiveBuffer)
{
    auto Header = safe_cast<GETPSHOPITEMLIST_HEADERINFO>(ReceiveBuffer);
    if (Header == nullptr)
    {
    ...
    {
        sessionKeeper_.GameData()->Inventory(InventoryRole::MyShop).RemoveAllItems();
        for (int i = 0; i < Header->ItemCount; i++)
        {
            auto pShopItem = safe_cast<GETPSHOPITEM_DATAINFO>(ReceiveBuffer.subspan(Offset));
            if (pShopItem == nullptr)
    ...
    else
    {
        if (Header->byResult == 0x01)
        {
            auto *pCurrentInvenCtrl =
                &sessionKeeper_.GameData()->Inventory(InventoryRole::OtherShop);

            size_t uiCntInvenCtrl = pCurrentInvenCtrl->GetNumberOfItems();
            g_ErrorReport.Write(
                L"@ [Notice] ReceiveRefreshItemList (InventoryCtrl Count Items(%d))\n",
                uiCntInvenCtrl);
        }
        else
        {
...
void SessionNetworkUnit::ReceivePurchaseItem(std::span<const BYTE> ReceiveBuffer)
{
    ...
    {
        const int sourceSlot = sessionKeeper_.InventoryStorage().purchaseSourceSlot;
        if (sourceSlot >= 0)
        {
            RemovePersonalItemPrice(sourceSlot, PSHOPWNDTYPE_PURCHASE);
            sessionKeeper_.GameData()->Inventory(InventoryRole::OtherShop).RemoveItemAt(sourceSlot);
        }
    ...
    {
        g_pSystemLogBox->AddText(I18N::Game::FailedToPurchasePleaseTryAgain,
                                 SEASON3B::TYPE_ERROR_MESSAGE);
        g_pNewUISystem->Hide(SEASON3B::INTERFACE_MYSHOP_INVENTORY);
        g_pNewUISystem->Hide(SEASON3B::INTERFACE_PURCHASESHOP_INVENTORY);
    }
...
void SessionNetworkUnit::NotifySoldItem(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::NotifyClosePersonalShop(const BYTE *ReceiveBuffer)
{
    if (g_IsPurchaseShop == PSHOPWNDTYPE_PURCHASE)
    {
        ...
        g_pSystemLogBox->AddText(I18N::Game::TheOtherCharacterHasClosedTheStore,
                                 SEASON3B::TYPE_ERROR_MESSAGE);
    }
}

void SessionNetworkUnit::ReceiveDisplayEffectViewport(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveFriendList(const BYTE *ReceiveBuffer)
{
    g_pWindowMgr->Reset();
    ...
    SocketClient->ToGameServer()->SendSetFriendOnlineState(g_iChatInputType != 0 &&
                                                           !g_pWindowMgr->GetChatReject());
    ...
    if (Header->MemoCount > 0)
    {
        ...
        mu_swprintf(temp, I18N::Game::DLettersAreSavedInYourMailboxMaxD, Header->MemoCount,
                    Header->MaxMemo);
        g_pSystemLogBox->AddText(temp, SEASON3B::TYPE_SYSTEM_MESSAGE);
    }
}

void SessionNetworkUnit::ReceiveAddFriendResult(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveRequestAcceptAddFriend(const BYTE *ReceiveBuffer)
{
    ...
    DWORD dwWindowID =
        g_pWindowMgr->AddWindow(UIWNDTYPE_QUESTION_FORCE, UIWND_DEFAULT, UIWND_DEFAULT, szText, -1);
    if (dwWindowID != 0)
    {
...
void SessionNetworkUnit::ReceiveDeleteFriendResult(const BYTE *ReceiveBuffer)
{
    auto Data = (LPFS_FRIEND_RESULT)ReceiveBuffer;
...
void SessionNetworkUnit::ReceiveFriendStateChange(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveLetterSendResult(const BYTE *ReceiveBuffer)
{
    ...
    case 0x00:
        if (Data->WindowGuid != 0)
            ((CUILetterWriteWindow *)g_pWindowMgr->GetWindow(Data->WindowGuid))
                ->SetSendState(FALSE);
        g_pWindowMgr->AddWindow(UIWNDTYPE_OK_FORCE, UIWND_DEFAULT, UIWND_DEFAULT,
...
namespace
{
LETTERLIST_TEXT DecodeLetterHeader(const FS_LETTER_ALERT &packet)
{
    LETTERLIST_TEXT letter{};
    CMultiLanguage::ConvertFromUtf8(letter.m_szID, packet.Name, MAX_USERNAME_SIZE);
    CMultiLanguage::ConvertFromUtf8(letter.m_szText, packet.Subject, sizeof(packet.Subject));
    CMultiLanguage::ConvertFromUtf8(letter.m_szDate, packet.Date, MAX_LETTER_DATE_LENGTH);
    CMultiLanguage::ConvertFromUtf8(letter.m_szTime, packet.Time, MAX_LETTER_TIME_LENGTH);
    letter.m_dwLetterID = packet.Index;
    letter.m_bIsRead = packet.Read == 0;
    return letter;
}
} // namespace

void SessionNetworkUnit::ReceiveLetter(const BYTE *ReceiveBuffer)
{
    const auto *Data = reinterpret_cast<const FS_LETTER_ALERT *>(ReceiveBuffer);
    const auto letter = DecodeLetterHeader(*Data);
    ...
    case 0x02:
        ...
        g_pLetterList->AddLetter(letter.m_dwLetterID, letter.m_szID, letter.m_szText,
                                 letter.m_szDate, letter.m_szTime, letter.m_bIsRead);
        g_pLetterList->Sort();
        break;
    case 0x00:
    case 0x01:
...
void SessionNetworkUnit::ReceiveLetterText(std::span<const BYTE> ReceiveBuffer, bool isCached)
{
    ...
    {
        dwUIID =
            g_pWindowMgr->AddWindow(UIWNDTYPE_READLETTER, g_iLetterReadNextPos_x,
                                    g_iLetterReadNextPos_y, tempTxt, 0, UIADDWND_FORCEPOSITION);
    ...
    else
    {
        pWindow->m_Photo.SetClass(
            gCharacterManager.ChangeServerClassTypeToClientClassType(Data->Class));
...
void SessionLegacyCalls::ReceiveLetterText(std::span<const BYTE> receiveBuffer, bool isCached)
{
    sessionKeeper_.Network()->ReceiveLetterText(receiveBuffer, isCached);
}

void SessionNetworkUnit::ReceiveLetterDeleteResult(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveCreateChatRoomResult(const BYTE *ReceiveBuffer)
{
    ...
    case 0x01:
        ...
        else if (Data->Type == 1)
        {
            ...
            {
                dwUIID =
                    g_pWindowMgr->AddWindow(UIWNDTYPE_CHAT_READY, 100, 100, I18N::Game::Talking);
                ((CUIChatWindow *)g_pWindowMgr->GetWindow(dwUIID))
                    ->ConnectToChatServer(szIP, Data->RoomNumber, Data->Ticket);
...
void SessionNetworkUnit::ReceiveChatRoomInviteResult(const BYTE *ReceiveBuffer)
{
    ...
    case 0x01:
        ...
        break;
    case 0x03:
        pChatWindow->AddChatText(255, I18N::Game::YouHaveReachedTheMaximumNumberOfFriendsYouCanList,
                                 1, 0);
        break;
    default:
        break;
...
void SessionNetworkUnit::ReceiveOption(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveEventChipInfomation(const BYTE *ReceiveBuffer)
{
...
void SessionLegacyCalls::ReceiveEventChipInfomation(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveEventChipInfomation(receiveBuffer);
}

void SessionNetworkUnit::ReceiveEventChip(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveBuffState(const BYTE *ReceiveBuffer)
{
    ...
    if (Data->byEffectOption == 0)
    {
        ...
        {
            g_pSystemLogBox->AddText(I18N::Game::DamageAndDefenseIncreasedWithABlessing,
                                     SEASON3B::TYPE_SYSTEM_MESSAGE);
        }
    }
    else
...
void SessionNetworkUnit::ReceiveMutoNumber(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveServerImmigration(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
    ...
    case 0:
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CServerImmigrationErrorMsgBoxLayout, sessionKeeper_));
        break;
    case 1:
        CreateOkMessageBox(L"ReceiveServerImmigration");
        break;
    }
}

void SessionNetworkUnit::ReceiveScratchResult(const BYTE *ReceiveBuffer)
{
    ...
    case 3:
    case 4:
        CreateOkMessageBox(I18N::Game::Lookup(886 + Data->m_byIsRegistered));
        break;

    case 5:
        CreateOkMessageBox(I18N::Game::YouHaveAlreadyRegistered);
        break;
    }
...
void SessionNetworkUnit::ReceivePlaySoundEffect(const BYTE *ReceiveBuffer)
{
...
void SessionLegacyCalls::ReceivePlaySoundEffect(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceivePlaySoundEffect(receiveBuffer);
}

void SessionNetworkUnit::ReceiveEventCount(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveQuestHistory(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveQuestState(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveQuestResult(const BYTE *ReceiveBuffer)
{
...
void SessionLegacyCalls::ReceiveQuestResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveQuestResult(receiveBuffer);
}

void SessionNetworkUnit::ReceiveQuestPrize(const BYTE *ReceiveBuffer)
{
    ...
    case 204: {
        ...
        CreateEffect(MODEL_CHANGE_UP_EFF, c->Object.Position, c->Object.Angle, c->Object.Light, 0,
                     &c->Object);
        CreateEffect(MODEL_CHANGE_UP_NASA, c->Object.Position, c->Object.Angle, c->Object.Light, 0,
                     &c->Object);
        CreateEffect(MODEL_CHANGE_UP_CYLINDER, c->Object.Position, c->Object.Angle, c->Object.Light,
                     0, &c->Object);
        CreateEffect(MODEL_INFINITY_ARROW3, c->Object.Position, c->Object.Angle, c->Object.Light, 1,
                     &c->Object);
...
void SessionNetworkUnit::ReceiveQuestMonKillInfo(const BYTE *ReceiveBuffer)
{
...
void SessionLegacyCalls::ReceiveQuestMonKillInfo(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveQuestMonKillInfo(receiveBuffer);
}
...
void ReceiveQuestLimitResult(const BYTE *ReceiveBuffer)
{
    ...
    case QUEST_RESULT_CNT_LIMIT:
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CQuestCountLimitMsgBoxLayout, sessionKeeper_));
        break;
    }
}
...
void SessionNetworkUnit::ReceiveQuestByEtcEPList(const BYTE *ReceiveBuffer)
{
...
void SessionLegacyCalls::ReceiveQuestByEtcEPList(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveQuestByEtcEPList(receiveBuffer);
}

void SessionNetworkUnit::ReceiveQuestByNPCEPList(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPMSG_NPCTALK_QUESTLIST)ReceiveBuffer;
...
void SessionLegacyCalls::ReceiveQuestByNPCEPList(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveQuestByNPCEPList(receiveBuffer);
}

void SessionNetworkUnit::ReceiveQuestQSSelSentence(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveQuestQSRequestReward(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveQuestCompleteResult(const BYTE *ReceiveBuffer)
{
    ...
    case 2:
        ...
        g_pSystemLogBox->AddText(I18N::Game::YouHaveReachedYourZenLimit,
                                 SEASON3B::TYPE_ERROR_MESSAGE);
        break;
    ...
    case 3:
        if (g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_QUEST_PROGRESS))
            ...
            g_pQuestProgressByEtc->EnableCompleteBtn(false);
        g_pSystemLogBox->AddText(I18N::Game::InventoryIsFull, SEASON3B::TYPE_ERROR_MESSAGE);
        g_pSystemLogBox->AddText(I18N::Game::TheSameItemThatYouWantToTrade,
                                 SEASON3B::TYPE_ERROR_MESSAGE);
        break;
    }
}

void SessionLegacyCalls::ReceiveQuestCompleteResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveQuestCompleteResult(receiveBuffer);
}

void SessionNetworkUnit::ReceiveQuestGiveUp(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPMSG_ANS_QUESTEXP_GIVEUP)ReceiveBuffer;
    g_QuestMng.RemoveCurQuestIndexList(pData->m_dwQuestGiveUpIndex);
}

void SessionLegacyCalls::ReceiveQuestGiveUp(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveQuestGiveUp(receiveBuffer);
}

void SessionNetworkUnit::ReceiveProgressQuestList(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPMSG_ANS_QUESTEXP_PROGRESS_LIST)ReceiveBuffer;
    g_QuestMng.SetCurQuestIndexList(
        (DWORD *)(ReceiveBuffer + sizeof(PMSG_ANS_QUESTEXP_PROGRESS_LIST)),
        int(pData->m_byQuestCount));
}

void SessionLegacyCalls::ReceiveProgressQuestList(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveProgressQuestList(receiveBuffer);
}

void SessionNetworkUnit::ReceiveProgressQuestRequestReward(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPMSG_NPC_QUESTEXP_INFO)ReceiveBuffer;
    g_QuestMng.SetQuestRequestReward(ReceiveBuffer);
    g_QuestMng.SetEPRequestRewardState(pData->m_dwQuestIndex, true);
...
void SessionLegacyCalls::ReceiveProgressQuestRequestReward(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveProgressQuestRequestReward(receiveBuffer);
}

void SessionNetworkUnit::ReceiveProgressQuestListReady(const BYTE *ReceiveBuffer)
{
...
void SessionLegacyCalls::ReceiveProgressQuestListReady(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveProgressQuestListReady(receiveBuffer);
}

void SessionNetworkUnit::ReceiveGensJoining(const BYTE *ReceiveBuffer)
{
...
void SessionLegacyCalls::ReceiveGensJoining(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveGensJoining(receiveBuffer);
}

void SessionNetworkUnit::ReceiveGensSecession(const BYTE *ReceiveBuffer)
{
...
void SessionLegacyCalls::ReceiveGensSecession(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveGensSecession(receiveBuffer);
}

void SessionNetworkUnit::ReceivePlayerGensInfluence(const BYTE *ReceiveBuffer)
{
...
void SessionLegacyCalls::ReceivePlayerGensInfluence(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceivePlayerGensInfluence(receiveBuffer);
}

void SessionNetworkUnit::ReceiveOtherPlayerGensInfluenceViewport(const BYTE *ReceiveBuffer)
{
    ...
    for (i = 0; i < Data->m_byCount; ++i)
    {
        ...
        if (IsStrifeMap(gMapManager.ContextMap()))
        {
            ...
            if (Hero->m_byGensInfluence == c->m_byGensInfluence)
                CreateEffect(BITMAP_OUR_INFLUENCE_GROUND, c->Object.Position, vTemp, vTemp, 0,
                             &c->Object);
            else
                CreateEffect(BITMAP_ENEMY_INFLUENCE_GROUND, c->Object.Position, vTemp, vTemp, 0,
                             &c->Object);
        }
...
void SessionNetworkUnit::SetQuestNpc(int npcIndex)
{
    g_QuestMng.m_nNPCIndex = npcIndex;
    g_QuestMng.m_szNPCName = getMonsterName(npcIndex);
}

void SessionNetworkUnit::ReceiveNPCDlgUIStart(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPMSG_ANS_NPC_CLICK)ReceiveBuffer;
    if (!g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_NPC_DIALOGUE))
    {
        SetQuestNpc(pData->m_wNPCIndex);
...
void SessionNetworkUnit::ReceiveReward(const BYTE *ReceiveBuffer)
{
...
void SessionLegacyCalls::ReceiveReward(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveReward(receiveBuffer);
}
#endif //PBG_ADD_GENSRANKING

void SessionNetworkUnit::ReceiveUseStateItem(const BYTE *ReceiveBuffer)
{
    ...
    case 0x00:
        if (fruit >= 0 && fruit <= 4)
        {
            ...
            mu_swprintf(strText, I18N::Game::SFruitStatDPointsHaveBeenS, I18N::Game::Lookup(index),
                        point, I18N::Game::Create);
            CreateOkMessageBox(strText);
        }
        break;

    case 0x01:
        CreateOkMessageBox(I18N::Game::StatCreationFailedFromFruitCombination);
        break;
    ...
    case 0x03:
        if (fruit >= 0 && fruit <= 4)
        {
            ...
            wchar_t strText[128];
            mu_swprintf(strText, I18N::Game::SFruitStatDPointsHaveBeenS, I18N::Game::Lookup(index),
                        point, I18N::Game::Decrease);
            CreateOkMessageBox(strText);
        }
        break;

    case 0x04:
        CreateOkMessageBox(I18N::Game::FruitDecreaseIsFailed);
        break;
    ...
    case 0x06:
        if (fruit >= 0 && fruit <= 4)
        {
            ...
            mu_swprintf(Text, I18N::Game::SFruitStatDPointsHaveBeenS, I18N::Game::Lookup(index),
                        point, I18N::Game::Decrease);
    ...
    case 0x21:
        CreateOkMessageBox(I18N::Game::ImpossibleSinceTheUsableFruitPointsAreAtMaximum);
        break;
    ...
    case 0x26:
        CreateOkMessageBox(I18N::Game::CannotBeDecreasedUnderTheDefaultStatValue);
        break;
    }
...
void SessionNetworkUnit::ReceivePetCommand(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPRECEIVE_PET_COMMAND)ReceiveBuffer;
    WORD Key = ((WORD)(Data->m_byKeyH) << 8) + Data->m_byKeyL;

    SetPetCommand(Hero, Key, Data->m_byCommand);
}

void SessionLegacyCalls::ReceivePetCommand(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceivePetCommand(receiveBuffer);
}

void SessionNetworkUnit::ReceivePetAttack(const BYTE *ReceiveBuffer)
{
    ...
    SetAttack(sc, Key, Data->m_bySkillType);
}

void SessionLegacyCalls::ReceivePetAttack(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceivePetAttack(receiveBuffer);
}

void SessionNetworkUnit::ReceivePetInfo(const BYTE *ReceiveBuffer)
{
    ...
    SetPetInfo(Data->m_byInvType, Data->m_byPos, &Petinfo);
}

void SessionLegacyCalls::ReceivePetInfo(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceivePetInfo(receiveBuffer);
}

void SessionNetworkUnit::ReceiveWTTimeLeft(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveWTMatchResult(const BYTE *ReceiveBuffer)
{
    ...
    if (Data->m_Type >= 0 && Data->m_Type < 3)
    {
        ...
        CMultiLanguage::ConvertFromUtf8(g_wtMatchResult.m_MatchTeamName1, Data->m_MatchTeamName1,
                                        MAX_USERNAME_SIZE);
        CMultiLanguage::ConvertFromUtf8(g_wtMatchResult.m_MatchTeamName2, Data->m_MatchTeamName2,
                                        MAX_USERNAME_SIZE);
    }
}

void SessionLegacyCalls::ReceiveWTTimeLeft(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveWTTimeLeft(receiveBuffer);
}

void SessionLegacyCalls::ReceiveWTMatchResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveWTMatchResult(receiveBuffer);
}

void SessionNetworkUnit::ReceiveWTBattleSoccerGoalIn(const BYTE *ReceiveBuffer)
{
...
void SessionLegacyCalls::ReceiveWTBattleSoccerGoalIn(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveWTBattleSoccerGoalIn(receiveBuffer);
}

bool SessionNetworkUnit::ReceiveChangeMapServerInfo(const BYTE *ReceiveBuffer)
{
    ...
    csMapServer_.ConnectChangeMapServer(Data->m_vSvrInfo);
    return false;
}

void SessionNetworkUnit::ReceiveChangeMapServerResult(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
...
void SessionNetworkUnit::ReceiveBCStatus(const BYTE *ReceiveBuffer)
{
    ...
    {
    case 0x00:
        g_pSystemLogBox->AddText(I18N::Game::CastleInformationFailed,
                                 SEASON3B::TYPE_SYSTEM_MESSAGE);
        break;
    case 0x01:
    case 0x02:
        ...
        break;
    case 0x03:
        g_pSystemLogBox->AddText(I18N::Game::UnusualCastleInformation,
                                 SEASON3B::TYPE_SYSTEM_MESSAGE);
        break;
    case 0x04:
        g_pSystemLogBox->AddText(I18N::Game::CastleGuildIsDisappeared,
                                 SEASON3B::TYPE_SYSTEM_MESSAGE);
        break;
    }
}

void SessionNetworkUnit::ReceiveBCReg(const BYTE *ReceiveBuffer)
{
    ...
    {
    case 0x00:
        g_pSystemLogBox->AddText(I18N::Game::FailedToRegisterForCastleSiege,
                                 SEASON3B::TYPE_SYSTEM_MESSAGE);
        break;
    case 0x01:
        g_GuardsMan.SetRegStatus(1);
        g_pSystemLogBox->AddText(I18N::Game::CastleSiegeRegistrationIsSuccessful,
                                 SEASON3B::TYPE_SYSTEM_MESSAGE);
        break;
    case 0x02:
        g_pSystemLogBox->AddText(I18N::Game::AlreadyRegisteredInCastleSiege,
                                 SEASON3B::TYPE_SYSTEM_MESSAGE);
        break;
    case 0x03:
        g_pSystemLogBox->AddText(I18N::Game::YouBelongToTheGuildOfTheDefendingTeam,
                                 SEASON3B::TYPE_SYSTEM_MESSAGE);
        break;
    case 0x04:
        ...
        break;
    case 0x05:
        g_pSystemLogBox->AddText(I18N::Game::GuildMasterSLevelIsInsufficient,
                                 SEASON3B::TYPE_SYSTEM_MESSAGE);
        break;
    case 0x06:
        ...
        break;
    case 0x07:
        g_pSystemLogBox->AddText(I18N::Game::ItSNotARegistrationPeriodForCastleSiege,
                                 SEASON3B::TYPE_SYSTEM_MESSAGE);
        break;
    case 0x08:
        g_pSystemLogBox->AddText(I18N::Game::NumberOfGuildMembersIsLacking,
                                 SEASON3B::TYPE_SYSTEM_MESSAGE);
        break;
    default:
...
void SessionLegacyCalls::ReceiveBCReg(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveBCReg(receiveBuffer);
}

void SessionNetworkUnit::ReceiveBCGiveUp(const BYTE *ReceiveBuffer)
{
    ...
    {
    case 0x00:
        g_pSystemLogBox->AddText(I18N::Game::SurrenderingCastleSiegeHasFailed,
                                 SEASON3B::TYPE_SYSTEM_MESSAGE);
        break;
    case 0x01:
        ...
        g_pSystemLogBox->AddText(I18N::Game::SurrenderingCastleSiegeIsSuccessful,
                                 SEASON3B::TYPE_SYSTEM_MESSAGE);
        break;
    case 0x02:
        g_pSystemLogBox->AddText(I18N::Game::ThisGuildIsNotRegisteredInCastleSiege,
                                 SEASON3B::TYPE_SYSTEM_MESSAGE);
        break;
    case 0x03:
        g_pSystemLogBox->AddText(I18N::Game::ItSNotASurrenderingPeriodForCastleSiege,
                                 SEASON3B::TYPE_SYSTEM_MESSAGE);
        break;
    default:
...
void SessionLegacyCalls::ReceiveBCGiveUp(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveBCGiveUp(receiveBuffer);
}
void SessionLegacyCalls::ReceiveBCDeclareGuildList(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveBCDeclareGuildList(receiveBuffer);
}
void SessionLegacyCalls::ReceiveBCGuildList(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveBCGuildList(receiveBuffer);
}
void SessionLegacyCalls::ReceiveGuildCommand(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveGuildCommand(receiveBuffer);
}
void SessionLegacyCalls::ReceiveGuildMemberLocation(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveGuildMemberLocation(receiveBuffer);
}
void SessionLegacyCalls::ReceiveGuildNpcLocation(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveGuildNpcLocation(receiveBuffer);
}
void SessionLegacyCalls::ReceiveMatchTimer(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveMatchTimer(receiveBuffer);
}
void SessionLegacyCalls::ReceiveKanturu3rdTimer(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveKanturu3rdTimer(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCursedTempSkillPoint(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCursedTempSkillPoint(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCursedTempleHolyItemRelics(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCursedTempleHolyItemRelics(receiveBuffer);
}
bool SessionLegacyCalls::ReceiveRegistedLuckyCoin(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveRegistedLuckyCoin(receiveBuffer);
}
bool SessionLegacyCalls::ReceiveEnterDoppelGangerEvent(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveEnterDoppelGangerEvent(receiveBuffer);
}
bool SessionLegacyCalls::ReceiveDoppelGangerMonsterPosition(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveDoppelGangerMonsterPosition(receiveBuffer);
}
bool SessionLegacyCalls::ReceiveDoppelGangerIcewalkerState(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveDoppelGangerIcewalkerState(receiveBuffer);
}
bool SessionLegacyCalls::ReceiveDoppelGangerTimePartyState(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveDoppelGangerTimePartyState(receiveBuffer);
}
bool SessionLegacyCalls::ReceiveDoppelGangerMonsterGoal(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveDoppelGangerMonsterGoal(receiveBuffer);
}
bool SessionLegacyCalls::ReceiveMoveMapChecksum(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveMoveMapChecksum(receiveBuffer);
}

void SessionNetworkUnit::ReceiveBCRegInfo(const BYTE *ReceiveBuffer)
{
...
void SessionLegacyCalls::ReceiveBCRegInfo(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveBCRegInfo(receiveBuffer);
}

void SessionNetworkUnit::ReceiveBCRegMark(const BYTE *ReceiveBuffer)
{
    ...
    {
    case 0x00:
        g_pSystemLogBox->AddText(I18N::Game::RegistrationOfSignHasFailed,
                                 SEASON3B::TYPE_SYSTEM_MESSAGE);
        break;
    case 0x01: {
        DWORD dwMarkCount;
    ...
    break;
    case 0x02:
        g_pSystemLogBox->AddText(I18N::Game::ThisGuildHasNotParticipatedInCastleSiege,
                                 SEASON3B::TYPE_SYSTEM_MESSAGE);
        break;
    case 0x03:
        g_pSystemLogBox->AddText(I18N::Game::IncorrectItemWasRegistered,
                                 SEASON3B::TYPE_SYSTEM_MESSAGE);
        break;
    }
}

void SessionLegacyCalls::ReceiveBCRegMark(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveBCRegMark(receiveBuffer);
}

void SessionNetworkUnit::ReceiveBCNPCBuy(const BYTE *ReceiveBuffer)
{
    ...
    case 2:
        ...
        break;
    case 3:
        g_pSystemLogBox->AddText(I18N::Game::PurchasingCostIsInsufficient,
                                 SEASON3B::TYPE_SYSTEM_MESSAGE);
        break;
    case 4:
...
void SessionNetworkUnit::ReceiveBCNPCRepair(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveBCNPCUpgrade(const BYTE *ReceiveBuffer)
{
    ...
    case 5:
        ...
        break;
    case 6:
        g_pSystemLogBox->AddText(I18N::Game::IncorrectRequestedValue,
                                 SEASON3B::TYPE_SYSTEM_MESSAGE);
        break;
    case 7:
...
void SessionNetworkUnit::ReceiveBCGetTaxInfo(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPMSG_ANS_TAXMONEYINFO)ReceiveBuffer;
    switch (Data->btResult)
    {
    case 0:
        g_pSystemLogBox->AddText(I18N::Game::AcquiringTaxRateInformationHasFailed,
                                 SEASON3B::TYPE_SYSTEM_MESSAGE);
        break;
    case 1:
...
void SessionNetworkUnit::ReceiveBCChangeTaxRate(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPMSG_ANS_TAXRATECHANGE)ReceiveBuffer;
    switch (Data->btResult)
    {
    case 0:
        g_pSystemLogBox->AddText(I18N::Game::ChangingTaxRateInformationHasFailed,
                                 SEASON3B::TYPE_SYSTEM_MESSAGE);
        break;
    case 1:
        if (Data->btTaxType == 3)
...
void SessionNetworkUnit::ReceiveBCWithdraw(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveTaxInfo(const BYTE *ReceiveBuffer)
{
...
void SessionLegacyCalls::ReceiveTaxInfo(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveTaxInfo(receiveBuffer);
}

void SessionNetworkUnit::ReceiveHuntZoneEnter(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveBCNPCList(const BYTE *ReceiveBuffer)
{
    ...
    {
    case 0:
        g_pSystemLogBox->AddText(I18N::Game::UnfortunatelyYouHaveFailed,
                                 SEASON3B::TYPE_SYSTEM_MESSAGE);
        break;
    case 1: {
...
void SessionNetworkUnit::ReceiveBCDeclareGuildList(const BYTE *ReceiveBuffer)
{
    ...
    case 1: {
        ...
        for (int i = 0; i < Data->iCount; ++i)
        {
            ...
            *pMarkCount++ = pData2->btRegMarks1;
            ...
            wchar_t guildName[MAX_GUILDNAME + 1]{};
            CMultiLanguage::ConvertFromUtf8(guildName, pData2->szGuildName, MAX_GUILDNAME);
            g_pGuardWindow->AddDeclareGuildList(guildName, dwMarkCount, pData2->btIsGiveUp,
                                                pData2->btSeqNum);
...
void SessionNetworkUnit::ReceiveBCGuildList(const BYTE *ReceiveBuffer)
{
    ...
    case 1: {
        ...
        for (int i = 0; i < Data->iCount; ++i)
        {
            ...
            g_pGuardWindow->AddGuildList(guildName, pData2->btCsJoinSide, pData2->btGuildInvolved,
                                         pData2->iGuildScore);
...
void SessionNetworkUnit::ReceiveGateState(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveGateOperator(const BYTE *ReceiveBuffer)
{
    auto Data = (LPPRECEIVE_GATE_OPERATOR)ReceiveBuffer;
    int Key = ((int)(Data->m_byKeyH) << 8) + Data->m_byKeyL;

    ProcessState(Key, Data->m_byOperator, Data->m_byResult);
}

void SessionNetworkUnit::ReceiveGateCurrentState(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveCrownSwitchState(const BYTE *ReceiveBuffer)
{
    ...
    case 0: {
        ...
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CCrownSwitchPopLayout, sessionKeeper_));
    }
    break;

    case 1:
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CCrownSwitchPushLayout, sessionKeeper_));
        break;
    ...
    case 2: {
        ...
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CCrownSwitchOtherPushLayout, sessionKeeper_), &pMsgBox);
        if (pMsgBox)
        {
...
int SessionNetworkUnit::DenyCrownRegistPopupClose(POPUP_RESULT Result)
{
...
int SessionLegacyCalls::DenyCrownRegistPopupClose(POPUP_RESULT result)
{
    return sessionKeeper_.Network()->DenyCrownRegistPopupClose(result);
}

void SessionLegacyCalls::ReceiveMapInfoResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveMapInfoResult(receiveBuffer);
}

void SessionLegacyCalls::RecevieRaklionBattleResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->RecevieRaklionBattleResult(receiveBuffer);
}

void SessionLegacyCalls::RecevieRaklionWideAreaAttack(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->RecevieRaklionWideAreaAttack(receiveBuffer);
}

void SessionLegacyCalls::RecevieRaklionUserMonsterCount(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->RecevieRaklionUserMonsterCount(receiveBuffer);
}

bool SessionLegacyCalls::ReceiveRequestMoveMap(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveRequestMoveMap(receiveBuffer);
}

bool SessionLegacyCalls::ReceiveIGS_SendCashGift(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveIGS_SendCashGift(receiveBuffer);
}

bool SessionLegacyCalls::ReceiveIGS_PossibleBuy(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveIGS_PossibleBuy(receiveBuffer);
}

bool SessionLegacyCalls::ReceiveIGS_LeftCountItem(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveIGS_LeftCountItem(receiveBuffer);
}

bool SessionLegacyCalls::ReceivePeriodItemListCount(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceivePeriodItemListCount(receiveBuffer);
}

void SessionNetworkUnit::ReceiveCrownRegist(const BYTE *ReceiveBuffer)
{
    ...
    case 0: {
        SEASON3B::CProgressMsgBox *pMsgBox = nullptr;
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CSealRegisterStartLayout, sessionKeeper_), &pMsgBox);
        if (pMsgBox)
        {
            wchar_t strText[256];
            ...
            if (iTime >= 59)
                iTime = 59;
            mu_swprintf(strText, I18N::Game::SAccumulatedHourDseconds,
                        I18N::Game::OfficialSealRegistrationWillStart, iTime);
            pMsgBox->AddMsg(strText);
    ...
    case 1:
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CSealRegisterSuccessLayout, sessionKeeper_));
        break;

    case 2: {
        SEASON3B::CProgressMsgBox *pMsgBox = nullptr;
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CSealRegisterFailLayout, sessionKeeper_), &pMsgBox);
        if (pMsgBox)
        {
            wchar_t strText[256];
            int iTime = (pData->m_dwCrownAccessTime / 1000);
            if (iTime >= 59)
                iTime = 59;
            mu_swprintf(strText, I18N::Game::SAccumulatedHourDseconds,
                        I18N::Game::OfficialSealRegistrationIsFailed, iTime);
            pMsgBox->AddMsg(strText);
        }
    }
    break;
    ...
    case 3: {
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CSealRegisterOtherLayout, sessionKeeper_));
    }
    break;
    case 4:
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CSealRegisterOtherCampLayout, sessionKeeper_));
        break;
    }
}

void SessionNetworkUnit::ReceiveCrownState(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPRECEIVE_CROWN_STATE)ReceiveBuffer;
    ...
    case 0: {
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CCrownDefenseRemoveLayout, sessionKeeper_));
    ...
    case 1: {
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CCrownDefenseCreateLayout, sessionKeeper_));
...
void SessionNetworkUnit::ReceiveBattleCastleRegiment(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPRECEIVE_CROWN_STATE)ReceiveBuffer;
    ...
    {
    case 0:
        SettingBattleFormation(Hero, static_cast<eBuffState>(0));
        return;
    case 1:
        SettingBattleFormation(Hero, eBuff_CastleRegimentDefense);
        break;
    case 2:
        SettingBattleFormation(Hero, eBuff_CastleRegimentAttack1);
        break;
    case 3:
        SettingBattleFormation(Hero, eBuff_CastleRegimentAttack2);
        break;
    case 4:
        SettingBattleFormation(Hero, eBuff_CastleRegimentAttack3);
        break;
    }
}

void SessionNetworkUnit::ReceiveBattleCasleSwitchInfo(const BYTE *ReceiveBuffer)
{
    auto pData = (LPRECEIVE_CROWN_SWITCH_INFO)ReceiveBuffer;
    Check_Switch(pData);
}

bool SessionNetworkUnit::Check_Switch(PRECEIVE_CROWN_SWITCH_INFO *Data)
{
    if (Switch_Info == nullptr)
    {
    ...
    else
    {
        ...
        if (Key == FIRST_CROWN_SWITCH_NUMBER)
        {
            Switch_Info[0].m_bySwitchState = Data->m_bySwitchState;
            Switch_Info[0].m_JoinSide = Data->m_JoinSide;
            CMultiLanguage::ConvertFromUtf8(Switch_Info[0].m_szGuildName, Data->m_szGuildName,
                                            MAX_GUILDNAME);
            CMultiLanguage::ConvertFromUtf8(Switch_Info[0].m_szUserName, Data->m_szUserName,
                                            MAX_USERNAME_SIZE);
        }
        else
        {
            Switch_Info[1].m_bySwitchState = Data->m_bySwitchState;
            Switch_Info[1].m_JoinSide = Data->m_JoinSide;
            CMultiLanguage::ConvertFromUtf8(Switch_Info[1].m_szGuildName, Data->m_szGuildName,
                                            MAX_GUILDNAME);
            CMultiLanguage::ConvertFromUtf8(Switch_Info[1].m_szUserName, Data->m_szUserName,
                                            MAX_USERNAME_SIZE);
        }
    }
    return true;
...
bool SessionNetworkUnit::Delete_Switch()
{
    if (Switch_Info != nullptr)
    {
        delete[] Switch_Info;
...
void SessionLegacyCalls::ReceiveBCStatus(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveBCStatus(receiveBuffer);
}
void SessionLegacyCalls::ReceiveBCNPCBuy(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveBCNPCBuy(receiveBuffer);
}
void SessionLegacyCalls::ReceiveBCNPCRepair(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveBCNPCRepair(receiveBuffer);
}
void SessionLegacyCalls::ReceiveBCNPCUpgrade(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveBCNPCUpgrade(receiveBuffer);
}
void SessionLegacyCalls::ReceiveBCGetTaxInfo(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveBCGetTaxInfo(receiveBuffer);
}
void SessionLegacyCalls::ReceiveBCChangeTaxRate(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveBCChangeTaxRate(receiveBuffer);
}
void SessionLegacyCalls::ReceiveBCWithdraw(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveBCWithdraw(receiveBuffer);
}
void SessionLegacyCalls::ReceiveHuntZoneEnter(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveHuntZoneEnter(receiveBuffer);
}
void SessionLegacyCalls::ReceiveBCNPCList(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveBCNPCList(receiveBuffer);
}
void SessionLegacyCalls::ReceiveBattleCasleSwitchInfo(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveBattleCasleSwitchInfo(receiveBuffer);
}
bool SessionLegacyCalls::Check_Switch(PRECEIVE_CROWN_SWITCH_INFO *data)
{
    return sessionKeeper_.Network()->Check_Switch(data);
}
bool SessionLegacyCalls::Delete_Switch()
{
    return sessionKeeper_.Network()->Delete_Switch();
}

void SessionNetworkUnit::ReceiveBattleCastleStart(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPRECEIVE_CROWN_STATE)ReceiveBuffer;

    auto bStartBattleCastle = (bool)(pData->m_byCrownState);

    SetBattleCastleStart(bStartBattleCastle);
...
void SessionNetworkUnit::ReceiveBattleCastleProcess(const BYTE *ReceiveBuffer)
{
    ...
    case 1: {
        ...
        notices_.Create(Text2, 1);
    }
    break;
    }
}

void SessionNetworkUnit::ReceiveKillCount(const BYTE *ReceiveBuffer)
{
...
void SessionLegacyCalls::ReceiveKillCount(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveKillCount(receiveBuffer);
}

void SessionNetworkUnit::ReceiveBuildTime(const BYTE *ReceiveBuffer)
{
...
void SessionLegacyCalls::ReceiveBuildTime(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveBuildTime(receiveBuffer);
}

void SessionNetworkUnit::ReceiveCastleGuildMark(const BYTE *ReceiveBuffer)
{
...
void SessionLegacyCalls::ReceiveCastleGuildMark(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCastleGuildMark(receiveBuffer);
}

void SessionNetworkUnit::ReceiveCastleHuntZoneInfo(const BYTE *ReceiveBuffer)
{
    ...
    {
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CGatemanFailMsgBoxLayout, sessionKeeper_));
    }
    else
    {
        g_pUIGateKeeper->SetInfo(pData->m_byResult, (bool)pData->m_byEnable, pData->m_iCurrPrice,
                                 pData->m_iUnitPrice, pData->m_iMaxPrice);
        g_pNewUISystem->Show(SEASON3B::INTERFACE_GATEKEEPER);
    }
}

void SessionNetworkUnit::ReceiveCastleHuntZoneResult(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveCatapultState(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveCatapultFire(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveCatapultFireToMe(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPRECEIVE_BOMBING_ALERT)ReceiveBuffer;

    g_pCatapultWindow->DoFireFixStartPosition(pData->m_byWeaponType, pData->m_byTargetX,
                                              pData->m_byTargetY);
}

void SessionLegacyCalls::ReceiveCatapultState(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCatapultState(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCatapultFire(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCatapultFire(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCatapultFireToMe(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCatapultFireToMe(receiveBuffer);
}

void SessionNetworkUnit::ReceivePreviewPort(std::span<const BYTE> ReceiveBuffer)
{
    ...
    for (int i = 0; i < pData->Value; i++)
    {
        ...
        case 1: {
            CHARACTER *c =
                CreateCharacter(Key, MODEL_PLAYER, pData2->m_byPosX, pData2->m_byPosY, 0);
            OBJECT *o = &c->Object;

            c->Class = gCharacterManager.ChangeServerClassTypeToClientClassType(
                (SERVER_CLASS_TYPE)pData2->m_byTypeH);
            c->SkinIndex = gCharacterManager.GetSkinModelIndex(c->Class);
            c->Skin = 0;
            c->PK = 0;
            o->Kind = KIND_TMP;
        ...
        case 3: //   NPC
        {
            ...
            if (gMapManager.InBattleCastle() && c->MonsterIndex == MONSTER_CASTLE_GATE1)
            {
                ...
                {
                    SetAction(&c->Object, 1);
                    SetCastleGate_Attribute(pData2->m_byPosX, pData2->m_byPosY, 0);
                }
                else if (g_isNotCharacterBuff((&c->Object)))
                {
                    SetAction(&c->Object, 0);
                    SetCastleGate_Attribute(pData2->m_byPosX, pData2->m_byPosY, 1);
                }
            }
        }
        break;
        ...
        Offset += (sizeof(PRECEIVE_PREVIEW_PORT_EXTENDED) -
                   (sizeof(BYTE) * (MAX_BUFF_SLOT_INDEX - pData2->s_BuffCount)));
    }
}

void SessionNetworkUnit::ReceiveMapInfoResult(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPRECEIVE_MAP_INFO_RESULT)ReceiveBuffer;
}

void SessionNetworkUnit::ReceiveGuildCommand(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveGuildMemberLocation(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveGuildNpcLocation(const BYTE *ReceiveBuffer)
{
    if (g_pSiegeWarfare->GetCurSiegeWarType() != TYPE_GUILD_COMMANDER)
        return;
...
void SessionNetworkUnit::ReceiveMatchTimer(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveCrywolfInfo(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPMSG_ANS_CRYWOLF_INFO)ReceiveBuffer;

    TheMapProcess().Crywolf1st().CheckCryWolf1stMVP(pData->btOccupationState,
                                                    pData->btCrywolfState);
    g_Direction.m_CMVP.GetCryWolfState(pData->btCrywolfState);
}

void SessionNetworkUnit::ReceiveCrywolStateAltarfInfo(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPMSG_ANS_CRYWOLF_STATE_ALTAR_INFO)ReceiveBuffer;

    TheMapProcess().Crywolf1st().CheckCryWolf1stMVPAltarfInfo(
        pData->iCrywolfStatueHP, pData->btAltarState1, pData->btAltarState2, pData->btAltarState3,
        pData->btAltarState4, pData->btAltarState5);
}

void SessionNetworkUnit::ReceiveCrywolfAltarContract(const BYTE *ReceiveBuffer)
{
    ...
    if (pData->bResult == 0)
    {
        ...
        {
            SEASON3B::CreateMessageBox(
                MSGBOX_LAYOUT_CLASS(SEASON3B::CCry_Wolf_Dont_Set_Temple1, sessionKeeper_));
        }
        else
        {
            SEASON3B::CreateMessageBox(
                MSGBOX_LAYOUT_CLASS(SEASON3B::CCry_Wolf_Wat_Set_Temple1, sessionKeeper_));
        }
    }
    else if (pData->bResult == 1)
    {
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CCry_Wolf_Set_Temple, sessionKeeper_));

        TheMapProcess().Crywolf1st().Check_AltarState(Key - 316, pData->btAltarState);
...
void SessionNetworkUnit::ReceiveCrywolfLifeTime(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPPMSG_ANS_CRYWOLF_LEFTTIME)ReceiveBuffer;

    TheMapProcess().Crywolf1st().SetTime(pData->btHour, pData->btMinute);
    g_pCryWolfInterface->SetTime((int)(pData->btHour), (int)(pData->btMinute));
}

void SessionNetworkUnit::ReceiveCrywolfTankerHit(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPMSG_NOTIFY_REGION_MONSTER_ATTACK)ReceiveBuffer;

    TheMapProcess().Crywolf1st().DoTankerFireFixStartPosition(pData->btSourceX, pData->btSourceY,
                                                              pData->btPointX, pData->btPointY);
}

void SessionNetworkUnit::ReceiveCrywolfBenefitPlusChaosRate(const BYTE *ReceiveBuffer)
{
...
void SessionLegacyCalls::ReceiveCrywolfBenefitPlusChaosRate(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCrywolfBenefitPlusChaosRate(receiveBuffer);
}

void SessionNetworkUnit::ReceiveCrywolfBossMonsterInfo(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPMSG_ANS_CRYWOLF_BOSSMONSTER_INFO)ReceiveBuffer;

    TheMapProcess().Crywolf1st().Set_BossMonster(pData->btBossHP, pData->btMonster2);
}

void SessionNetworkUnit::ReceiveCrywolfPersonalRank(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPMSG_ANS_CRYWOLF_PERSONAL_RANK)ReceiveBuffer;

    TheMapProcess().Crywolf1st().Set_MyRank(pData->btRank, pData->iGettingExp);
}

void SessionNetworkUnit::ReceiveCrywolfHeroList(const BYTE *ReceiveBuffer)
{
    ...
    for (int i = 0; i < pData->btCount; i++)
    {
        ...
        auto heroClass =
            gCharacterManager.ChangeServerClassTypeToClientClassType(pData2->btHeroClass);
        TheMapProcess().Crywolf1st().Set_WorldRank(pData2->iRank, heroClass, pData2->iHeroScore,
                                                   playerName);
    }
}

void SessionNetworkUnit::ReceiveKanturu3rdStateInfo(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPMSG_ANS_KANTURU_STATE_INFO)ReceiveBuffer;

    g_pKanturu2ndEnterNpc->ReceiveKanturu3rdInfo(pData->btState, pData->btDetailState,
                                                 pData->btEnter, pData->btUserCount,
                                                 pData->iRemainTime);
}

void SessionNetworkUnit::ReceiveKanturu3rdEnterBossMap(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPMSG_ANS_ENTER_KANTURU_BOSS_MAP)ReceiveBuffer;
    g_pKanturu2ndEnterNpc->ReceiveKanturu3rdEnter(pData->btResult);
}

void SessionLegacyCalls::ReceiveKanturu3rdStateInfo(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveKanturu3rdStateInfo(receiveBuffer);
}
void SessionLegacyCalls::ReceiveKanturu3rdEnterBossMap(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveKanturu3rdEnterBossMap(receiveBuffer);
}

void SessionNetworkUnit::ReceiveKanturu3rdCurrentState(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPMSG_ANS_KANTURU_CURRENT_STATE)ReceiveBuffer;
    Kanturu3rdState(pData->btCurrentState, pData->btCurrentDetailState);
    CheckSuccessBattle(pData->btCurrentState, pData->btCurrentDetailState);
}

void SessionNetworkUnit::ReceiveKanturu3rdState(const BYTE *ReceiveBuffer)
{
    ...
    if (IsInKanturu3rd())
    {
        ...
        Kanturu3rdState(pData->btState, pData->btDetailState);
        CheckSuccessBattle(pData->btState, pData->btDetailState);
    }
}

void SessionNetworkUnit::ReceiveKanturu3rdResult(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPMSG_ANS_KANTURU_BATTLE_RESULT)ReceiveBuffer;
    Kanturu3rdResult(pData->btResult);
}

void SessionNetworkUnit::ReceiveKanturu3rdTimer(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPMSG_ANS_KANTURU_BATTLE_SCENE_TIMELIMIT)ReceiveBuffer;
    g_pKanturuInfoWindow->SetTime(pData->btTimeLimit);
}

void SessionNetworkUnit::RecevieKanturu3rdMayaSKill(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPMSG_NOTIFY_KANTURU_WIDE_AREA_ATTACK)ReceiveBuffer;
    MayaSceneMayaAction(pData->btType);
}

void SessionNetworkUnit::RecevieKanturu3rdLeftUserandMonsterCount(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPMSG_NOTIFY_KANTURU_USER_MONSTER_COUNT)ReceiveBuffer;
    Kanturu3rdUserandMonsterCount(pData->bMonsterCount, pData->btUserCount);
}

void SessionNetworkUnit::ReceiveCursedTempleEnterInfo(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::ReceiveCursedTempleEnterResult(const BYTE *ReceiveBuffer)
{
    ...
    cursedTemple_.UpdateTempleSystemMsg(data->Result);
}

void SessionNetworkUnit::ReceiveCursedTempleInfo(const BYTE *ReceiveBuffer)
{
    cursedTemple_.ReceiveCursedTempleInfo(ReceiveBuffer);
    if (g_pNewUISystem && g_pCursedTempleWindow)
        g_pCursedTempleWindow->ReceiveCursedTempleInfo(ReceiveBuffer);
}

void SessionNetworkUnit::ReceiveCursedTempMagicResult(const BYTE *ReceiveBuffer)
{
    sessionKeeper_.Gameplay()->ReceiveCursedTempRegisterSkill(ReceiveBuffer);
}

void SessionNetworkUnit::ReceiveCursedTempSkillEnd(const BYTE *ReceiveBuffer)
{
    sessionKeeper_.Gameplay()->ReceiveCursedTempUnRegisterSkill(ReceiveBuffer);
}

void SessionLegacyCalls::ReceiveCursedTempleEnterInfo(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCursedTempleEnterInfo(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCursedTempMagicResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCursedTempMagicResult(receiveBuffer);
}
void SessionLegacyCalls::ReceiveCursedTempSkillEnd(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCursedTempSkillEnd(receiveBuffer);
}

void SessionNetworkUnit::ReceiveCursedTempSkillPoint(const BYTE *ReceiveBuffer)
{
    cursedTemple_.SetSkillPoints(
        reinterpret_cast<const PMSG_CURSED_TEMPLE_SKILL_POINT *>(ReceiveBuffer)->btSkillPoint);
    if (g_pNewUISystem && g_pCursedTempleWindow)
        g_pCursedTempleWindow->ReceiveCursedTempSkillPoint(ReceiveBuffer);
}

void SessionNetworkUnit::ReceiveCursedTempleHolyItemRelics(const BYTE *ReceiveBuffer)
{
    g_pCursedTempleWindow->ReceiveCursedTempleHolyItemRelics(ReceiveBuffer);
}

void SessionNetworkUnit::ReceiveCursedTempleGameResult(const BYTE *ReceiveBuffer)
{
    g_pNewUISystem->HideAll();
...
void SessionLegacyCalls::ReceiveCursedTempleGameResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCursedTempleGameResult(receiveBuffer);
}

void SessionNetworkUnit::ReceiveCursedTempleState(const BYTE *ReceiveBuffer)
{
    ...
    cursedTemple_.ReceiveCursedTempleState(cursedtemple);
}

void SessionNetworkUnit::ReceiveRaklionStateInfo(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPMSG_ANS_RAKLION_STATE_INFO)ReceiveBuffer;
    g_Raklion.SetState(pData->btState, pData->btDetailState);
}

void SessionNetworkUnit::ReceiveRaklionCurrentState(const BYTE *ReceiveBuffer)
{
...
void SessionNetworkUnit::RecevieRaklionStateChange(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPMSG_ANS_RAKLION_STATE_CHANGE)ReceiveBuffer;
    g_Raklion.SetState(pData->btState, pData->btDetailState);
}

void SessionLegacyCalls::ReceiveRaklionStateInfo(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveRaklionStateInfo(receiveBuffer);
}
void SessionLegacyCalls::ReceiveRaklionCurrentState(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveRaklionCurrentState(receiveBuffer);
}
void SessionLegacyCalls::RecevieRaklionStateChange(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->RecevieRaklionStateChange(receiveBuffer);
}

void SessionNetworkUnit::RecevieRaklionBattleResult(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPMSG_ANS_RAKLION_BATTLE_RESULT)ReceiveBuffer;
}

void SessionNetworkUnit::RecevieRaklionWideAreaAttack(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPMSG_NOTIFY_RAKLION_WIDE_AREA_ATTACK)ReceiveBuffer;
}

void SessionNetworkUnit::RecevieRaklionUserMonsterCount(const BYTE *ReceiveBuffer)
{
    auto pData = (LPPMSG_NOTIFY_RAKLION_USER_MONSTER_COUNT)ReceiveBuffer;
}

void SessionNetworkUnit::ReceiveCheckSumRequest(const BYTE *ReceiveBuffer)
{
...
bool SessionNetworkUnit::ReceiveRegistedLuckyCoin(const BYTE *ReceiveBuffer)
{
...
bool SessionNetworkUnit::ReceiveRegistLuckyCoin(const BYTE *ReceiveBuffer)
{
    ...
    case 0: {
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CUseRegistLuckyCoinMsgBoxLayout, sessionKeeper_));
    }
    break;
    case 1: {
    ...
    case 100: {
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CRegistOverLuckyCoinMsgBoxLayout, sessionKeeper_));
    }
    break;
    default:
        return false;
...
bool SessionNetworkUnit::ReceiveRequestExChangeLuckyCoin(const BYTE *ReceiveBuffer)
{
    ...
    case 0: {
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CExchangeLuckyCoinMsgBoxLayout, sessionKeeper_));
    }
    break;
    case 1: {
    ...
    case 2: {
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CExchangeLuckyCoinInvenErrMsgBoxLayout, sessionKeeper_));
    }
    break;
    default:
        return false;
...
bool SessionNetworkUnit::ReceiveEnterDoppelGangerEvent(const BYTE *ReceiveBuffer)
{
...
bool SessionNetworkUnit::ReceiveDoppelGangerMonsterPosition(const BYTE *ReceiveBuffer)
{
...
bool SessionNetworkUnit::ReceiveDoppelGangerState(const BYTE *ReceiveBuffer)
{
    ...
    case 2: // ready->play
    {
        ...
        SEASON3B::CNewUICommonMessageBox *pMsgBox;
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CDoppelGangerMsgBoxLayout, sessionKeeper_), &pMsgBox);
        ...
        pMsgBox->AddMsg(L" ");
        pMsgBox->AddMsg(I18N::Game::WillResultInDoppelgangerDefenseFailure,
                        RGBA(255, 255, 255, 255), SEASON3B::MSGBOX_FONT_NORMAL);
    }
    break;
    case 3: // play->end
...
bool SessionNetworkUnit::ReceiveDoppelGangerIcewalkerState(const BYTE *ReceiveBuffer)
{
...
bool SessionNetworkUnit::ReceiveDoppelGangerTimePartyState(const BYTE *ReceiveBuffer)
{
    ...
    {
        g_pDoppelGangerFrame->SetPartyMemberInfo(pUserPos[i].wUserIndex,
                                                 (float)(22 - pUserPos[i].btPosIndex) / 22.0f);
    }

    return true;
}

bool SessionNetworkUnit::ReceiveDoppelGangerResult(const BYTE *ReceiveBuffer)
{
    ...
    case 0: {
        ...
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CDoppelGangerMsgBoxLayout, sessionKeeper_), &pMsgBox);
        pMsgBox->AddMsg(I18N::Game::Congratulations, RGBA(255, 255, 255, 255),
                        SEASON3B::MSGBOX_FONT_NORMAL);
        pMsgBox->AddMsg(L" ");
    ...
    case 1: {
        ...
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CDoppelGangerMsgBoxLayout, sessionKeeper_), &pMsgBox);
        pMsgBox->AddMsg(I18N::Game::DoppelgangerDefenseFailed, RGBA(255, 255, 255, 255),
                        SEASON3B::MSGBOX_FONT_NORMAL);
    }
    break;
    case 2: {
...
bool SessionNetworkUnit::ReceiveDoppelGangerMonsterGoal(const BYTE *ReceiveBuffer)
{
...
bool SessionNetworkUnit::ReceiveMoveMapChecksum(const BYTE *ReceiveBuffer)
{
...
bool SessionNetworkUnit::ReceiveRequestMoveMap(const BYTE *ReceiveBuffer)
{
...
bool SessionNetworkUnit::ReceiveEnterEmpireGuardianEvent(const BYTE *ReceiveBuffer)
{
    ...
    case 1: {
        SEASON3B::CNewUICommonMessageBox *pMsgBox;
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CEmpireGuardianMsgBoxLayout, sessionKeeper_), &pMsgBox);
        pMsgBox->AddMsg(I18N::Game::EntryTime2798, RGBA(255, 255, 255, 255),
                        SEASON3B::MSGBOX_FONT_NORMAL);
        pMsgBox->AddMsg(L" ");
        wchar_t szText[256] = {};
    ...
    case 2: {
        ...
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CEmpireGuardianMsgBoxLayout, sessionKeeper_), &pMsgBox);
        pMsgBox->AddMsg(I18N::Game::QuestItemMissing, RGBA(255, 255, 255, 255),
                        SEASON3B::MSGBOX_FONT_NORMAL);
    }
    break;
    case 3: {
        ...
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CEmpireGuardianMsgBoxLayout, sessionKeeper_), &pMsgBox);
        pMsgBox->AddMsg(I18N::Game::CapacityExceeded, RGBA(255, 255, 255, 255),
                        SEASON3B::MSGBOX_FONT_NORMAL);
    }
    break;
    case 4: {
...
bool SessionLegacyCalls::ReceiveEnterEmpireGuardianEvent(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveEnterEmpireGuardianEvent(receiveBuffer);
}

bool SessionNetworkUnit::ReceiveRemainTickEmpireGuardian(const BYTE *ReceiveBuffer)
{
...
bool SessionLegacyCalls::ReceiveRemainTickEmpireGuardian(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveRemainTickEmpireGuardian(receiveBuffer);
}

bool SessionNetworkUnit::ReceiveResultEmpireGuardian(const BYTE *ReceiveBuffer)
{
    ...
    case 0: {
        ...
        SEASON3B::CreateMessageBox(
            MSGBOX_LAYOUT_CLASS(SEASON3B::CEmpireGuardianMsgBoxLayout, sessionKeeper_), &pMsgBox);
        pMsgBox->AddMsg(I18N::Game::YouHaveFailedToConquerThe, RGBA(255, 255, 255, 255),
                        SEASON3B::MSGBOX_FONT_NORMAL);
        pMsgBox->AddMsg(I18N::Game::FortressOfEmpireGuardians, RGBA(255, 255, 255, 255),
                        SEASON3B::MSGBOX_FONT_NORMAL);
    }
    break;
    case 1: {
    ...
    case 2: {
        ...
        mu_swprintf(szText, I18N::Game::FortressOfEmpireGuardiansRoundD, day);
        pMsgBox->AddMsg(szText, RGBA(255, 255, 255, 255), SEASON3B::MSGBOX_FONT_NORMAL);
        pMsgBox->AddMsg(I18N::Game::HasBeenCleared, RGBA(255, 255, 255, 255),
                        SEASON3B::MSGBOX_FONT_NORMAL);
...
// (0xD2)(0x01)
bool SessionNetworkUnit::ReceiveIGS_CashPoint(const BYTE *pReceiveBuffer)
{
...
// (0xD2)(0x02)
bool SessionNetworkUnit::ReceiveIGS_ShopOpenResult(const BYTE *pReceiveBuffer)
{
...
// (0xD2)(0x03)
bool SessionNetworkUnit::ReceiveIGS_BuyItem(const BYTE *pReceiveBuffer)
{
    ...
    case 1: {
        ...
        pMsgBox->Initialize(I18N::Game::PurchaseFailed,
                            I18N::Game::YouDoNotHaveEnoughWCoinOrPoints);
    }
    break;
    case 2: {
        CMsgBoxIGSCommon *pMsgBox = nullptr;
        CreateMessageBox(MSGBOX_LAYOUT_CLASS(CMsgBoxIGSCommonLayout, sessionKeeper_), &pMsgBox);
        pMsgBox->Initialize(I18N::Game::PurchaseFailed,
                            I18N::Game::YouDoNotHaveEnoughSpaceInStorage);
    }
    break;
    case 3: {
        CMsgBoxIGSCommon *pMsgBox = nullptr;
    ...
    case 4: {
        ...
        CreateMessageBox(MSGBOX_LAYOUT_CLASS(CMsgBoxIGSCommonLayout, sessionKeeper_), &pMsgBox);
        pMsgBox->Initialize(I18N::Game::PurchaseFailed,
                            I18N::Game::ThisItemIsNotCurrentlyAvailable);
    }
    break;
    case 5: {
        CMsgBoxIGSCommon *pMsgBox = nullptr;
...
// (0xD2)(0x04)
bool SessionNetworkUnit::ReceiveIGS_SendItemGift(const BYTE *pReceiveBuffer)
{
    ...
    case 5: {
        ...
        pMsgBox->Initialize(I18N::Game::GiftDeliveryFailed,
                            I18N::Game::ThisItemIsNoLongerAvailable);
    }
    break;
    case 6: {
        CMsgBoxIGSCommon *pMsgBox = nullptr;
    ...
    case 7: {
        ...
        CreateMessageBox(MSGBOX_LAYOUT_CLASS(CMsgBoxIGSCommonLayout, sessionKeeper_), &pMsgBox);
        pMsgBox->Initialize(I18N::Game::GiftDeliveryFailed,
                            I18N::Game::ThisItemCannotBeSentAsAGift);
    }
    break;
    case 8: {
        CMsgBoxIGSCommon *pMsgBox = nullptr;
        CreateMessageBox(MSGBOX_LAYOUT_CLASS(CMsgBoxIGSCommonLayout, sessionKeeper_), &pMsgBox);
        pMsgBox->Initialize(I18N::Game::GiftDeliveryFailed,
                            I18N::Game::ThisEventItemCannotBeSentAsAGift);
    }
    break;
    case 9: {
        CMsgBoxIGSCommon *pMsgBox = nullptr;
        CreateMessageBox(MSGBOX_LAYOUT_CLASS(CMsgBoxIGSCommonLayout, sessionKeeper_), &pMsgBox);
        pMsgBox->Initialize(I18N::Game::GiftDeliveryFailed,
                            I18N::Game::YouVeExceededTheNumberOfEventItemGiftsAllowed);
    }
    break;
    case 10: {
        CMsgBoxIGSCommon *pMsgBox = nullptr;
...
// (0xD2)(0x06)
bool SessionNetworkUnit::ReceiveIGS_StorageItemListCount(const BYTE *pReceiveBuffer)
{
    auto Data = (LPPMSG_CASHSHOP_STORAGECOUNT)pReceiveBuffer;
    g_pInGameShop->InitStorage((int)Data->wTotalItemCount, (int)Data->wCurrentItemCount,
                               (int)Data->wTotalPage, (int)Data->wPageIndex);
    return true;
}

// (0xD2)(0x0D)
bool SessionNetworkUnit::ReceiveIGS_StorageItemList(const BYTE *pReceiveBuffer)
{
    ...
    g_pInGameShop->AddStorageItem((int)Data->lStorageIndex, (int)Data->lItemSeq,
                                  (int)Data->lStorageGroupCode, (int)Data->lProductSeq,
                                  (int)Data->lPriceSeq, (int)Data->dCashPoint,
                                  (char)Data->chItemType);
    return true;
}

// (0xD2)(0x0E)
bool SessionNetworkUnit::ReceiveIGS_StorageGiftItemList(const BYTE *pReceiveBuffer)
{
...
// (0xD2)(0x07)
bool SessionNetworkUnit::ReceiveIGS_SendCashGift(const BYTE *pReceiveBuffer)
{
...
// (0xD2)(0x08)
bool SessionNetworkUnit::ReceiveIGS_PossibleBuy(const BYTE *pReceiveBuffer)
{
...
// (0xD2)(0x09)
bool SessionNetworkUnit::ReceiveIGS_LeftCountItem(const BYTE *pReceiveBuffer)
{
...
// (0xD2)(0x0B)
bool SessionNetworkUnit::ReceiveIGS_UseStorageItem(const BYTE *pReceiveBuffer)
{
    ...
    case 2: {
        CMsgBoxIGSCommon *pMsgBox = nullptr;
        CreateMessageBox(MSGBOX_LAYOUT_CLASS(CMsgBoxIGSCommonLayout, sessionKeeper_), &pMsgBox);
        pMsgBox->Initialize(I18N::Game::FailedToUse,
                            I18N::Game::YouCanReceiveThisItemOnlyFromAPCCafe);
    }
    break;
    case 3: {
        CMsgBoxIGSCommon *pMsgBox = nullptr;
        CreateMessageBox(MSGBOX_LAYOUT_CLASS(CMsgBoxIGSCommonLayout, sessionKeeper_), &pMsgBox);
        pMsgBox->Initialize(I18N::Game::FailedToUse,
                            I18N::Game::AnActiveColorPlanExistsInTheSelectedPeriod);
    }
    break;
    case 4: {
        CMsgBoxIGSCommon *pMsgBox = nullptr;
        CreateMessageBox(MSGBOX_LAYOUT_CLASS(CMsgBoxIGSCommonLayout, sessionKeeper_), &pMsgBox);
        pMsgBox->Initialize(I18N::Game::FailedToUse,
                            I18N::Game::AnActivePersonalFixedPlanExistsInTheSelectedPeriod);
    }
    break;
    case 21: {
        CMsgBoxIGSCommon *pMsgBox = nullptr;
        CreateMessageBox(MSGBOX_LAYOUT_CLASS(CMsgBoxIGSCommonLayout, sessionKeeper_), &pMsgBox);
        pMsgBox->Initialize(I18N::Game::FailedToUse,
                            I18N::Game::NotEnoughSpacePleaseCheckFreeSpaceInYourInventory);
    }
    break;
    case 22: {
        CMsgBoxIGSCommon *pMsgBox = nullptr;
...
// (0xD2)(0x0C)
bool SessionNetworkUnit::ReceiveIGS_UpdateScript(const BYTE *pReceiveBuffer)
{
    ...
    if (g_InGameShopSystem.ScriptDownload(g_ConsoleDebug) == false)
    {
        return false;
    }
...
// (0xD2)(0x13)
bool SessionNetworkUnit::ReceiveIGS_EventItemlistCnt(const BYTE *pReceiveBuffer)
{
...
//(0xD2)(0x14)
bool SessionNetworkUnit::ReceiveIGS_EventItemlist(const BYTE *pReceiveBuffer)
{
...
// (0xD2)(0x15)
bool SessionNetworkUnit::ReceiveIGS_UpdateBanner(const BYTE *pReceiveBuffer)
{
...
bool SessionNetworkUnit::ReceiveEquippingInventoryItem(const BYTE *pReceiveBuffer)
{
    ...
    ITEM *pItem = sessionKeeper_.GameData()->FindInventoryItemBySlot(iItemPos);
...
bool SessionNetworkUnit::ReceivePeriodItemListCount(const BYTE *pReceiveBuffer)
{
    auto Data = (LPPMSG_PERIODITEMEX_ITEMCOUNT)pReceiveBuffer;
    return true;
}

bool SessionNetworkUnit::ReceivePeriodItemList(const BYTE *pReceiveBuffer)
{
    ...
    {
        ITEM *pItem = sessionKeeper_.GameData()->FindInventoryItemBySlot(Data->wItemSlotIndex);
...
bool SessionLegacyCalls::ReceivePeriodItemList(const BYTE *receiveBuffer)
{
    return sessionKeeper_.Network()->ReceivePeriodItemList(receiveBuffer);
}
#endif // KJH_ADD_PERIOD_ITEM_SYSTEM

BOOL SessionNetworkUnit::ReceiveStraightAttack(const BYTE *ReceiveBuffer, int Size, BOOL bEncrypted)
{
    ...
    int Index = FindCharacterIndex(SourceKey);
    int TargetIndex = FindCharacterIndex(TargetKey);
    if (!CharactersClient.IsValidIndex(TargetIndex) && AttackNumber != AT_SKILL_DARKSIDE &&
    ...
    if (TargetKey == HeroKey && IsPlayer(sc) && sc != Hero &&
        !IsCorrectSkillType_FrendlySkill(AttackNumber) &&
        !IsCorrectSkillType_Buff(AttackNumber))
    {
        muHelper_.AddTarget(SourceKey, true);
    }
    ...
    {
        auto isBeastUppercut = AttackNumber == AT_SKILL_BEAST_UPPERCUT ||
                               AttackNumber == AT_SKILL_BEAST_UPPERCUT_STR ||
                               AttackNumber == AT_SKILL_BEAST_UPPERCUT_MASTERY;
        if (sc != Hero && isBeastUppercut)
        {
            g_CMonkSystem.SetRageSkillAni(AttackNumber, *sc);
...
void SessionNetworkUnit::ReceiveDarkside(const BYTE *ReceiveBuffer)
{
    ...
    if (skill == AT_SKILL_DARKSIDE || skill == AT_SKILL_DARKSIDE_STR)
    {
        ...
        gameplay_.SetDarksideTargetIndex(Data->TargerIndex, skill);
    }
}

void SessionLegacyCalls::ReceiveDarkside(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDarkside(receiveBuffer);
}

GameplayExternalEventBatch SessionNetworkUnit::ProcessPacket(const BYTE *ReceiveBuffer,
                                                             int32_t Size)
{
    GameplayExternalEventBatch gameplayEvents;
    ...
    case 0xF1: {
        ...
        case 0x00: //receive join server
            if (ReceiveJoinServer(ReceiveBuffer))
            {
                gameplayEvents.Add({GameplayExternalEventKind::ResetInteraction});
                gameplayEvents.Add({GameplayExternalEventKind::ClearFollow});
            }
            break;
        case 0x01: //receive log in
        {
            //AddDebugText(ReceiveBuffer,Size);
            const auto deferLoginMessage = [this](int messageCode) noexcept {
                sessionKeeper_.NetworkStorage().PendingLoginMessageCode = messageCode;
            };
            switch (Data->Value)
            {
            case 0x20:
                sessionKeeper_.NetworkStorage().PendingLoginMessageCode = 0;
                CurrentProtocolState = RECEIVE_LOG_IN_SUCCESS;
                LogIn = 2;
                CheckHack();
                break;
            case 0x00:
                deferLoginMessage(RECEIVE_LOG_IN_FAIL_PASSWORD);
                break;
            case 0x01:
                sessionKeeper_.NetworkStorage().PendingLoginMessageCode = 0;
                CurrentProtocolState = RECEIVE_LOG_IN_SUCCESS;
                ...
                break;
            case 0x02:
                deferLoginMessage(RECEIVE_LOG_IN_FAIL_ID);
                break;
            case 0x03:
                deferLoginMessage(RECEIVE_LOG_IN_FAIL_ID_CONNECTED);
                break;
            case 0x04:
                deferLoginMessage(RECEIVE_LOG_IN_FAIL_SERVER_BUSY);
                break;
            case 0x05:
                deferLoginMessage(RECEIVE_LOG_IN_FAIL_ID_BLOCK);
                break;
            case 0x06:
                deferLoginMessage(RECEIVE_LOG_IN_FAIL_VERSION);
                g_ErrorReport.Write(L"Version dismatch. - Login\r\n");
                break;
            case 0x07:
            default:
                deferLoginMessage(RECEIVE_LOG_IN_FAIL_CONNECT);
                break;
            case 0x08:
                deferLoginMessage(RECEIVE_LOG_IN_FAIL_ERROR);
                break;
            case 0x09:
                deferLoginMessage(RECEIVE_LOG_IN_FAIL_NO_PAYMENT_INFO);
                break;
            case 0x0a:
                deferLoginMessage(RECEIVE_LOG_IN_FAIL_USER_TIME1);
                break;
            case 0x0b:
                deferLoginMessage(RECEIVE_LOG_IN_FAIL_USER_TIME2);
                break;
            case 0x0c:
                deferLoginMessage(RECEIVE_LOG_IN_FAIL_PC_TIME1);
                break;
            case 0x0d:
                deferLoginMessage(RECEIVE_LOG_IN_FAIL_PC_TIME2);
                break;
            case 0x11:
                deferLoginMessage(RECEIVE_LOG_IN_FAIL_ONLY_OVER_15);
                break;
            case 0x40:
                deferLoginMessage(RECEIVE_LOG_IN_FAIL_CHARGED_CHANNEL);
                break;
            case 0xc0:
            case 0xd0:
                deferLoginMessage(RECEIVE_LOG_IN_FAIL_POINT_DATE);
                break;
            case 0xc1:
            case 0xd1:
                deferLoginMessage(RECEIVE_LOG_IN_FAIL_POINT_HOUR);
                break;
            case 0xc2:
            case 0xd2:
                deferLoginMessage(RECEIVE_LOG_IN_FAIL_INVALID_IP);
                break;
            }
            break;
        }
        case 0x02: {
            const bool logoutSucceeded = ReceiveLogOut(ReceiveBuffer, bEncrypted);
            if (!logoutSucceeded)
            {
                //return ( FALSE);
            }
    ...
    case 0xF3: {
        ...
        case 0x03: //receive join map server
            ...
            {
                gameplayEvents.Add({GameplayExternalEventKind::StartArrivalCooldown, std::nullopt,
                                    World::ArrivalGateCooldownMilliseconds, 6});
                gameplayEvents.Add({GameplayExternalEventKind::ResetInteraction});
            }
            break;
    ...
    case 0x56:
        ...
        if (guildCreateResetCadence_)
        {
            gameplayEvents.Add({GameplayExternalEventKind::ResetCadence, std::nullopt, 0, 6});
        }
        break;
    case 0x65:
    ...
    case 0xB1: {
        ...
        {
        case 0x00:
            if (ReceiveChangeMapServerInfo(ReceiveBuffer))
            {
                gameplayEvents.Add({GameplayExternalEventKind::CancelWorldTransfer});
            }
            break;
    ...
    case 0xB2: {
        ...
        case 0x13:
            ...
            if (gateAttackCleared_)
            {
                gameplayEvents.Add({GameplayExternalEventKind::ClearAttack});
            }
            break;
...
GameplayExternalEventBatch SessionNetworkUnit::ProcessPacket(const PacketInfo &Packet)
{
    try
    {
        if (Packet.Route.role == ConnectionRole::ChatServer)
        {
            sessionKeeper_.Ui()->ProcessChatPacket(Packet.Route.connectionHandle,
                                                   Packet.ReceiveBuffer.get(), Packet.Size);
            return {};
        }
        if (sessionKeeper_.WorldUnit()->BlocksFrameAdmission())
            return {};
        return ProcessPacket(Packet.ReceiveBuffer.get(), Packet.Size);
    }
    catch (const std::exception &)
    {
        if (sessionKeeper_.WorldUnit()->HasLoadFailed())
            RejectWorldTransfer();
        return {};
    }
}

bool SessionNetworkUnit::CheckExceptionBuff(eBuffState buff, OBJECT *o, bool iserase)
{
    if (iserase)
    {
        switch (buff)
...
void SessionNetworkUnit::InsertBuffLogicalEffect(eBuffState buff, OBJECT *o, const int bufftime)
{
    if (o && o == &Hero->Object)
    {
        switch (buff)
...
void SessionNetworkUnit::ClearBuffLogicalEffect(eBuffState buff, OBJECT *o)
{
    if (o && o == &Hero->Object)
    {
        switch (buff)
        ...
        case eBuff_Hellowin5: {
            ...
            if (buff == eBuff_Hellowin2)
            {
                ...
                if (iBaseClass == CLASS_WIZARD || iBaseClass == CLASS_DARK ||
                    iBaseClass == CLASS_SUMMONER)
                {
                    CharacterMachine->CalculateMagicDamage();
                }
                else
...
void SessionNetworkUnit::InsertBuffPhysicalEffect(eBuffState buff, OBJECT *o)
{
    switch (buff)
    {
    case eBuff_CursedTempleProdection: {
        if (o->Type == MODEL_PLAYER)
        {
            ...
            DeleteEffect(BITMAP_SHOCK_WAVE, o, 10);
            vec3_t Light;
...
void SessionNetworkUnit::ClearBuffPhysicalEffect(eBuffState buff, OBJECT *o)
{
    switch (buff)
    {
    case eBuff_CursedTempleProdection: {
...
void SessionNetworkUnit::RegisterBuff(eBuffState buff, OBJECT *o, const int bufftime)
{
    if (!o)
    {
        return;
...
void SessionNetworkUnit::UnRegisterBuff(eBuffState buff, OBJECT *o)
{
    eBuffClass buffclasstype = g_IsBuffClass(buff);
...
void SessionLegacyCalls::AddDebugText(const unsigned char *buffer, int size)
{
    sessionKeeper_.Network()->AddDebugText(buffer, size);
} // OMF-00896
int SessionLegacyCalls::FindGuildName(wchar_t *name)
{
    return sessionKeeper_.Network()->FindGuildName(name);
} // OMF-00905
void SessionLegacyCalls::GuildTeam(CHARACTER *character)
{
    sessionKeeper_.Network()->GuildTeam(character);
} // OMF-00906
void SessionLegacyCalls::InitGuildWar()
{
    sessionKeeper_.Network()->InitGuildWar();
} // OMF-00907
void SessionLegacyCalls::ReceiveDeclareWarResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDeclareWarResult(receiveBuffer);
} // OMF-01019
void SessionLegacyCalls::ReceiveChatWhisperResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveChatWhisperResult(receiveBuffer);
} // OMF-00935
void SessionLegacyCalls::ReceiveDamage(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDamage(receiveBuffer);
} // OMF-00949
void SessionLegacyCalls::ReceiveSkillCount(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveSkillCount(receiveBuffer);
} // OMF-00963
void SessionLegacyCalls::ReceiveDropItem(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDropItem(receiveBuffer);
} // OMF-00973
void SessionLegacyCalls::ReceiveAddPoint(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveAddPoint(receiveBuffer);
} // OMF-00983
void SessionLegacyCalls::ReceiveAddPointExtended(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveAddPointExtended(receiveBuffer);
} // OMF-00984
void SessionLegacyCalls::ReceiveStatsExtended(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveStatsExtended(receiveBuffer);
} // OMF-00986
BOOL SessionLegacyCalls::ReceiveHelperItem(const BYTE *receiveBuffer, BOOL encrypted)
{
    return sessionKeeper_.Network()->ReceiveHelperItem(receiveBuffer, encrypted);
} // OMF-00989
void SessionLegacyCalls::ReceiveWeather(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveWeather(receiveBuffer);
} // OMF-00990
void SessionLegacyCalls::ReceiveEvent(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveEvent(receiveBuffer);
} // OMF-00991
void SessionLegacyCalls::ReceiveSummonLife(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveSummonLife(receiveBuffer);
} // OMF-00992
BOOL SessionLegacyCalls::ReceiveTrade(const BYTE *receiveBuffer, BOOL encrypted)
{
    return sessionKeeper_.Network()->ReceiveTrade(receiveBuffer, encrypted);
} // OMF-00993
void SessionLegacyCalls::ReceiveTradeResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveTradeResult(receiveBuffer);
} // OMF-00994
void SessionLegacyCalls::ReceiveTradeYourInventoryDelete(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveTradeYourInventoryDelete(receiveBuffer);
} // OMF-00995
void SessionLegacyCalls::ReceiveTradeMyGold(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveTradeMyGold(receiveBuffer);
} // OMF-00996
void SessionLegacyCalls::ReceiveTradeYourGold(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveTradeYourGold(receiveBuffer);
} // OMF-00997
void SessionLegacyCalls::ReceiveTradeYourResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveTradeYourResult(receiveBuffer);
} // OMF-00998
void SessionLegacyCalls::ReceiveTradeExit(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveTradeExit(receiveBuffer);
} // OMF-00999
void SessionLegacyCalls::ReceiveStorageStatus(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveStorageStatus(receiveBuffer);
} // OMF-01003
void SessionLegacyCalls::ReceivePartyResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceivePartyResult(receiveBuffer);
} // OMF-01005
void SessionLegacyCalls::ReceivePartyInfo(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceivePartyInfo(receiveBuffer);
} // OMF-01007
void SessionLegacyCalls::ReceivePartyLeave(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceivePartyLeave(receiveBuffer);
} // OMF-01008
void SessionLegacyCalls::ReceiveGuildResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveGuildResult(receiveBuffer);
} // OMF-01011
void SessionLegacyCalls::ReceiveGuildList(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveGuildList(receiveBuffer);
} // OMF-01012
void SessionLegacyCalls::ReceiveCreateGuildInterface(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCreateGuildInterface(receiveBuffer);
} // OMF-01014
void SessionLegacyCalls::ReceiveCreateGuildMasterInterface(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCreateGuildMasterInterface(receiveBuffer);
} // OMF-01015
void SessionLegacyCalls::ReceiveDeleteGuildViewport(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDeleteGuildViewport(receiveBuffer);
} // OMF-01016
void SessionLegacyCalls::ReceiveCreateGuildResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveCreateGuildResult(receiveBuffer);
} // OMF-01017
void SessionLegacyCalls::ReceiveGuildWarScore(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveGuildWarScore(receiveBuffer);
} // OMF-01022
void SessionLegacyCalls::ReceiveGuildInfo(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveGuildInfo(receiveBuffer);
} // OMF-01024
void SessionLegacyCalls::ReceiveGuildAssign(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveGuildAssign(receiveBuffer);
} // OMF-01025
void SessionLegacyCalls::ReceiveGuildRelationShip(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveGuildRelationShip(receiveBuffer);
} // OMF-01026
void SessionLegacyCalls::ReceiveGuildRelationShipResult(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveGuildRelationShipResult(receiveBuffer);
} // OMF-01027
void SessionLegacyCalls::ReceiveUnionViewportNotify(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveUnionViewportNotify(receiveBuffer);
} // OMF-01029
void SessionLegacyCalls::ReceiveUnionList(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveUnionList(receiveBuffer);
} // OMF-01030
void SessionLegacyCalls::ReceiveSoccerTime(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveSoccerTime(receiveBuffer);
} // OMF-01031
void SessionLegacyCalls::ReceiveSoccerGoal(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveSoccerGoal(receiveBuffer);
} // OMF-01033
void SessionLegacyCalls::ReceiveDevilSquareRank(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDevilSquareRank(receiveBuffer);
} // OMF-01044
void SessionLegacyCalls::ReceiveMatchGameCommand(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveMatchGameCommand(receiveBuffer);
} // OMF-01049
void SessionLegacyCalls::ReceiveDuelScore(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDuelScore(receiveBuffer);
} // OMF-01053
void SessionLegacyCalls::ReceiveDuelHP(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDuelHP(receiveBuffer);
} // OMF-01054
void SessionLegacyCalls::ReceiveDuelChannelList(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDuelChannelList(receiveBuffer);
} // OMF-01055
void SessionLegacyCalls::ReceiveDuelWatcherJoin(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDuelWatcherJoin(receiveBuffer);
} // OMF-01057
void SessionLegacyCalls::ReceiveDuelWatchEnd(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDuelWatchEnd(receiveBuffer);
} // OMF-01058
void SessionLegacyCalls::ReceiveDuelWatcherQuit(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDuelWatcherQuit(receiveBuffer);
} // OMF-01059
void SessionLegacyCalls::ReceiveDuelWatcherList(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDuelWatcherList(receiveBuffer);
} // OMF-01060
void SessionLegacyCalls::ReceiveDuelRound(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveDuelRound(receiveBuffer);
} // OMF-01062
void SessionLegacyCalls::NotifySoldItem(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->NotifySoldItem(receiveBuffer);
} // OMF-01071
void SessionLegacyCalls::NotifyClosePersonalShop(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->NotifyClosePersonalShop(receiveBuffer);
} // OMF-01072
void SessionLegacyCalls::ReceiveOption(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveOption(receiveBuffer);
} // OMF-01085
void SessionLegacyCalls::ReceiveEventChip(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveEventChip(receiveBuffer);
} // OMF-01087
void SessionLegacyCalls::ReceiveMutoNumber(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveMutoNumber(receiveBuffer);
} // OMF-01089
void SessionLegacyCalls::ReceiveEventCount(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveEventCount(receiveBuffer);
} // OMF-01093
void SessionLegacyCalls::ReceiveQuestHistory(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveQuestHistory(receiveBuffer);
} // OMF-01094
void SessionLegacyCalls::ReceiveQuestState(const BYTE *receiveBuffer)
{
    sessionKeeper_.Network()->ReceiveQuestState(receiveBuffer);
} // OMF-01095
void SessionLegacyCalls::ReceiveServerConnect(const BYTE *ReceiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveServerConnect(ReceiveBuffer);
} // OMF-00909
void SessionLegacyCalls::ReceiveMuHelperStatusUpdate(std::span<const BYTE> ReceiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveMuHelperStatusUpdate(ReceiveBuffer);
} // OMF-00928
void SessionLegacyCalls::ReceiveMoveCharacter(std::span<const BYTE> ReceiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveMoveCharacter(ReceiveBuffer);
} // OMF-00938
BOOL SessionLegacyCalls::ReceiveTeleport(const BYTE *ReceiveBuffer, BOOL bEncrypted)
{
    return sessionKeeper_.Network()->ReceiveTeleport(ReceiveBuffer, bEncrypted);
} // OMF-00940
void SessionLegacyCalls::ReceiveCreateMonsterViewport(const BYTE *ReceiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveCreateMonsterViewport(ReceiveBuffer);
} // OMF-00946
void SessionLegacyCalls::ReceiveAttackDamageExtended(const BYTE *ReceiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveAttackDamageExtended(ReceiveBuffer);
} // OMF-00952
void SessionLegacyCalls::ReceiveAction(const BYTE *ReceiveBuffer, int Size)
{
    return sessionKeeper_.Network()->ReceiveAction(ReceiveBuffer, Size);
} // OMF-00953
BOOL SessionLegacyCalls::ReceiveMonsterSkill(const BYTE *ReceiveBuffer, int Size, BOOL bEncrypted)
{
    return sessionKeeper_.Network()->ReceiveMonsterSkill(ReceiveBuffer, Size, bEncrypted);
} // OMF-00958
BOOL SessionLegacyCalls::ReceiveMagic(const BYTE *ReceiveBuffer, int Size, BOOL bEncrypted)
{
    return sessionKeeper_.Network()->ReceiveMagic(ReceiveBuffer, Size, bEncrypted);
} // OMF-00959
BOOL SessionLegacyCalls::ReceiveMagicContinue(const BYTE *ReceiveBuffer, int Size, BOOL bEncrypted)
{
    return sessionKeeper_.Network()->ReceiveMagicContinue(ReceiveBuffer, Size, bEncrypted);
} // OMF-00960
void SessionLegacyCalls::ReceiveDie(const BYTE *ReceiveBuffer, int Size)
{
    return sessionKeeper_.Network()->ReceiveDie(ReceiveBuffer, Size);
} // OMF-00967
void SessionLegacyCalls::ReceiveCreateMoney(std::span<const BYTE> ReceiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveCreateMoney(ReceiveBuffer);
} // OMF-00968
void SessionLegacyCalls::ReceiveCreateItemViewportExtended(std::span<const BYTE> ReceiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveCreateItemViewportExtended(ReceiveBuffer);
} // OMF-00969
void SessionLegacyCalls::ReceiveDeleteItemViewport(const BYTE *ReceiveBuffer)
{
    return sessionKeeper_.Network()->ReceiveDeleteItemViewport(ReceiveBuffer);
} // OMF-00970
void SessionLegacyCalls::RequestInventorySync()
{
    return sessionKeeper_.Network()->RequestInventorySync();
} // OMF-00971
bool SessionLegacyCalls::CheckExceptionBuff(eBuffState buff, OBJECT *object, bool isErase)
{
    return sessionKeeper_.Network()->CheckExceptionBuff(buff, object, isErase);
} // OMF-01236
void SessionLegacyCalls::InsertBuffLogicalEffect(eBuffState buff, OBJECT *object,
                                                 const int buffTime)
{
    return sessionKeeper_.Network()->InsertBuffLogicalEffect(buff, object, buffTime);
} // OMF-01237
void SessionLegacyCalls::ClearBuffLogicalEffect(eBuffState buff, OBJECT *object)
{
    return sessionKeeper_.Network()->ClearBuffLogicalEffect(buff, object);
} // OMF-01238

#ifdef PK_ATTACK_TESTSERVER_LOG
void SessionNetworkUnit::PrintPKLog(CHARACTER *pCha)
{
