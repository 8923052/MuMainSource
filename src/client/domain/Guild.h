...
class CGuildCache final : protected SessionLegacyCalls
{
  public:
    explicit CGuildCache(SessionKeeper &keeper);
    ~CGuildCache();
  ...
  public:
    void Reset();
    void PrepareMark(int index);
    const LogicalRenderAssetLease *MarkTexture(int index, bool blend) const;

  private:
    std::unordered_map<int, std::array<LogicalRenderAssetLease, 2>> textures_;
...
namespace GuildMarkConstants
{
inline constexpr int Count = 2000;
inline constexpr int EditIndex = Count - 1;
inline constexpr int NameBufferSize = 9;
inline constexpr int MarkSize = 64;
} // namespace GuildMarkConstants

struct MARK_t
{
    int Key;
    wchar_t UnionName[GuildMarkConstants::NameBufferSize];
    wchar_t GuildName[GuildMarkConstants::NameBufferSize];
    unsigned char Mark[GuildMarkConstants::MarkSize];
};
